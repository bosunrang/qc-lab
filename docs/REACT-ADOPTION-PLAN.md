...
| 1 trang còn lại | Chưa chuyển, vẫn chạy code cổ điển như cũ. |

### Thứ tự dự kiến cho các trang còn lại

1. Nhập QC & Biểu đồ — phức tạp và dùng nhiều nhất, làm cuối cùng. **(tiếp theo)**
...
đúng trạng thái, mở modal chi tiết và hướng dẫn 8 bước không lỗi) → xoá code
cũ → chạy lại `npm test` + `npm run a11y-audit` +
`npm run check-build-freshness` + `npm run nce-check` lần nữa. Tổng: `npm
test` 488/488 (giảm dần từ 613 ban đầu do xoá các test file của code cổ điển
đã dọn qua từng trang), `npm run a11y-audit` 0 vi phạm giữ nguyên baseline
(bao gồm modal `users:edit-permissions`/`settings:lis-queue`/7 modal của
trang `manage`/2 modal của trang `reagent`/3 modal của trang `sigma`/1 modal
`actions:nce-guide`), `npm run check-build-freshness` khớp cả 4 bundle,
`npm run nce-check` 91/91 đạt.
