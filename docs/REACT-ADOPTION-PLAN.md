...
| 3 trang còn lại | Chưa chuyển, vẫn chạy code cổ điển như cũ. |

### Thứ tự dự kiến cho các trang còn lại

1. Phân tích Westgard — Web Worker (≥3000 điểm) + canvas + XLSX. **(tiếp theo)**
2. Khắc phục sự cố (NCE) — có bộ Playwright riêng vì lỗi ở đây là lỗi trạng thái `rerender()`.
3. Nhập QC & Biểu đồ — phức tạp và dùng nhiều nhất, làm cuối cùng.

## Kiểm chứng đã chạy (đầy đủ, kể cả bước dọn dẹp) cho từng trang đã xong

Cả tám trang đều theo cùng một quy trình: `npm run build:pilot` (gồm
...
PDF theo kỳ/tổng hợp không lỗi) → xoá code cũ → chạy lại `npm test` +
`npm run a11y-audit` + `npm run check-build-freshness` lần nữa. Tổng:
`npm test` 515/515 (giảm dần từ 613 ban đầu do xoá các test file của code cổ
...
của trang `manage`/2 modal của trang `reagent`/3 modal của trang `sigma`),
`npm run check-build-freshness` khớp cả 4 bundle.
