# Kế hoạch app — rà soát nghiệp vụ theo từng thẻ
...
> thẻ trả lời đúng 3 câu: *nghiệp vụ của thẻ này là gì*, *app đã có gì*,
> *còn lại việc gì*.
>
...
Người dùng sở hữu giao diện app. Nhiều chỗ đã được người dùng tự cải tiến
...
- **Không sửa giao diện app cho giống app cũ.** Lệch app cũ không phải lỗi.
- Cần dựng màn hình mới thì **xem app cũ để hiểu luồng thao tác và thông tin
  cần hiển thị**, rồi viết code mới theo hệ thống app (token, component dùng
...
CÓ mà app THIẾU", nên mọi cải tiến giao diện của người dùng đều làm gate đỏ,
...
| Gate | Trước | Nay |
|---|---|---|
| `app:style-parity` | so computed style với app cũ, FAIL nếu khác | **đã xoá** (script + baseline) |
| `app:ui-parity` | FAIL nếu app thiếu class/dòng chữ so app cũ | **đã xoá khi cắt app cũ** |
| `app:css-parity` | FAIL nếu app cũ có rule CSS mà app không | **class chết**: FAIL khi class dùng trong `.tsx` không có rule nào trong `app/renderer/styles/`; không còn đọc `assets/` |
...
- **57 khối `@media(max-width:N)` với N ≤ 980** (520/560/640/760/900/980) khỏi
  `app/renderer/styles/**` — bố cục xếp dọc cho điện thoại/tablet, modal
...
Electron nay có `minWidth: 1024` / `minHeight: 700` (`app/main/index.ts`).
...
Manifest/baseline UI parity đã được xoá khi cắt app cũ vì chúng không còn là
công cụ có ý nghĩa. **Đừng thêm lại breakpoint < 1024 hay viewport hẹp hơn.**
...
1. **Định nghĩa chuẩn** — westgard.com, ISO 15189, ISO/TS 20914, CLIA/EFLM.
   Gate: `app/tests/westgard-standard.test.mjs` (174 phép kiểm, sống tiếp
   sau khi cắt app cũ).
2. **Quyết định sản phẩm** của người dùng, ghi trong tài liệu này.
3. **App cũ** — đã được dùng để phát hiện lệch trong lúc port (2442 phép đối
   chiếu), rồi được xoá cùng bộ dò đối chiếu khi cắt.
...
Phương pháp đã hoàn tất: liệt kê mọi thao tác nghiệp vụ của app cũ rồi đối
chiếu với kênh IPC + trang tương ứng của app (`app/main/preload.ts`,
`app/renderer/pages/`).
...
- **Mean/SD "Dự kiến" (làm 2026-09-12) — port đúng nghiệp vụ app cũ.** Người
  ...
  - Gate: `app/tests/planned-targets.test.mjs` (10 nhóm phép kiểm; đã chứng
...
- **Cây danh mục** (2026-09-12): `renderer/lib/entry-tree-navigation.ts` —
  ...
  bằng chuột. Lệch app cũ có chủ đích: nút MÁY của app luôn mở và không có
...
- **Đã thống nhất 22/09/2026:** mọi luật chuỗi đánh giá các điểm chưa hủy
  trong cùng giai đoạn vận hành. Không tự xóa bằng chứng chỉ vì một điểm bị
  loại. Hồ sơ khắc phục không bị hủy, đã duyệt và xác nhận hiệu quả mở giai
  đoạn mới cho xét nghiệm: do DB chỉ lưu ngày, điểm sau ngày hoàn thành mới
  bắt đầu chuỗi mới. Áp dụng cùng mốc cho Entry, Westgard và CUSUM.
- Ngày + mã run là định danh chung; thứ tự mã tự nhiên (1, 2, 10).
  Lô lịch sử giữ snapshot Mean/SD từng điểm, chỉ dùng target của lô làm
  fallback. Hai cách xem lô cũ dùng chung `historical-westgard.ts`, gồm luật
  liên mức của nhóm snapshot lịch sử; lô legacy mất nhóm có ghi rõ giới hạn.
- “Thực tế” là thống kê mô tả, không tự áp làm dải kiểm soát: hiện n được
  chấp nhận, SD mẫu n−1 và CV theo |Mean|; n<2 chưa đủ tính SD, mẫu nhỏ ghi
  tạm tính. Điều kiện lập dải PXN riêng ở mục Nhập QC không thay đổi.
- Bảng và xuất Excel/PDF dùng chung lựa chọn lô hiện hành/lô cũ; export là
  bảng dữ liệu, không hứa chứa ảnh biểu đồ. Có mã run, target từng điểm,
  kết luận điểm, trạng thái run và bằng chứng lịch sử.
...
**Cập nhật sau rà soát 22/09/2026:**

- Nguồn EFLM có cột riêng `tests.eflm_tea`; cột TEa dùng chung cũ không đủ
  căn cứ để migration tự xác nhận. Người dùng nhập/xác nhận lại EFLM một lần;
  snapshot lịch sử không bị ghi đè. Mã nguồn và mô tả tiêu chí TEa tách biệt.
- Kỳ mới chốt TEa và tiêu chí theo mức. Đổi lô quy đổi theo tiêu chí đã chốt;
  kỳ cũ thiếu tiêu chí không được suy lại bằng catalog hiện tại khi đổi Mean.
- CV cohort là snapshot: sửa Bias/MU không tự nạp CV mới. Nạp lô là hành động
  chủ động, ghi người/thời điểm xác nhận rà soát và fingerprint dữ liệu/luật/NCE.
  Dữ liệu nền thay đổi sẽ gỡ hiệu lực rà soát, không sửa số CV đã lưu.
- `eligible` chỉ đủ điều kiện số học/cổng thô, KHÔNG là duyệt multirule.
  Đề xuất QC đòi xác nhận rà soát SOP; đề xuất chung đòi tất cả mức đủ điều kiện.
  Thiếu snapshot Mean/SD không được coi là nhóm ổn định. Không loại điểm xấu
  khỏi CV và không tự tuyên bố chứng nhận ISO.
- MU thiếu thành phần không kết luận đạt TEa; Excel/PDF không xuất U tạm tính
  như số cuối cùng. Nhập u chuẩn theo %, dùng U/k của chứng chỉ (chỉ /2 nếu k=2).
- Giao diện bảo vệ request trễ/đổi xét nghiệm và no-op blur. Mọi write Sigma
  gộp dữ liệu + audit trong transaction. Chi tiết: `SIGMA-REVIEW-2026-09-22.md`.
...
`actionUpdateBiasHint`); app để người dùng tự gõ. Tiện ích, không phải
nghiệp vụ bắt buộc. ⬜
...
**Máy chủ / máy trạm nội bộ (2026-09-22):** QC Lab chạy HTTP LAN cố định ở
cổng `3200`; máy nhân viên dùng trình duyệt mở địa chỉ IPv4 mà mục “Địa chỉ
cho máy nhân viên…” ở khay hệ thống hiển thị. Dữ liệu SQLite chỉ ở máy chủ.
Bấm dấu X chỉ ẩn QC Lab xuống khay để server tiếp tục chạy; chỉ “Thoát QC Lab”
mới dừng server và ngắt máy trạm. Cổng bận là lỗi khởi động rõ ràng, không tự
đổi cổng vì máy trạm cần một địa chỉ ổn định. Chỉ dùng trong mạng nội bộ tin
cậy, không mở cổng này ra Internet.
...
**Nghiệp vụ app đã đầy đủ so với app cũ.** Rà lại toàn bộ thao tác người
...
Test: `app/tests/sigma-qc-design.test.mjs` (50 phép kiểm theo bảng công bố).
...
Test: `app/tests/sigma-metrics.test.mjs` (viết lại theo công thức thay vì so
...
Test: `app/tests/sigma-cohort.test.mjs` mục (5).

**Verify chung:** `app:typecheck` sạch, `app:test` **78/78** (thêm
`sigma-qc-design.test.mjs`; `cross-app` lên 2463 phép), `app:build` sạch,
`app:css-parity` đạt. Cả ba bản sửa đều được chứng minh test BẮT ĐƯỢC lỗi
...
3. Verify: `app:typecheck` + `app:test` + `app:build` sạch, cộng chạy
   thật (Electron hoặc `app:dev`) và **đọc lại dữ liệu từ nguồn**, không tin
...
**Không làm:** mở lại một quyết định nghiệp vụ đã chốt chỉ vì bản ghi lịch sử
của app cũ từng làm khác.
...
1. ✅ Đối chiếu Westgard/Sigma giữa hai bản — đã đạt trước khi cắt; bộ dò đã
   được xoá vì không còn nguồn app cũ để đối chiếu.
2. ✅ Hết 4 mục ưu tiên 1–2 ở bảng mục 4 (xong 2026-09-12).
3. ✅ Người dùng xác nhận giao diện app đã đủ dùng (2026-09-22; tiêu chí này
   thuộc về người dùng, **không** đo bằng gate parity).

**Đã cắt (2026-09-22):** `build.files` chỉ đóng gói `app-dist`, Electron
khởi động `app-dist/main/index.js`, và script phát hành build app. Không
có file SQLite thử nào tại user-data để xoá; lần chạy Electron đầu tiên sẽ tạo
database trống và yêu cầu khởi tạo admin. Đã xoá
`app/tests/cross-app-westgard-sigma.test.mjs` (bộ dò trôi khi port đã hết
tác dụng), loại `.wasm` của sql.js khỏi gói Electron (Electron dùng
`node:sqlite`, không bao giờ tải WASM), và xoá toàn bộ mã nguồn legacy.
