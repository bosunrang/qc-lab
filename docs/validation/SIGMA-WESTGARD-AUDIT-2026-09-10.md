...
| Cohort IQC không kiểm trạng thái in-control | **ĐÃ SỬA 11/09** (lệch app cũ có chủ đích) — ISO/TS 20914 lấy `u(Rw)` từ dữ liệu IQC "đại diện cho hoạt động thường quy ĐÃ được thẩm định sau khi quản lý QC", nên nhóm còn điểm vượt ±3SD mà CHƯA có hồ sơ NCE đã duyệt + kết luận hiệu quả (khớp theo `point_id`) nay mang trạng thái mới `out-of-control` và không được `eligible`. **KHÔNG tự loại điểm mất kiểm soát khỏi CV** — loại theo kết quả là selection bias, CV sẽ đẹp giả; test chốt rằng CV của nhóm có điểm lệch phải CAO HƠN nhóm sạch. Giới hạn đã ghi rõ tại chỗ: đây là cổng THÔ (`1-3s`), không chạy toàn bộ multirule (module cohort cố ý không import engine Westgard), nên nhóm qua cổng vẫn cần người phụ trách rà soát biểu đồ. `app-v2/tests/sigma-cohort.test.mjs` mục (5) |
| Bảng gợi ý Sigma Rules chưa đúng N/R theo số mức QC | **ĐÃ SỬA 11/09** (lệch app cũ có chủ đích) — Westgard công bố HAI bảng khác nhau cho 2 mức và 3 mức QC, khác cả bộ luật lẫn N/R. `QCCore.westgardSigmaRules(sigma)` app cũ chỉ nhận sigma nên dùng một bảng pha trộn (thêm `4-1s` ở 5σ, thêm `8x` ở 4σ, lấy `6x` — luật của bảng 3 mức — cho dưới 4σ, và N=8 ở 4σ/3σ, con số không có trong bảng nào). `sigmaQualityDesign(sigma, levelCount)` nay trả đúng hai bảng kèm `tier`/`alternatives`, giao diện ghi rõ đang áp bảng mấy mức. Sigma chưa tính được thì trả `null` thay vì tier `<3` (app cũ nói "phương pháp chưa đủ năng lực" cho một ô trống vì `Number(null)` ra 0). `app-v2/tests/sigma-qc-design.test.mjs` (50 phép kiểm theo bảng công bố) + mục 7b của `cross-app` |
| Mô hình MU gán sai ý nghĩa `u(Cref)` | **ĐÃ SỬA 11/09** (lệch app cũ có chủ đích) — Nordtest TR 537: `u(bias) = √(RMS_bias² + u(Cref)²)` với `u(Cref)` là độ không đảm bảo của GIÁ TRỊ GÁN (CRM lấy `U(Cref)/2`; PT/EQA theo ISO 13528 lấy `U/2` của giá trị gán). Cả hai bản đang nạp vào chỗ đó SD chuỗi bias quan sát / √n — sai số chuẩn của chính ước lượng bias, và là số app TỰ SUY nên ngân sách MU luôn "đủ thành phần" một cách giả tạo. Nay `uCref` là ô nhập riêng theo từng mức; thiếu thì VẮNG MẶT và ngân sách bị đánh dấu chưa đủ (đúng nguyên tắc đã áp cho `u(cal)`), tắt nhánh bias thì không đòi. Số cũ giữ lại dưới tên đúng `biasSem`, chỉ tham khảo. `sigma-metrics.test.mjs` (viết lại theo công thức, không so app cũ) + mục 9 của `cross-app` |
...
> **CẬP NHẬT 11/09/2026 — kết luận điều hành dưới đây ĐÃ LỖI THỜI.** Cả ba lỗi
> mức cao nó nêu (Entry↔Westgard lệch tập mức vận hành; `accepted:true` cho
> điểm bị luật liên mức loại; cohort IQC không kiểm in-control) đều đã sửa, cùng
> hai lỗi thuật toán `2of3-2s`/`7T`, bảng Sigma Rules theo số mức QC và ý nghĩa
> `u(Cref)`. Xem bảng trạng thái ở trên. Giữ nguyên văn phần thân để truy xuất.
