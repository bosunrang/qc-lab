...
Migration vẫn ở pha song song: `src/compat/modular-pilot.global.ts` (6.283
dòng, KHÔNG giảm qua Pha G/H — mọi module classic retire đều đổ glue vào đây
nguyên văn, không rút gọn) còn công bố bridge global để router/HTML builder
gọi hàm nghiệp vụ qua `root.X`. Bridge là cơ chế chuyển tiếp, không phải kiến
trúc đích — đây là hạng mục LỚN NHẤT còn lại trong tiêu chí hoàn thành cuối
cùng (mục 7), và Pha H2 (event delegation + bỏ CSP `unsafe-inline`) đã đổi
