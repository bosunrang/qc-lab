(function(root,factory){
  ...
  function cusumScan(points,mean,sd,k=0.5,h=4,maWindow=0){
    mean=Number(mean);sd=Number(sd);
    ...
    (points||[]).forEach(p=>{
      ...
      if(maWindow){queue.push(z);if(Number.isFinite(z)){maSum+=z;maCount++;}if(queue.length>maWindow){const old=queue.shift();if(Number.isFinite(old)){maSum-=old;maCount--;}}ma.push(maCount?maSum/maCount:NaN);}
    });
    return maWindow?{cPos:cPosArr,cNeg:cNegArr,flags,k,h,ma}:{cPos:cPosArr,cNeg:cNegArr,flags,k,h};
  }
  function cusum(points,mean,sd,k=0.5,h=4){return cusumScan(points,mean,sd,k,h,0);}
  function cusumMovingAverage(points,mean,sd,k=0.5,h=4,window=5){return cusumScan(points,mean,sd,k,h,window);}
  ...
  function movingAverage(points,mean,sd,window=5){
    ...
    const queue=[],out=[];let sum=0,count=0;
    (points||[]).forEach(p=>{const z=pointZ(p,mean,sd),finite=Number.isFinite(z);queue.push(z);if(finite){sum+=z;count++;}if(queue.length>window){const old=queue.shift();if(Number.isFinite(old)){sum-=old;count--;}}out.push(count?sum/count:NaN);});
    return out;
  }
  ...
  function auditSha256Core(ascii){
    ...
    ascii=unescape(encodeURIComponent(String(ascii)));const asciiBitLength=ascii[lengthProperty]*8;ascii+='\x80';while(ascii[lengthProperty]%64-56)ascii+='\x00';
    for(let i=0;i<ascii[lengthProperty];i++)words[i>>2]|=ascii.charCodeAt(i)<<((3-i)%4)*8;words[words[lengthProperty]]=((asciiBitLength/maxWord)|0);words[words[lengthProperty]]=asciiBitLength;
  ...
  function auditEntryHashCore(entry){const{hash,prevHash,...payload}=entry||{};return auditSha256Core(String(entry&&entry.prevHash||'')+'|'+auditCanonicalCore(payload));}
  ...
  return{STATE_SCHEMA_VERSION,ROLE_SET,PAGE_SET,WG_RULE_REGISTRY,WG_RULES,WG_DEFAULT_ON,WG_RULE_DESCRIPTIONS,RULE_ACTIONS,RULE_SCOPES,WG_ALERT_RULES,WG_SE_RULES,WG_RE_RULES,cleanText,cleanId,finiteNumber,stats,westgard,westgardMulti,pointTarget,pointZ,westgardByPoint,westgardLatestRules,westgardLatestRulesFromZ,westgardMultiByPoint,cusum,cusumMovingAverage,movingAverage,erf,normalCdf,dpmoFromSigma,sigmaMetric,systematicShiftCritical,uncertaintyBudget,westgardSigmaRules,targetFromLimits,limitsFromTarget,ruleEnabled,defaultRuleAction,resolveRuleAction,defaultRuleScope,resolveRuleScope,ruleOnInScope,ruleVerdictLevel,primaryErrorRule,errorType,fixHint,auditCanonical:auditCanonicalCore,auditSha256:auditSha256Core,auditEntryHash:auditEntryHashCore,verifyAuditChain,validateBackup,validateStateInvariants,sanitizeBackup};
});
