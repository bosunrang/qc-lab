...
function actionSection(key,badge,title,hint,bodyHtml,chipInfo,openSet){
  if(key==='cause'&&actionStepRenderInputs)bodyHtml=globalThis.actionCauseStepPresentation(actionStepRenderInputs.cause);
  if(key==='patient'&&actionStepRenderInputs)bodyHtml=globalThis.actionPatientStepPresentation(actionStepRenderInputs.patient);
  if(key==='eff'&&actionStepRenderInputs)bodyHtml=globalThis.actionEffectivenessStepPresentation(actionStepRenderInputs.eff);
...
function actionFormHtml(issueCount){
  ...
  actionStepRenderInputs={cause:{causeCategory:actSel('aCauseCategory','Nhóm nguyên nhân',ACT_CAUSE_OPTS,form.causeCategory,'onchange="syncActionSuggestions()"'),cause:esc(form.cause||''),causeSuggest:actionSuggestBox('aCause',actionCausePhrases(form.causeCategory)),action:esc(form.action||''),actionSuggest:actionSuggestBox('aAct',actionActionPhrases(form.errorType)),completedDate:dateBox('aActionCompletedDate',form.actionCompletedDate||'','action-date'),biasBefore:escAttr(form.biasBefore||''),sigmaBias:sigmaBiasChip,biasAfter:escAttr(form.biasAfter||''),threshold:actionBiasThresholdHtml(biasInfo),rerun:formAction?actionRerunEvidenceHtml(formAction,formRerun,state.tests.find(x=>x.id===formAction.testId)):'',containmentHeld:form.containmentStatus==='held',releaseStatus:actSel('aReleaseStatus','Quyết định cho phép trở lại',ACT_RELEASE_OPTS,form.releaseStatus),releaseDate:dateBox('aReleaseDate',form.releaseDate||'','action-date'),releaseBy:escAttr(form.releaseBy||''),releaseNote:escAttr(form.releaseNote||''),releaseSuggest:actionSuggestBox('aReleaseNote',ACT_SUGGEST.releaseNote)},patient:{reference:actionBiasReferenceHtml(biasInfo),impact:actSel('aPatientImpact','Kết luận ảnh hưởng',ACT_PATIENT_OPTS,form.patientImpact),action:esc(form.patientAction||''),suggest:actionSuggestBox('aPatientAction',ACT_SUGGEST.patientAction)},eff:{status:actSel('aEffectivenessStatus','Kết luận hiệu lực',ACT_EFF_OPTS,form.effectivenessStatus||'pending'),date:dateBox('aEffectivenessDate',form.effectivenessDate||'','action-date'),note:esc(form.effectivenessNote||''),noteSuggest:actionSuggestBox('aEffectivenessNote',ACT_SUGGEST.effectivenessNote),severity:actSel('aResidualSeverity','Mức độ còn lại',ACT_SEVERITY_OPTS,form.residualSeverity,'onchange="syncActionResidualRiskScore()"'),occurrence:actSel('aResidualOccurrence','Khả năng xảy ra còn lại',ACT_OCCURRENCE_OPTS,form.residualOccurrence,'onchange="syncActionResidualRiskScore()"'),detectability:actSel('aResidualDetectability','Khả năng không phát hiện còn lại',ACT_DETECT_OPTS,form.residualDetectability,'onchange="syncActionResidualRiskScore()"'),level:actSel('aResidualRiskLevel','Phân loại nguy cơ còn lại',ACT_RISK_LEVEL_OPTS,form.residualRiskLevel,'onchange="syncActionResidualRiskScore()"'),scoreClass:escAttr(['low','medium','high','critical'].includes(form.residualRiskLevel)?form.residualRiskLevel:'none'),score:actionResidualRiskScore(form)||'—',basis:escAttr(form.residualRiskBasis||''),basisSuggest:actionSuggestBox('aResidualRiskBasis',ACT_SUGGEST.residualRiskBasis)}};
  const formBodyHtml=`${actionIncidentBanner(form,editing)}<div class="action-form-section"><div class="action-form-section-title"><span>Hồ sơ</span><div><b>Nhận diện sự cố</b><small>Có thể lưu ngay sau khi kiểm soát tức thời; không cần chờ điều tra xong</small></div></div>
     ...
     ${actionSection('immediate','1','Kiểm soát và xử lý tức thời','Phần tối thiểu bắt buộc để mở hồ sơ NCE; kết luận ảnh hưởng bệnh nhân ghi ở mục 7',globalThis.actionImmediateStepPresentation({containmentSelectHtml:actSel('aContainment','Phạm vi kiểm soát tức thời',ACT_CONTAIN_OPTS,form.containmentStatus),containmentNoteValueHtml:escAttr(form.containmentNote||''),containmentNoteSuggestHtml:actionSuggestBox('aContainmentNote',ACT_SUGGEST.containmentNote),correctionTextHtml:esc(form.correction||''),correctionSuggestHtml:actionSuggestBox('aCorrection',ACT_SUGGEST.correction)}),actionSectionChip(miss.immediate),openSet)}
     ${actionSection('risk','2','Đánh giá nguy cơ','RPN được tính tự động; mức phân loại phải truy xuất được về SOP của đơn vị',globalThis.actionRiskStepPresentation({severitySelectHtml:actSel('aRiskSeverity','Mức độ ảnh hưởng',ACT_SEVERITY_OPTS,form.riskSeverity,'onchange="syncActionRiskScore()"'),occurrenceSelectHtml:actSel('aRiskOccurrence','Khả năng xảy ra',ACT_OCCURRENCE_OPTS,form.riskOccurrence,'onchange="syncActionRiskScore()"'),detectabilitySelectHtml:actSel('aRiskDetectability','Khả năng không phát hiện',ACT_DETECT_OPTS,form.riskDetectability,'onchange="syncActionRiskScore()"'),levelSelectHtml:actSel('aRiskLevel','Phân loại nguy cơ',ACT_RISK_LEVEL_OPTS,form.riskLevel,'onchange="syncActionRiskScore()"'),scoreClassHtml:escAttr(['low','medium','high','critical'].includes(form.riskLevel)?form.riskLevel:'none'),scoreHtml:actionRiskScore(form)||'—',basisValueHtml:escAttr(form.riskBasis||''),basisSuggestHtml:actionSuggestBox('aRiskBasis',ACT_SUGGEST.riskBasis)}),actionSectionChip(miss.risk),openSet)}
     ${actionSection('check','3','Checklist điều tra','Ghi rõ khi bất thường hoặc không áp dụng',globalThis.actionInvestigationStepPresentation([
     ...
     <div class="action-form-submit"><div><b>${editing?'Cập nhật tiến độ hồ sơ':'Lưu ngay ở trạng thái đang điều tra'}</b><span>Chỉ cần hoàn tất phần nhận diện và kiểm soát tức thời để lưu; phê duyệt chỉ xuất hiện khi hồ sơ đủ điều kiện khép vòng.</span></div><div class="action-submit-buttons">${btn(editing?'Hủy chỉnh sửa':'Đóng','closeActionForm()','ghost')}${btn(editing?'Lưu thay đổi':'Lập hồ sơ NCE','addAction()','teal')}</div></div>`;
  return globalThis.actionFormPanelPresentation({editing,formOpen,guideButtonHtml:btn('Quy trình 8 bước','openActionGuide()','ghost sm'),closedHtml:actionFormClosedHtml(issueCount),formBodyHtml});
}
