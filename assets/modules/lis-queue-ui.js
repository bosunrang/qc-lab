...
async function lisGatewaySaveSettings(){
  ...
  let outcome;try{outcome=await globalThis.LisGatewayCommand.apply(plan&&plan.ok?plan.settings:{enabled,url,token});}catch(e){await infoDialog('Không lưu được cấu hình LIS Gateway trên máy này.');return;}
  if(outcome.status==='disabled'){await infoDialog('Đã tắt nhận kết quả QC từ LIS trên máy này.',{type:'success'});return;}
  const result=outcome.result;if(result.ok)await infoDialog(`Đã kết nối. ${result.pending} kết quả chờ nhận${result.unresolved?`, ${result.unresolved} chưa khớp cấu hình mapping`:''}.`,{type:'success'});
}
