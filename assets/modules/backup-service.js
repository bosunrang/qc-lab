...
async function inspectBackupText(text,size=0){const parsed=await parseBackupPackage(text),next=prepareBackupState(parsed.incoming);return{meta:parsed.meta,summary:backupSummary(next),state:next,size:Number(size)||backupTextBytes(text)};}
