/* ===== WESTGARD PAGE ROUTE ===== */
function wgMultiViews(t){return globalThis.westgardMultiViews(t,wgPrevOpen);}
function wgTogglePrevLot(level){wgPrevOpen=globalThis.westgardUiState.toggleOpen(wgPrevOpen,selTest+'|'+level);rerender();}
...
function wgArchivedGroups(){return globalThis.westgardArchivedGroups(state.lotGroups);}
function wgSetViewMode(mode){wgViewMode=globalThis.westgardUiState.viewMode(mode);rerender();}
function wgSetChartMode(mode){wgChartMode=globalThis.westgardUiState.chartMode(mode);rerender();}
function wgChartModeTabs(){return globalThis.westgardModeTabs.chart(wgChartMode);}
...
function pageWestgardCusum(t){const cfg=testCusumConfig(t),levels=globalThis.westgardCusumLevels(t);return globalThis.westgardCusumPageHtml({test:t,cfg,levels,canWrite:canWrite()});}
function wgSetArchivedGroup(id){const next=globalThis.westgardUiState.archivedGroup(id);wgArchivedGroupId=next.groupId;wgArchivedTestId=next.testId;rerender();}
function wgSetArchivedTest(id){const next=globalThis.westgardUiState.archivedTest(id);wgArchivedTestId=next.testId;rerender();}
function wgViewModeTabs(archivedGroups){return globalThis.westgardModeTabs.view(wgViewMode,archivedGroups.length);}
const WG_TABLE_INITIAL_ROWS=120;
function wgRowsWindow(rows,key){return globalThis.westgardRowsWindow(rows,wgExpandedRows.has(key),WG_TABLE_INITIAL_ROWS);}
function wgToggleRows(key){wgExpandedRows=globalThis.westgardUiState.toggleOpen(wgExpandedRows,key);rerender();}
function wgRowsControl(view,key){return globalThis.westgardRowsControl(view,key,WG_TABLE_INITIAL_ROWS);}
...
function wgLotBlock(t,level,lotNo,mean,sd,pts,badge,titleMain,lotLabel,extraMeta=''){
  return globalThis.westgardLotBlockHtml({test:t,level,lotNo,mean,sd,points:pts,badge,title:titleMain,lotLabel,extraMeta});
}
...
function wgArchivedGroupMatches(g,q){return globalThis.westgardArchivedGroupMatches(g,q,searchText,id=>(state.qcLots||[]).find(x=>x.id===id));}
function pageWestgardArchived(archivedGroups){
  ...
  const archiveTestSelection=globalThis.westgardArchivedTestSelection(testEntries,q,wgArchivedTestId,{searchText,testDisplayName,instrumentName});
  ...
  wgArchivedTestId=archiveTestSelection.selected;
...
function pageWestgard(){
  ...
  const blocks=levelViews.map(v=>{const{l,pts,cfg,lotPicker}=v;
    ...
    const rowsHtml=globalThis.westgardPointRowsHtml(view.rows,t);
  ...
  const ruleToggles=globalThis.westgardRuleTogglesHtml(WG_RULE_REGISTRY,wgOn,canWrite());
  const exportActions=globalThis.westgardExportActionsHtml(wgChartMode);
  return headOnly('Phân tích Westgard','Đối chiếu luật theo mức QC, lô và lần chạy')+
   `<div class="panel"><h2 class="panel-title">Thiết lập phân tích</h2>${wgViewModeTabs(archivedGroups)}<div class="wg-test-picker${exportActions?' wg-test-picker-3':''}"><div><label>Tìm nhanh</label><input id="wgTestSearch" type="search" placeholder="Tên xét nghiệm, LOT hoặc máy..." value="${escAttr(wgTestQ)}" oninput="wgFilterTests(this.value)"></div><div><label>Chọn xét nghiệm <span id="wgTestCount" class="hint">(${matched.length}/${tests.length})</span></label><select id="wgTestSelect" aria-label="Chọn xét nghiệm" ${matched.length?'':'disabled'} onchange="if(this.value){selTest=this.value;rerender()}">${opts}</select></div>${exportActions}</div>
     ...
     ${globalThis.westgardRuleGuideHtml(WG_RULE_REGISTRY)}${wgChartModeTabs()}</div>${wgChartMode==='cusum'?pageWestgardCusum(t):multiChart+blocks}`;
}
function wgFilterTests(v){
  wgTestQ=globalThis.westgardUiState.query(v);
  scheduleSearchRender(wgFilterTests,()=>{
    const tests=operationalTests(),result=globalThis.westgardTestSearch.select(tests,wgTestQ,selTest),matches=result.matches;
    if(result.changed){selTest=result.selected;rerender();return;}
...
function wgFilterArchivedTests(v){
  wgArchivedTestQ=globalThis.westgardUiState.query(v);
