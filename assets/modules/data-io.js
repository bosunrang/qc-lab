...
function downloadJsonFile(name,value){
  let json;try{json=JSON.stringify(value,null,2);}catch(e){return false;}
...
function sigmaReportRows(onlyTestId='',mode='latest',period='',periodId=''){
  return sgTrackedTests().filter(t=>!onlyTestId||t.id===onlyTestId).flatMap(t=>{
    ...
    const name=(t.name&&String(t.name).trim())||'(chưa đặt tên)';
...
function reportXlsxDoc(tid,start,end){
  ...
  metaRow('Xét nghiệm',(t.name||'')+(t.unit?' ('+t.unit+')':''),'Máy',t.machine||'');
...
async function exportSigmaPeriodsXLSX(){
  ...
  await buildSigmaXlsx(rows,`BÁO CÁO TỔNG HỢP SIX SIGMA THEO KỲ - ${t.name}`,`Nguồn: QC Lab · Xuất ${formatDateTimeVN(new Date().toISOString())} · Kỳ xuất: ${sigmaExportPeriods(rows)} · Người xuất: ${userName()} · App ${app.version||'dev'} · Bộ luật: ${rules} · Truy xuất TEa: ${sigmaTeaTrace(rows)}`,`Bao_Cao_Six_Sigma_Theo_Ky_${safeName(t.name)}.xlsx`);
}
...
async function importData(e){
  ...
  const f=e.target.files[0];if(!f)return;if(f.size>10*1024*1024){await infoDialog('File backup vượt quá giới hạn 10 MB.');e.target.value='';return;}
  const oldActivity=[...(state.activity||[])],fr=new FileReader();
  fr.onload=async()=>{try{
    const incoming=JSON.parse(fr.result),errors=validateBackup(incoming);if(errors.length)throw new Error(errors.join('\n'));
    ...
    state=QCCore.sanitizeBackup(incoming,{owned:true});ensureShape({sanitized:true});
    const invariantErrors=QCCore.validateStateInvariants(state,{sanitized:true});
    if(invariantErrors.length){state=previousState;throw new Error('Backup sau chuẩn hóa không đạt kiểm tra dữ liệu:\n'+invariantErrors.join('\n'));}
    ...
    // Backup có thể đến từ một chuỗi audit độc lập; nhập vào dưới dạng legacy
    // để không ghép sai hash chain hiện tại của máy/phòng này.
...
/* ===== BACKUP REMINDER (theo từng thiết bị, không đồng bộ đám mây) =====
   Chấm cảnh báo cố định cạnh #cloudStatus trong <aside> (không bị render() xóa/
   dựng lại như #main), thay vì chèn banner đầu #main mỗi lượt render — banner cũ
   gây giật layout (đẩy nội dung xuống) mỗi lần chuyển trang. */
...
function backupOverdue(){
  if(typeof fb!=='undefined'&&fb&&fb.ready)return false;
  return lastBackupInfo().days>=BACKUP_REMIND_DAYS;
}
function updateBackupBanner(){
  ...
  var i=lastBackupInfo();
  dot.hidden=false;
  dot.className='backup-dot'+(i.never?' crit':'');
