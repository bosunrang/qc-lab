...
function exportActionsCSV(){const rows=[...exportMetaRows('Nhật ký khắc phục'),[],['Mã NCE','Ngày','Thời điểm ghi nhận','Xét nghiệm','Mức / lô','Luật','Loại sai số','Hành động','Điều tra & ảnh hưởng','Người phụ trách','Hạn hoàn thành','QC chạy lại','Trạng thái duyệt','Người duyệt','Thời điểm duyệt','Ý kiến duyệt','Trạng thái hồ sơ']];(state.actions||[]).forEach(a=>{const t=state.tests.find(x=>x.id===a.testId),wf=typeof actionWorkflowStatus==='function'?actionWorkflowStatus(a):{complete:false,label:'Chưa hoàn tất'},rr=typeof actionRerunStatus==='function'?actionRerunStatus(a):{label:''};rows.push([a.nceId||'',vnDate(a.date),a.createdAt?formatDateTimeVN(a.createdAt):'',t?testDisplayName(t):'',actionLevelShort(t,a.level,a.lot),a.rule||'',a.errorType||'',a.action||a.correction||'',typeof actionProtocolSummary==='function'?actionProtocolSummary(a):'',a.by||'',a.dueDate?vnDate(a.dueDate):'',rr.label||'',typeof actionApprovalLabel==='function'?actionApprovalLabel(a):(a.approvalStatus||'pending'),a.approvedBy||'',a.approvedAt?formatDateTimeVN(a.approvedAt):'',a.approvalNote||'',wf.label||'Chưa hoàn tất']);});downloadCSV('Nhat_ky_khac_phuc_QC.csv',rows);}
