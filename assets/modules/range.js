...
function rangeGateHtml(r,tid,level){
  ...
  return `<div class="alert warn" style="margin-top:10px"><b>Hồ sơ NCE ${esc(r.nce.nceId||'NCE')} đang ghi nhận vi phạm hệ thống (${esc(r.nce.rule||'')})</b><div>Xác nhận 2 điều kiện dưới đây trước khi áp dụng dải mới — tránh "đuổi theo mean" khi nguyên nhân dịch chuyển chưa được lý giải.</div></div>
    <label style="display:flex;gap:8px;align-items:flex-start;margin-top:10px"><input type="checkbox" id="rangeCauseConfirm" style="margin-top:3px" onchange="document.getElementById('rangeGateErr').style.display='none'"><span>Xác nhận nguyên nhân dịch chuyển đã được xác định và ghi nhận trong hồ sơ NCE ${esc(r.nce.nceId||'NCE')} (không phải lỗi chưa lý giải)</span></label>
    <div class="field-row" style="margin-top:8px"><div><label>Bias đo lại (%)</label><input id="rangeBiasInput" type="text" inputmode="decimal" oninput="rangeUpdateBiasHint('${tid}',${level})"></div><div><label>Ngưỡng cho phép (≤ TEa/4)</label><input id="rangeBiasThreshold" readonly value="${threshold!=null?fmt(threshold)+'%':'—'}"></div></div>
    <div id="rangeBiasHint" class="hint" style="margin-top:4px">${tea?'':'Chưa có TEa% cho xét nghiệm này — vào Cấu hình Sigma để bổ sung, hoặc vẫn có thể xác nhận thủ công nếu ngưỡng đã biết theo cách khác.'}</div>
    <div id="rangeGateErr" class="hint" style="color:var(--red);display:none;margin-top:4px">Cần xác nhận nguyên nhân dịch chuyển và nhập Bias trong ngưỡng cho phép trước khi áp dụng.</div>`;
}
...
async function confirmApplyNewRange(tid,level){
  ...
  l.meanSdHistory.push({id:uid(),qcLotId:l.qcLotId||'',lot:l.lot||'',mean:c.m,sd:c.sd,low:c.m-2*c.sd,high:c.m+2*c.sd,effectiveFrom:isoToday(),effectiveTo:l.exp||'',source:'lab',note:reason+gateNote});
