...
function pageSettings(){
  ...
  return headOnly('Cài đặt & Đồng bộ','Thông tin đơn vị, backup và kết nối Firebase')+
   ...
   <div class="panel"><h3>Quản trị dữ liệu</h3>
     <div class="admin-tools">
        ...
        <div class="admin-tool"><b>Kiểm tra backup / archive</b><span>Kiểm tra checksum, cấu trúc, số điểm và khoảng ngày. Không đụng tới dữ liệu QC, nhưng archive năm hợp lệ sẽ được ghi vào danh mục bên dưới và ghi nhật ký.</span>${btn('Chọn file để kiểm tra',"document.getElementById('verifyBackup').click()",'ghost')}<input id="verifyBackup" type="file" accept="application/json" style="display:none" onchange="verifyBackupFile(event)"></div>
  ...
  appId: "..."
   ...
   <div class="panel lis-gateway-panel"><h3>LIS Gateway (thử nghiệm)</h3>
     <div class="lis-gateway-body"><div class="lis-gateway-grid"><div><label for="lisGatewayUrl">Địa chỉ Gateway cục bộ</label><input id="lisGatewayUrl" value="${escAttr(liscfg.url)}" placeholder="http://127.0.0.1:8787"></div><label class="lis-gateway-toggle"><input id="lisGatewayEnabled" type="checkbox" ${liscfg.enabled?'checked':''}><span>Tự động gửi trạng thái QC sau khi dữ liệu thay đổi</span></label></div>
       ...
       <div class="hint">Chỉ gửi mã xét nghiệm, thiết bị, trạng thái Westgard và bằng chứng điểm QC. Không gửi thông tin bệnh nhân. Prototype chỉ cho phép localhost:8787.</div></div>
     <div class="settings-panel-actions">${btn('Lưu &amp; kiểm tra','lisGatewaySaveSettings()','teal')}${btn('Đồng bộ QC ngay','lisGatewaySyncNow()','ghost')}</div></div>
