...
function pageSigma(){
  ...
  const combinedExport=sgData(t.id).length?btn(icoDownload()+'Xuất tổng hợp các kỳ','exportSigmaPeriodsXLSX()','teal sg-combined-export','Xuất báo cáo Excel tổng hợp để so sánh Sigma giữa các kỳ')+btn(printIcon+'PDF tổng hợp các kỳ','printSigmaPeriods()','teal sg-combined-print','Tạo bản in PDF/HTML tổng hợp để so sánh Sigma giữa các kỳ'):'';
  ...
  const biasActions=canWrite()?levels.map(l=>btn(`<span class="sg-bias-action-icon" aria-hidden="true">🧮</span>Bias EQA% Mức ${l}`,latestEntry?`sgOpenBias('${latestEntry.id}',${l})`:'','ghost sm',latestEntry?`Tính Bias EQA/EQC Mức ${l} cho kỳ ${escAttr(latestEntry.period||'mới nhất')}`:'Hãy thêm kỳ trước khi tính Bias',{disabled:!latestEntry})).join(''):'';
  ...
  return headOnly('Six Sigma & Sai số','Đánh giá hiệu năng phương pháp theo TEa, CV IQC và Bias EQA/EQC')+
   ...
   <div class="panel sg-mu-panel"><div class="sg-data-head"><h3>Độ không đảm bảo đo (MU) — ISO 15189:2022</h3><div id="sgMUAction" class="sg-data-head-actions"></div></div><div id="sgMU"></div></div>
