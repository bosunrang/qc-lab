...
const sigmaUiState=SigmaUIState;
...
function sgReadiness(L){if(!sgIsAutoCV(L))return{status:'manual',label:'CV nhập tay — chưa xác nhận bằng nhóm dữ liệu IQC cùng lô/mức',classifiable:true,qcpEligible:false};const status=['insufficient','provisional','eligible','unstable'].includes(L.cohortStatus)?L.cohortStatus:(+L.n<20?'insufficient':+L.n<30?'provisional':'eligible');if(status==='insufficient')return{status,label:'Chưa đủ 20 điểm QC',classifiable:false,qcpEligible:false};if(status==='unstable')return{status,label:'Cohort không ổn định',classifiable:false,qcpEligible:false};if(status==='provisional')return{status,label:'Kết quả tạm thời (20–29 điểm)',classifiable:true,qcpEligible:false};return{status:'eligible',label:'Đủ điều kiện dữ liệu',classifiable:true,qcpEligible:true};}
...
function sgRenderAddTestModal(){
  ...
  const rows=matched.map(t=>{const tracked=!!t.sgTracked,meta=[t.machine,t.unit,t.section].filter(Boolean).map(esc).join(' · ')||'Chưa có thông tin máy/đơn vị';return `<button class="refrow sg-add-test-row${tracked?' is-tracked':''}" ${tracked?'disabled':''} onclick="sgTrackTest('${jsq(t.id)}')"><span><b>${esc(testDisplayName(t))}</b><span class="meta">${meta}</span></span><span class="tag ${tracked?'ok':'none'}">${tracked?'Đã thêm':'Thêm'}</span></button>`;}).join('');
  ...
  openModal(`<div class="modal"><div class="modal-h"><h3>Thêm xét nghiệm vào Six Sigma</h3><button class="modal-close" onclick="closeModal()">✕</button></div>
...
function pageSigma(){
  ...
  const testActions=`${role()==='admin'?'<button class="btn teal" onclick="sgOpenAddTest()">+ Thêm</button>':''}${canWrite()?`<button class="btn ghost btn-remove-tint" onclick="sgRemoveTracked('${t.id}')">${trashIcon}Xóa</button>`:''}`;
  ...
  const teaHint=teaSrc==='clia'?`Tiêu chí CLIA đang dùng: ${sgTeaCriterionText(t,'clia')}. TEa% được tính riêng tại Mean mục tiêu của từng mức QC.`:teaVal?`TEa đang dùng: ${fmt(teaVal,2)}% · nguồn ${sgTeaLabel(teaSrc)}.`:(teaSrc==='eflm'?'Chọn EFLM thì cần tra database EFLM và nhập TEa% cùng thông tin truy xuất.':'Chưa có TEa% cho nguồn đang chọn. Hãy chọn nguồn khác hoặc cập nhật danh mục tham chiếu.');
  ...
  const rows=data.map(e=>{const periodLabel=vnPeriod(e.period)||e.period||'',selected=e.id===selectedPeriodId;return `<tr data-sg-period-id="${escAttr(e.id)}" class="sg-period-row${selected?' sg-period-selected':''}" tabindex="0" aria-selected="${selected?'true':'false'}" aria-label="Chọn kỳ ${escAttr(periodLabel)} để xem tình trạng" onclick="sgSelectPeriod('${e.id}')" onkeydown="if((event.key==='Enter'||event.key===' ')&&event.target===event.currentTarget){event.preventDefault();sgSelectPeriod('${e.id}')}"><td class="sg-period-cell"><div class="sg-period-select-wrap">${sgPeriodSel(e,ro)}</div></td>${levels.map(l=>levelCells(e,l)).join('')}<td class="sg-row-action sg-action-col"><div class="sg-row-action-buttons">${canWrite()&&isOperational?`<button class="btn ghost sm sg-row-cv" onclick="sgPullCV('${e.id}')" title="Chọn CV IQC theo lô cho kỳ ${escAttr(periodLabel)}">CV lô</button>`:''}<button class="btn ghost sm sg-row-export" onclick="exportSigmaPeriodXLSX('${e.id}')" title="Xuất Excel riêng kỳ ${escAttr(periodLabel)}">${icoDownload()}Excel</button><button class="btn ghost sm sg-row-print" onclick="printSigmaPeriod('${e.id}')" title="Tạo bản in PDF/HTML riêng kỳ ${escAttr(periodLabel)}">${printIcon}In PDF</button>${canWrite()?`<button class="btn sm sg-row-delete" onclick="sgDelPeriod('${e.id}')" title="Xóa kỳ ${escAttr(periodLabel)}">Xóa</button>`:''}</div></td></tr>`;}).join('');
  ...
  const combinedExport=data.length?`<button class="btn ghost sm sg-combined-export" onclick="exportSigmaPeriodsXLSX()" title="Xuất báo cáo tổng hợp để so sánh Sigma giữa các kỳ">${icoDownload()}Xuất tổng hợp các kỳ</button>`:'';
  const addPeriodAction=canWrite()?'<button class="btn teal sm" onclick="sgAddPeriod()">+ Thêm kỳ</button>':'';
  const headerActions=biasActions+combinedExport+addPeriodAction;
...
function sgTrendSVG(t,valid,levels){
  ...
  const px=i=>valid.length<=1?L+(W-L-R)/2:L+i/(valid.length-1)*(W-L-R);const py=s=>T+(maxS-s)/maxS*(H-T-B);
...
function sgRenderBiasModal(){
  ...
  const rows=rounds.map((r,i)=>{const lab=parseFloat(r.lab),target=parseFloat(r.target),bias=(isFinite(lab)&&isFinite(target)&&target!==0)?(lab-target)/Math.abs(target)*100:null;
    return `<tr class="sg-eqa-row"><td class="sg-eqa-index">${i+1}</td><td><input type="number" step="any" data-f="lab" value="${escAttr(r.lab??'')}" placeholder="VD: 5.59" oninput="sgBiasUpdateSummary()"></td><td><input type="number" step="any" data-f="target" value="${escAttr(r.target??'')}" placeholder="VD: 5.55" oninput="sgBiasUpdateSummary()"></td><td class="sg-eqa-bias" data-bias>${bias==null?'—':fmt(bias,2)+'%'}</td><td><button class="sg-eqa-del" onclick="sgBiasDel(${i})" title="Xóa vòng">✕</button></td></tr>`;}).join('');
  ...
  openModal(`<div class="modal sg-eqa-modal"><div class="modal-h"><h3>Tính Bias% từ EQA/EQC — Mức ${c.level}</h3><button class="modal-close" onclick="closeModal()">✕</button></div>
    <div class="modal-b"><div class="sg-eqa-table-wrap"><table class="sg-eqa-table"><thead><tr><th>#</th><th>PXN đo — KQ của bạn</th><th>Target EQA — Giá trị đích</th><th>Bias%</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>
      <button class="btn ghost sm sg-eqa-add" onclick="sgBiasAdd()">+ Thêm vòng</button><div id="sgBiasSummary" class="sg-eqa-summary alert info"></div>
      <div class="sg-eqa-period-wrap"><div class="sg-eqa-period-head"><b>Áp dụng cho kỳ nào?</b><div><button class="btn ghost sm" onclick="sgBiasSelectPeriods(true)">Chọn tất cả</button><button class="btn ghost sm" onclick="sgBiasSelectPeriods(false)">Bỏ chọn</button></div></div><div class="sg-eqa-period-list">${periodRows}</div></div></div>
    <div class="modal-f"><button class="btn ghost" onclick="closeModal()">Hủy</button><button class="btn teal" onclick="sgBiasApply()">✓ Áp dụng Bias%</button></div></div>`);
  sgBiasUpdateSummary();
}
...
function sgCohortImportMessage(e,s){const notes=[];if(s.missingLotLevels)notes.push(s.missingLotN+' điểm IQC ('+s.missingLotLevels+' mức) chưa gắn mã lô QC nên không dùng được — hãy gắn mã lô cho điểm QC để lấy CV tự động');if(s.mixedTargets)notes.push(s.mixedTargets+' mức thay đổi Mean/SD mục tiêu nên cohort chưa ổn định');if(s.unstable)notes.push(s.unstable+' mức không được phân loại');if(s.cleared)notes.push(s.cleared+' CV tự động cũ đã được xóa');if(!s.imported)return'Kỳ '+e.period+' chưa có cohort cùng lô với ít nhất 2 phép đo IQC hợp lệ.'+(notes.length?' '+notes.join('. ')+'.':'');return'Đã lấy CV theo lô đến '+vnDate(sgCohortCutoff(e.period))+'.'+(s.insufficient?' Có '+s.insufficient+' mức dưới 20 điểm; Sigma chỉ hiển thị ước tính.':'')+(notes.length?' '+notes.join('. ')+'.':'');}
function sgRenderCohortModal(){const c=sgCohortCtx;if(!c)return;const sections=c.groups.map(g=>{if(!g.cohorts.length)return`<tr><td>Mức ${g.level}</td><td colspan="5" class="muted">${g.missingLotN?`Có ${g.missingLotN} điểm IQC chưa gắn mã lô QC — hãy gắn mã lô cho điểm QC để dùng làm CV.`:'Không có cohort có mã lô và dữ liệu trong kỳ đánh giá.'}</td></tr>`;const preferred=g.cohorts.find(x=>x.lot===g.configuredLot)||g.cohorts[g.cohorts.length-1];return g.cohorts.map((x,i)=>{const a=SigmaCohortService.assess(x),checked=x===preferred?'checked':'';return`<tr><td>${i?'':`Mức ${g.level}`}</td><td><label><input type="radio" name="sgCohort_${g.level}" value="${escAttr(x.lot)}" ${checked}> Lô ${esc(x.lot)}</label></td><td>${vnDate(x.start)}–${vnDate(x.end)}</td><td class="num">${x.n}</td><td class="num">${x.stats&&x.stats.cv>0?fmt(x.stats.cv,2)+'%':'—'}</td><td>${esc(sgCohortStatusText(a))}</td></tr>`;}).join('');}).join('');openModal(`<div class="modal"><div class="modal-h"><h3>Chọn cohort CV IQC — ${esc(c.t.name)}</h3><button class="modal-close" onclick="sgCohortClose()">✕</button></div><div class="modal-b"><div class="hint" style="margin-bottom:10px">Cohort được gom xuyên tháng nhưng luôn tách theo lô và mức QC. Nếu Mean/SD mục tiêu thay đổi, cohort sẽ bị đánh dấu không ổn định. Dữ liệu chụp đến ${vnDate(sgCohortCutoff(c.e.period))}.</div><table><thead><tr><th>Mức</th><th>Lô QC</th><th>Khoảng dữ liệu</th><th class="num">n</th><th class="num">CV</th><th>Trạng thái</th></tr></thead><tbody>${sections}</tbody></table></div><div class="modal-f"><button class="btn ghost" onclick="sgCohortClose()">Hủy</button><button class="btn teal" onclick="sgCohortApply()">✓ Dùng cohort đã chọn</button></div></div>`);}
