...
function pageSigma(){
  ...
  const combinedExport=sgData(t.id).length?`<button class="btn teal sg-combined-export" onclick="exportSigmaPeriodsXLSX()" title="Xuất báo cáo Excel tổng hợp để so sánh Sigma giữa các kỳ">${icoDownload()}Xuất tổng hợp các kỳ</button><button class="btn teal sg-combined-print" onclick="printSigmaPeriods()" title="Tạo bản in PDF/HTML tổng hợp để so sánh Sigma giữa các kỳ">${printIcon}PDF tổng hợp các kỳ</button>`:'';
...
function sgRenderBiasModal(){
  ...
  const rows=rounds.map((r,i)=>{const lab=parseFloat(r.lab),target=parseFloat(r.target),bias=(isFinite(lab)&&isFinite(target)&&target!==0)?(lab-target)/Math.abs(target)*100:null;
    return `<tr class="sg-eqa-row"><td class="sg-eqa-index">${i+1}</td><td><input type="number" step="any" data-f="lab" value="${escAttr(r.lab??'')}" placeholder="—" oninput="sgBiasUpdateSummary()"></td><td><input type="number" step="any" data-f="target" value="${escAttr(r.target??'')}" placeholder="—" oninput="sgBiasUpdateSummary()"></td><td class="sg-eqa-bias" data-bias>${bias==null?'—':fmt(bias,2)+'%'}</td><td><button class="btn danger sm sg-eqa-del" onclick="sgBiasDel(${i})" title="Xóa vòng">Xóa</button></td></tr>`;}).join('');
  ...
  openModal(`<div class="modal sg-eqa-modal"><div class="modal-h"><h3>Tính Bias% từ EQA/EQC — Mức ${c.level}</h3><button class="modal-close" onclick="closeModal()">✕</button></div>
    <div class="modal-b"><div class="sg-eqa-table-wrap"><table class="sg-eqa-table"><thead><tr><th>#</th><th>KQ PXN</th><th>Target EQA</th><th>Bias%</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>
