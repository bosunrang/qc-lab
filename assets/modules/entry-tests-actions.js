...
async function openLotTransitionV2(id=''){
  ...
  openModal(`<div class="modal rcfg-modal lot-trans-modal"><div class="modal-h"><div><h3>${id?'Sửa hồ sơ chuyển lô':'Tạo hồ sơ chuyển lô'}</h3></div><button class="modal-close" onclick="closeModal()">✕</button></div><div class="modal-b">
    <div class="lot-trans-row2"><div><label>Panel QC áp dụng</label><select id="cfgTransPanel" onchange="refreshLotTransitionTargets()">${panels}</select></div><div><label>Ngày bắt đầu (dd/mm/yyyy)</label>${dateBox('cfgTransStart',tr.startDate||'')}</div></div>
    <div class="lot-trans-row3"><div><label>Lô cũ</label><select id="cfgTransFrom" onchange="refreshLotTransitionTargets()">${lotOptions(tr.fromLotId)}</select></div><div><label>Lô mới</label><select id="cfgTransTo" onchange="refreshLotTransitionTargets()">${lotOptions(tr.toLotId)}</select></div><div><label>Trạng thái</label><select id="cfgTransStatus"><option value="planned" ${tr.status==='planned'?'selected':''}>Dự kiến</option><option value="active" ${tr.status==='active'||tr.status==='completed'?'selected':''}>Đang chạy song song</option><option value="accepted" ${tr.status==='accepted'?'selected':''}>Chấp nhận lô mới</option><option value="rejected" ${tr.status==='rejected'?'selected':''}>Không chấp nhận</option></select></div></div>
...
function openConfigAssay(id=''){
  ...
  const cusum=testCusumConfig(t),decimalValue=t.decimalPlaces!==null&&t.decimalPlaces!==''&&Number.isInteger(Number(t.decimalPlaces))?String(t.decimalPlaces):String(QC_DECIMALS_DEFAULT);
