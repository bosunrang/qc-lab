...
function openQcHistoryDetail(tid,level,lotNo=''){
  ...
  const histRows=hist.length?hist.map(h=>`<tr><td><b>${esc(h.lot||lotNo||'—')}</b></td><td class="num">${fmtTestValue(t,h.mean)}</td><td class="num">${h.low!=null?fmtTestValue(t,h.low):'—'}</td><td class="num">${h.high!=null?fmtTestValue(t,h.high):'—'}</td><td class="num">${fmtTestValue(t,h.sd)}</td><td>${h.effectiveFrom?vnDate(h.effectiveFrom):'Không giới hạn'} → ${h.effectiveTo?vnDate(h.effectiveTo):'Không giới hạn'}</td><td>${h.source==='lab'?'PXN':'NSX'}</td></tr>`).join(''):'';
...
function openConfigAssay(id=''){
  ...
  const cusum=testCusumConfig(t),decimalValue=t.decimalPlaces!==null&&t.decimalPlaces!==''&&Number.isInteger(Number(t.decimalPlaces))?String(t.decimalPlaces):'';
...
async function saveConfigAssay(id){
  ...
  const decimalRaw=document.getElementById('cfgAssayDecimals').value,decimalPlaces=decimalRaw===''?null:Number(decimalRaw);if(decimalPlaces!==null&&(!Number.isInteger(decimalPlaces)||decimalPlaces<0||decimalPlaces>6)){await infoDialog('Số chữ số thập phân tùy chỉnh phải từ 0 đến 6. Để trống là dùng mặc định 1 chữ số.');return;}
