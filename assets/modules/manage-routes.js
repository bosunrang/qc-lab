...
function manageMatch(values){return globalThis.manageSearchMatchPresentation?globalThis.manageSearchMatchPresentation(values,manageQ,searchText):(()=>{const q=searchText(manageQ);return !q||values.some(v=>searchText(v).includes(q));})();}
function manageSearchPlaceholder(){
  if(globalThis.manageSearchPlaceholderPresentation)return globalThis.manageSearchPlaceholderPresentation(manageTab);
  const map={
...
function sameText(a,b){return globalThis.sameNormalizedTextPresentation?globalThis.sameNormalizedTextPresentation(a,b):searchText(a)===searchText(b);}
function sameIdSet(a,b){return globalThis.sameIdSetPresentation?globalThis.sameIdSetPresentation(a,b):(()=>{const A=[...new Set(a||[])].sort(),B=[...new Set(b||[])].sort();return A.length===B.length&&A.every((x,i)=>x===B[i]);})();}
function groupsOfLot(id){return globalThis.groupsOfLotPresentation?globalThis.groupsOfLotPresentation(state.lotGroups,id):state.lotGroups.filter(g=>(g.lotIds||[]).includes(id));}
function lotGroupLabels(id){return globalThis.manageLotGroupLabelsPresentation?globalThis.manageLotGroupLabelsPresentation(state.lotGroups,id):(()=>{const groups=groupsOfLot(id);return groups.length?groups.map(g=>g.name).join(', '):'Chưa thuộc nhóm';})();}
function instrumentName(id,fallback=''){return globalThis.manageInstrumentNamePresentation?globalThis.manageInstrumentNamePresentation(state.instruments,id,fallback):(()=>{const x=state.instruments.find(i=>i.id===id);return x?x.name:(fallback||'Chưa gán máy');})();}
function panelName(id){return globalThis.managePanelNamePresentation?globalThis.managePanelNamePresentation(state.qcPanels,id):(()=>{const x=state.qcPanels.find(p=>p.id===id);return x?x.name:'Chưa chọn Panel QC';})();}
function lotLabel(id){return globalThis.manageLotLabelPresentation?globalThis.manageLotLabelPresentation(state.qcLots,id):(()=>{const l=state.qcLots.find(x=>x.id===id);return l?`${l.lotNo} · Mức ${l.level}`:'Chưa chọn lô';})();}
function lotTransitionToNo(lotId){return globalThis.lotTransitionTargetNumberPresentation?globalThis.lotTransitionTargetNumberPresentation(state.lotTransitions||[],state.qcLots,lotId,transitionSwitchesLot):(()=>{const tr=(state.lotTransitions||[]).find(x=>x.fromLotId===lotId&&transitionSwitchesLot(x));if(!tr)return'';const to=state.qcLots.find(l=>l.id===tr.toLotId);return to?to.lotNo:'';})();}
function lotStatus(l){if(globalThis.manageLotStatusPresentation)return globalThis.manageLotStatusPresentation(l,lotTransitionToNo(l.id));if(l&&l.depleted){const to=lotTransitionToNo(l.id);return{text:to?`Đã chuyển tiếp qua lô ${to}`:'Đã chuyển tiếp',cls:'rej'};}const d=daysToExp(l.exp);return d==null?{text:'Chưa có HSD',cls:'none'}:d<0?{text:'Hết hạn',cls:'rej'}:d<=30?{text:`Còn ${d} ngày`,cls:'warn'}:{text:'Đang hoạt động',cls:'ok'};}
function transitionStatusLabelV2(s){return globalThis.manageTransitionStatusPresentation?globalThis.manageTransitionStatusPresentation(s):s==='active'?{text:'Đang chạy song song',cls:'warn'}:s==='accepted'?{text:'Chấp nhận lô mới',cls:'ok'}:s==='rejected'?{text:'Không chấp nhận',cls:'rej'}:{text:'Dự kiến',cls:'none'};}
function manageShell(body){
  ...
  if(globalThis.manageShellPresentation){const counts={lots:state.qcLots.length+' / '+state.lotGroups.length,panels:state.qcPanels.length,targets:state.tests.reduce((n,t)=>n+t.levels.filter(l=>l.qcLotId).length,0),history:histCount,transitions:state.lotTransitions.length,assays:state.tests.length,instruments:state.instruments.length,tearefs:effectiveTeaRefs().length};return globalThis.manageShellPresentation(items.map(x=>({id:x[0],label:x[1],count:counts[x[0]]||''})),manageTab,body);}
...
function manageToolbar(title,sub,action,label){
  ...
  if(globalThis.manageToolbarPresentation)return globalThis.manageToolbarPresentation({title,subtitle:sub,placeholder:ph,query:manageQ,action,actionLabel:label});
...
function manageLots(){
  const rows=state.qcLots.filter(l=>manageMatch([l.lotNo,l.description,l.supplier,l.program,lotGroupLabels(l.id),l.level,l.exp])).map(l=>{const used=state.tests.reduce((n,t)=>n+t.levels.filter(x=>x.qcLotId===l.id).length,0),s=lotStatus(l),model={id:l.id,lotNo:l.lotNo,description:l.description,program:l.program,level:l.level,expiry:l.exp?vnDate(l.exp):'',status:s,used};return globalThis.manageLotRowPresentation?globalThis.manageLotRowPresentation(model):`<tr><td><b>${esc(l.lotNo)}</b>${(l.description||l.program)?`<div class="hint">${esc(l.description||l.program)}</div>`:''}</td><td><span class="pill">M${l.level}</span></td><td>${l.exp?vnDate(l.exp):'—'}</td><td><span class="tag ${s.cls}">${s.text}</span></td><td class="num">${used}</td><td><div class="lot-row-actions">${btn('Sửa',`openConfigLot('${l.id}')`,'ghost sm')}${btn('Xóa',`deleteConfigLot('${l.id}')`,'danger sm')}</div></td></tr>`;}).join('');
  const groupRows=state.lotGroups.filter(g=>manageMatch([g.name,g.note,...(g.lotIds||[]).map(id=>(state.qcLots.find(l=>l.id===id)||{}).lotNo)])).map(g=>{
    ...
    const lotsHtml=lots.map(l=>`<span class="pill">${esc(l.lotNo)} · M${l.level}</span>`).join(''),actionsHtml=btn('Sửa nhóm',`openConfigGroup('${g.id}')`,'ghost sm')+btn('Mean/SD',`openTargetMatrix('','${g.id}')`,'ghost sm')+toggleBtn+btn('Xóa',`deleteConfigGroup('${g.id}')`,'danger sm'),model={archived,name:g.name,note:g.note,status:statusTag,lotsHtml,actionsHtml};return globalThis.manageLotGroupCardPresentation?globalThis.manageLotGroupCardPresentation(model):`<div class="lot-group-card${archived?' lot-opt-depleted':''}"><div class="lot-group-card-h"><div><b>${esc(g.name)}</b><small>${esc(g.note||'Nhóm lô để gán Mean/SD theo Panel')}</small></div><span class="tag ${statusTag.cls}">${statusTag.text}</span></div><div class="lot-group-chipline">${lotsHtml||'<span class="hint">Chưa chọn lô</span>'}</div><div class="lot-group-actions">${actionsHtml}</div></div>`;}).join('');
...
function manageInstruments(){
  const rows=state.instruments.filter(i=>manageMatch([i.name,i.manufacturer,i.model,i.serial,i.section])).map(i=>{const n=state.tests.filter(t=>t.instrumentId===i.id).length;const model={id:i.id,name:i.name,section:i.section,manufacturer:i.manufacturer,serial:i.serial,assayCount:n,active:!!i.active};return globalThis.manageInstrumentRowPresentation?globalThis.manageInstrumentRowPresentation(model):`<tr><td><b>${esc(i.name)}</b><div class="hint">${esc(i.section||'Chưa phân khoa')}</div></td><td>${esc(i.manufacturer||'—')}</td><td>${esc(i.serial||'—')}</td><td class="num">${n}</td><td><span class="tag ${i.active?'ok':'none'}">${i.active?'Đang hoạt động':'Ngừng hoạt động'}</span></td><td><div class="manage-actions">${btn('Sửa',`openConfigInstrument('${i.id}')`,'ghost sm')}${btn('Xóa',`deleteConfigInstrument('${i.id}')`,'danger sm')}</div></td></tr>`;}).join('');
...
function managePanels(){
  const rows=state.qcPanels.filter(p=>manageMatch([p.name,p.note,instrumentName(p.instrumentId),...(p.testIds||[]).map(id=>(state.tests.find(t=>t.id===id)||{}).name)])).map(p=>{const tests=(p.testIds||[]).map(id=>state.tests.find(t=>t.id===id)).filter(Boolean),testsHtml=tests.map(t=>`<span class="pill">${esc(testDisplayName(t))}</span>`).join(''),model={id:p.id,name:p.name,instrument:instrumentName(p.instrumentId),testsHtml,testCount:tests.length,active:p.active!==false};return globalThis.managePanelRowPresentation?globalThis.managePanelRowPresentation(model):`<tr><td><b>${esc(p.name)}</b></td><td>${esc(instrumentName(p.instrumentId))}</td><td>${testsHtml||'—'}</td><td class="num">${tests.length}</td><td><span class="tag ${p.active!==false?'ok':'none'}">${p.active!==false?'Đang dùng':'Tạm ngưng'}</span></td><td><div class="manage-actions">${btn('Sửa',`openConfigPanel('${p.id}')`,'ghost sm')}${btn('Xóa',`deleteConfigPanel('${p.id}')`,'danger sm')}</div></td></tr>`;}).join('');
...
function targetPanelTests(){return globalThis.targetPanelTestsPresentation?globalThis.targetPanelTestsPresentation(state.qcPanels,state.tests,manageTargetPanel):(()=>{const p=state.qcPanels.find(x=>x.id===manageTargetPanel);return(p&&p.testIds||[]).map(id=>state.tests.find(t=>t.id===id)).filter(Boolean);})();}
function targetPanelLabel(){return globalThis.targetPanelLabelPresentation?globalThis.targetPanelLabelPresentation(state.qcPanels,manageTargetPanel):(()=>{const p=state.qcPanels.find(x=>x.id===manageTargetPanel);return p?p.name:'Panel QC';})();}
function targetGroupLots(group){return globalThis.targetGroupLotsPresentation?globalThis.targetGroupLotsPresentation(state.qcLots,group):(group&&group.lotIds||[]).map(id=>state.qcLots.find(l=>l.id===id)).filter(Boolean).sort((a,b)=>(+a.level)-(+b.level)||String(a.lotNo||'').localeCompare(String(b.lotNo||''),'vi',{numeric:true}));}
function targetGroupLabel(group){return globalThis.targetGroupLabelPresentation?globalThis.targetGroupLabelPresentation(group):group?group.name:'Chưa chọn nhóm lô';}
function groupStatusSuffix(g){return globalThis.targetGroupStatusSuffixPresentation?globalThis.targetGroupStatusSuffixPresentation(g):g&&g.status==='stopped'?' · Đã dừng':g&&g.status==='planned'?' · Dự kiến':'';}
function targetGroupOptions(){
  const groups=state.lotGroups.filter(g=>g.active!==false);
  return globalThis.targetGroupOptionsPresentation?globalThis.targetGroupOptionsPresentation(groups,manageTargetGroup,targetGroupLots,targetGroupLabel,groupStatusSuffix,esc):(()=>{const available=groups.filter(g=>targetGroupLots(g).length);return available.length?available.map(g=>`<option value="${g.id}" ${g.id===manageTargetGroup?'selected':''}>${esc(targetGroupLabel(g)+groupStatusSuffix(g))}</option>`).join(''):'<option value="">Không tìm thấy nhóm lô QC phù hợp</option>';})();
}
function ensureTargetSelection(){
  if(globalThis.targetSelectionPresentation){const picked=globalThis.targetSelectionPresentation(state.qcPanels,state.lotGroups,manageTargetPanel,manageTargetGroup,targetGroupLots);manageTargetPanel=picked.panelId;manageTargetGroup=picked.groupId;return;}
...
function manageTargets(){
  ...
  const prerequisite=globalThis.targetPrerequisitePresentation?globalThis.targetPrerequisitePresentation({tests:state.tests.length,panels:state.qcPanels.length,lots:state.qcLots.length,groups:state.lotGroups.length}):(!state.tests.length?'tests':!state.qcPanels.length?'panels':!state.qcLots.length?'lots':!state.lotGroups.length?'groups':null);
  ...
  const group=state.lotGroups.find(x=>x.id===manageTargetGroup),groupLots=targetGroupLots(group),targetLevelPick=globalThis.targetLevelSelectionPresentation?globalThis.targetLevelSelectionPresentation(groupLots,manageTargetLevel):(()=>{const levels=[...new Set(groupLots.map(l=>+l.level).filter(Number.isFinite))].sort((a,b)=>a-b);return{levels,level:levels.map(String).includes(String(manageTargetLevel))?String(manageTargetLevel):levels[0]!=null?String(levels[0]):''};})(),targetLevels=targetLevelPick.levels;
  manageTargetLevel=targetLevelPick.level;
  const selectedLevel=Number(manageTargetLevel),levelLotPick=globalThis.targetLevelLotsPresentation?globalThis.targetLevelLotsPresentation(groupLots,selectedLevel):(()=>{const levelLots=groupLots.filter(l=>+l.level===selectedLevel);return{levelLots,depletedLots:levelLots.filter(l=>l.depleted)};})(),levelLots=levelLotPick.levelLots,levelDepletedLots=levelLotPick.depletedLots,q=searchText(manageQ),allTests=targetPanelTests(),tests=allTests.filter(t=>!q||(globalThis.targetSearchValuesPresentation?globalThis.targetSearchValuesPresentation(t,group&&group.name,levelLots,testDisplayName,instrumentName):[t.name,testDisplayName(t),t.unit,t.method,t.reagent,t.section,instrumentName(t.instrumentId,t.machine),group&&group.name,...levelLots.map(l=>l.lotNo)]).some(v=>searchText(v).includes(q)));
  ...
  const rowItems=globalThis.targetMatrixItemsPresentation?globalThis.targetMatrixItemsPresentation(tests,levelLots,targetConfigAssigned,plannedTargetFor,lotTargetSnapshot):tests.flatMap(t=>levelLots.map(lot=>({t,lot}))).filter(({t,lot})=>!lot.depleted||(t.levels||[]).some(l=>l.qcLotId===lot.id)).map(({t,lot})=>{
  ...
  const targetStats=globalThis.targetMatrixStatsPresentation?globalThis.targetMatrixStatsPresentation(rowItems):rowItems.reduce((a,{linked,assigned,cfg})=>{if(linked)a.linked++;else if(assigned)a.other++;else a.empty++;if(!cfg||!Number.isFinite(+cfg.mean)||((!Number.isFinite(+cfg.sd)||+cfg.sd<=0)&&(!Number.isFinite(+cfg.low)||!Number.isFinite(+cfg.high)||+cfg.high<=+cfg.low)))a.missing++;return a;},{linked:0,other:0,empty:0,missing:0});
  const rows=rowItems.map(({t,lot,linked,same,assigned,planned,cfg})=>{const draft=targetRangeDraft(cfg||{}),rowState=globalThis.targetRowStatePresentation?globalThis.targetRowStatePresentation(linked,assigned,planned,lot.depleted):(()=>{const locked=!!lot.depleted,checked=locked?false:(!!linked||!assigned);return{locked,checked,disabled:locked||!checked,status:locked?'retired':linked?'linked':planned?'planned':assigned?'other':'empty'};})(),locked=rowState.locked,retiredTo=locked?lotTransitionToNo(lot.id):'',checked=rowState.checked,disabled=rowState.disabled;if(globalThis.targetMatrixRowPresentation)return globalThis.targetMatrixRowPresentation({testId:t.id,lotId:lot.id,locked,checked,disabled,name:testDisplayName(t),unit:t.unit,mean:targetNumberText(draft.mean,t),low:targetNumberText(draft.low,t),high:targetNumberText(draft.high,t),sd:targetNumberText(draft.sd,t,'stat'),status:rowState.status,retiredTo,otherLot:same&&same.lot},esc,escAttr);return `<div class="target-row${locked?' target-row-locked':''}" data-test="${t.id}" data-lot="${lot.id}"${locked?' data-locked="1"':''}>
    ...
    <span>${rowState.status==='retired'?`<b class="tag rej">${retiredTo?'Đã chuyển tiếp qua lô '+esc(retiredTo):'Đã chuyển tiếp'}</b>`:rowState.status==='linked'?'<b class="tag ok">Đã gán</b>':rowState.status==='planned'?'<b class="tag warn">Dự kiến</b>':rowState.status==='other'?`<b class="tag warn">Đang dùng ${esc(same.lot||'lô khác')}</b>`:'<b class="tag none">Chưa gán</b>'}</span>
  </div>`;}).join('');
  const targetLevelTabs=globalThis.targetLevelTabsPresentation?globalThis.targetLevelTabsPresentation(targetLevels,manageTargetLevel):targetLevels.map(level=>`<button class="${String(level)===String(manageTargetLevel)?'on':''}" onclick="setTargetLevel(${level})">Mức ${level}</button>`).join(''),targetLevelToolbar=globalThis.targetLevelToolbarPresentation?globalThis.targetLevelToolbarPresentation(manageTargetLevel,levelLots.map(l=>l.lotNo),targetLevelTabs,esc):`<div class="target-level-toolbar"><div><b>Mức ${esc(manageTargetLevel)}</b><span class="target-level-lot">${levelLots.map(l=>esc(l.lotNo)).join(' / ')}</span></div><div class="dayseg">${targetLevelTabs}</div></div>`;
  ...
  `<div class="panel target-matrix-panel">
    ${globalThis.targetSelectorPresentation?globalThis.targetSelectorPresentation(targetPanelOptions(),targetGroupOptions()):`<div class="target-selector">
    ...
    ${rowItems.length?(globalThis.targetSummaryPresentation?globalThis.targetSummaryPresentation(targetStats):`<div class="target-summary"><span class="ok"><b>${targetStats.linked}</b> đã gán mức này</span><span class="${targetStats.other?'warn':'none'}"><b>${targetStats.other}</b> đang dùng lô khác</span><span class="${targetStats.empty?'warn':'none'}"><b>${targetStats.empty}</b> chưa gán lô</span><span class="${targetStats.missing?'warn':'ok'}"><b>${targetStats.missing}</b> thiếu Mean/SD</span></div>`):''}
    ${rowItems.length?targetLevelToolbar+`
    ${globalThis.targetMatrixTablePresentation?globalThis.targetMatrixTablePresentation(rows):`<div class="target-table"><div class="target-head"><span>Dùng</span><span>Xét nghiệm</span><span>Trung bình mục tiêu</span><span>Giới hạn dưới</span><span>Giới hạn trên</span><span>Độ lệch chuẩn</span><span>Trạng thái</span></div>${rows}</div>`}
    ${globalThis.targetMatrixActionsPresentation?globalThis.targetMatrixActionsPresentation(btn('Bỏ chọn tất cả','targetCheckAll(false)','ghost'),btn('Chọn tất cả','targetCheckAll(true)','ghost'),btn('Lưu Mean/SD mức này','saveTargetMatrix()','teal')):`<div class="modal-f target-actions">${btn('Bỏ chọn tất cả','targetCheckAll(false)','ghost')}${btn('Chọn tất cả','targetCheckAll(true)','ghost')}${btn('Lưu Mean/SD mức này','saveTargetMatrix()','teal')}</div>`}`:(()=>{const empty=globalThis.targetEmptyStatePresentation?globalThis.targetEmptyStatePresentation(allTests.length,levelLots.map(l=>l.lotNo),levelDepletedLots.map(l=>l.lotNo),manageTargetLevel):(allTests.length?(levelDepletedLots.length&&levelDepletedLots.length===levelLots.length?{title:`Lô mức ${manageTargetLevel} đã hết QC`,description:`Lô ${levelDepletedLots.map(l=>l.lotNo).join(', ')} (Mức ${manageTargetLevel}) trong nhóm này đã hết QC nên không nhập Mean/SD được. Hãy chọn nhóm lô khác ở ô “Nhóm lô QC” phía trên, hoặc tạo lô mới rồi lập hồ sơ chuyển tiếp lô.`}:{title:'Không tìm thấy xét nghiệm',description:'Thử tìm theo tên xét nghiệm, máy, khoa, đơn vị hoặc lô QC.'}):{title:'Panel chưa có xét nghiệm',description:'Sửa Panel QC và chọn các xét nghiệm thành viên trước.'});return emptyState(empty.title,empty.description);})()}</div>`;
}
function manageAssays(){
  ...
  const rows=matched.map((t,idx)=>{const model={index:idx+1,id:t.id,name:testDisplayName(t),method:t.method,unit:t.unit,instrument:instrumentName(t.instrumentId,t.machine),section:t.section,reagent:t.reagent,tea:t.tea,closed:!!t.closed};return globalThis.manageAssayRowPresentation?globalThis.manageAssayRowPresentation(model):`<tr><td class="num">${idx+1}</td><td><b>${esc(testDisplayName(t))}</b><div class="hint">${esc(t.method||'Chưa nhập phương pháp')} · ${esc(t.unit||'Chưa có đơn vị')}</div></td><td>${esc(model.instrument)}<div class="hint">${esc(t.section||'Chưa gán khoa/khu vực')}</div></td><td>${esc(t.reagent||'—')}</td><td>${t.tea?esc(t.tea)+'%':'—'}</td><td><span class="tag ${t.closed?'none':'ok'}">${t.closed?'Ngưng dùng':'Đang dùng'}</span></td><td><div class="manage-actions">${btn('Sửa',`openConfigAssay('${t.id}')`,'ghost sm')}${btn('Xóa',`delTest('${t.id}')`,'danger sm')}</div></td></tr>`;}).join('');
...
function manageHistory(){
  ...
  const historyPick=globalThis.historyAssaySelectionPresentation?globalThis.historyAssaySelectionPresentation(matches,manageHistoryTest):(()=>{const assay=matches.find(t=>t.id===manageHistoryTest)||matches[0];return{selectedId:assay&&assay.id||'',assay};})();
  manageHistoryTest=historyPick.selectedId;
  const t=historyPick.assay;
  const opts=globalThis.historyAssayOptionsPresentation?globalThis.historyAssayOptionsPresentation(matches,t.id,testDisplayName,esc):matches.map(x=>`<option value="${x.id}" ${x.id===t.id?'selected':''}>${esc(testDisplayName(x))}</option>`).join('');
  ...
  const visibleRows=globalThis.historyVisibleRowsPresentation?globalThis.historyVisibleRowsPresentation(rows,t.name,q,searchText):rows.filter(r=>!q||[t.name,r.l.level,`M${r.l.level}`,`Mức ${r.l.level}`,r.lotNo].some(v=>searchText(v).includes(q)));
  const html=(globalThis.historyRowSortPresentation?globalThis.historyRowSortPresentation(visibleRows):visibleRows.sort((a,b)=>(+a.l.level)-(+b.l.level)||(a.lotNo||'').localeCompare(b.lotNo||'','vi')||String(a.h.effectiveFrom||'').localeCompare(String(b.h.effectiveFrom||'')))).map(r=>{
    const period=globalThis.historyPeriodLabelPresentation?globalThis.historyPeriodLabelPresentation(r.h.effectiveFrom,r.h.effectiveTo,vnDate):`${r.h.effectiveFrom?vnDate(r.h.effectiveFrom):'Không giới hạn'} → ${r.h.effectiveTo?vnDate(r.h.effectiveTo):'Không giới hạn'}`;
    const model={testId:r.t.id,level:r.l.level,lot:r.lotNo||'',group:r.group,mean:fmtTestValue(r.t,r.h.mean),low:r.h.low!=null?fmtTestValue(r.t,r.h.low):'—',high:r.h.high!=null?fmtTestValue(r.t,r.h.high):'—',sd:fmtTestValue(r.t,r.h.sd),period,source:r.h.source,pointCount:r.pts.length};return globalThis.manageHistoryRowPresentation?globalThis.manageHistoryRowPresentation(model):`<tr><td><span class="pill">M${r.l.level}</span></td><td><b>${esc(r.lotNo||'—')}</b><div class="hint">${esc(r.group)}</div></td><td class="num">${model.mean}</td><td class="num">${model.low}</td><td class="num">${model.high}</td><td class="num">${model.sd}</td><td>${period}</td><td><span class="tag ${r.h.source==='lab'?'warn':'ok'}">${r.h.source==='lab'?'PXN':'NSX'}</span></td><td class="num">${r.pts.length}</td><td>${btn('Chi tiết',`openQcHistoryDetail('${r.t.id}',${r.l.level},'${jsq(r.lotNo||'')}')`,'ghost sm')}</td></tr>`;
  }).join('');
  const historyTotals=globalThis.historySummaryPresentation?globalThis.historySummaryPresentation(visibleRows):{rowCount:visibleRows.length,pointCount:visibleRows.reduce((n,r)=>n+r.pts.length,0)};
...
function teaLabBasisLabel(src){return globalThis.teaLabBasisLabelPresentation?globalThis.teaLabBasisLabelPresentation(TEA_LAB_BASIS_SOURCES,src):(TEA_LAB_BASIS_SOURCES.find(x=>x[0]===src)||[])[1]||'';}
...
function teaRefNumOrNull(v){if(globalThis.TeaReferenceService)return globalThis.TeaReferenceService.numberOrNull(v);return globalThis.teaPositiveNumberPresentation?globalThis.teaPositiveNumberPresentation(v):(()=>{const n=Number(v);return String(v==null?'':v).trim()!==''&&Number.isFinite(n)&&n>0?n:null;})();}
function teaRefExternalChanged(row,refKey){if(globalThis.TeaReferenceService)return globalThis.TeaReferenceService.externalChanged(row,refKey);const base=REFTESTS.find(r=>teaAnalyteMeta(r[0]).analyteId===refKey);return globalThis.teaReferenceExternalChangedPresentation?globalThis.teaReferenceExternalChangedPresentation(row,base):!!(row&&base&&(row.unit!==base[1]||row.clia!==base[2]||row.ricos!==base[3]||row.section!==base[4]||['cliaRule','cliaAbsolute','cliaAbsoluteUnit'].some(k=>row[k]!=null&&row[k]!=='')));}
...
function teaSourceRegistryHtml(){const items=globalThis.teaSourceRegistryItemsPresentation?globalThis.teaSourceRegistryItemsPresentation(TEA_SOURCE_REGISTRY,vnDate):['clia','ricos','eflm'].map(src=>{const m=TEA_SOURCE_REGISTRY[src],statusLabel=m.status==='retired'?'Nguồn cũ':m.status==='dynamic'?'Cập nhật liên tục':'Hiện hành',tagClass=m.status==='retired'?'warn':m.status==='dynamic'?'ok':'none';return{status:m.status,label:m.label,statusLabel,tagClass,version:m.version,effectiveDate:m.effectiveDate?vnDate(m.effectiveDate):'',reviewedDate:vnDate(m.reviewedDate),url:m.url};});if(globalThis.teaSourceRegistryPresentation)return globalThis.teaSourceRegistryPresentation(items);return`<div class="tea-source-registry">${items.map(m=>`<div class="tea-source-card ${m.status}"><div><b>${esc(m.label)}</b><span class="tag ${m.tagClass}">${esc(m.statusLabel)}</span></div><p>${esc(m.version)}${m.effectiveDate?' · hiệu lực '+m.effectiveDate:''} · rà soát ${m.reviewedDate}</p><a href="${escAttr(m.url)}" target="_blank" rel="noopener">Mở nguồn chính thức</a></div>`).join('')}</div>`;}
...
function manageTeaRefs(){
  ...
  const rows=effectiveTeaRefs()
    .map(([name,unit,clia,ricos,section,,analyteId,lab])=>{const isDef=teaRefIsDefault(analyteId),record=overMap.get(analyteId),naming=teaAnalyteMeta(name,record),externalChanged=teaRefExternalChanged(record,analyteId),kind=globalThis.teaReferenceKindPresentation?globalThis.teaReferenceKindPresentation(isDef,externalChanged,!!(record&&record.lab!=null)):!isDef?'custom':externalChanged?'override':record&&record.lab!=null?'lab':'default';return{name,unit,clia,ricos,lab,section,analyteId,record,...naming,kind};})
    .filter(r=>manageMatch([r.name,r.displayName,r.standardName,r.abbreviation,...r.aliases,r.matrix,r.unit,r.section]))
    .sort(globalThis.teaReferenceSortPresentation?globalThis.teaReferenceSortPresentation:(a,b)=>String(a.section||'').localeCompare(String(b.section||''),'vi')||String(a.displayName||'').localeCompare(String(b.displayName||''),'vi'));
  const kindTag={default:'<span class="tag none">Mặc định</span>',override:'<span class="tag warn">Đã sửa</span>',lab:'<span class="tag ok">TEa PXN</span>',custom:'<span class="tag ok">Tự thêm</span>'},teaStatus=kind=>globalThis.teaReferenceStatusPresentation?globalThis.teaReferenceStatusPresentation(kind):kindTag[kind]||kindTag.default;
  const body=rows.map(r=>{
    const rowActions=globalThis.teaReferenceRowActionsPresentation?globalThis.teaReferenceRowActionsPresentation(r.kind,canManage,r.lab!=null):{action:!canManage?'none':r.kind==='override'?'restore':r.kind==='custom'?'remove':'none',labProfile:canManage?(r.lab!=null?'view':'add'):'none'},act=rowActions.action==='restore'?btn('Khôi phục',`teaRefRemove('${escAttr(r.analyteId)}')`,'ghost sm','Khôi phục giá trị mặc định'):rowActions.action==='remove'?`<button class="x" onclick="teaRefRemove('${escAttr(r.analyteId)}')" title="Xóa xét nghiệm tự thêm">✕</button>`:'';
    const namingTitle=globalThis.teaReferenceNamingTitlePresentation?globalThis.teaReferenceNamingTitlePresentation(r):[r.standardName&&`Tên chuẩn: ${r.standardName}`,r.abbreviation&&`Viết tắt: ${r.abbreviation}`,r.matrix&&`Loại mẫu: ${r.matrix}`].filter(Boolean).join(' · ');
    ...
    return `<tr><td><b title="${escAttr(namingTitle)}">${esc(r.displayName||r.name)}</b></td><td>${esc(r.unit||'—')}</td><td>${esc(r.section||'—')}</td>
      <td><input class="tea-ref-value" ${ro} type="number" step="any" value="${globalThis.teaReferenceInputValuePresentation?globalThis.teaReferenceInputValuePresentation(r.clia):r.clia==null?'':r.clia}" onchange="teaRefEdit('${escAttr(r.analyteId)}','clia',this.value)"></td>
      <td><input class="tea-ref-value" ${ro} type="number" step="any" value="${globalThis.teaReferenceInputValuePresentation?globalThis.teaReferenceInputValuePresentation(r.ricos):r.ricos==null?'':r.ricos}" onchange="teaRefEdit('${escAttr(r.analyteId)}','ricos',this.value)"></td>
      <td><div class="tea-lab-cell">${globalThis.teaReferenceLabValuePresentation?globalThis.teaReferenceLabValuePresentation(r.lab,fmt):r.lab==null?'':`<b>${fmt(r.lab,2)}%</b>`}${labButton}</div></td>
  ...
  return manageToolbar('Bảng TEa tham chiếu','Tổng hợp TEa từ các nguồn tham chiếu, dùng thống nhất khi tính Sigma.',canManage?'teaRefOpenAdd()':'','Thêm xét nghiệm')+teaSourceRegistryHtml()+
    `<div class="panel rcfg-list tea-ref-panel">${rows.length?`<table class="tea-ref-table"><thead><tr><th>Xét nghiệm</th><th>Đơn vị</th><th>Nhóm</th><th>TEa CLIA %</th><th>TEa Ricos %</th><th>TEa chuẩn hóa %</th><th>Trạng thái</th></tr></thead><tbody>${body}</tbody></table>`:(()=>{const empty=globalThis.teaReferenceEmptyStatePresentation?globalThis.teaReferenceEmptyStatePresentation(!!searchText(manageQ)):(searchText(manageQ)?{title:'Không tìm thấy',description:'Thử từ khóa khác.'}:{title:'Chưa có bảng tham chiếu',description:'Không có xét nghiệm nào.'});return emptyState(empty.title,empty.description);})()}</div>`;
}
function manageView(){
