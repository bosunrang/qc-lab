...
function manageLots(){
  ...
  const groupRows=state.lotGroups.filter(g=>manageMatch([g.name,g.note,...(g.lotIds||[]).map(id=>(state.qcLots.find(l=>l.id===id)||{}).lotNo)])).map(g=>{
    ...
    const toggleBtn=archived?'':(g.status==='stopped'||g.status==='planned'||!inUse)
      ?`<button class="btn teal sm" onclick="activateLotGroup('${g.id}')">Kích hoạt</button>`
      :`<button class="btn ghost sm btn-stop-tint" onclick="toggleLotGroupStatus('${g.id}')">Dừng</button>`;
    return `<div class="lot-group-card${archived?' lot-opt-depleted':''}"><div class="lot-group-card-h"><div><b>${esc(g.name)}</b><small>${esc(g.note||'Nhóm lô để gán Mean/SD theo Panel')}</small></div><span class="tag ${statusTag.cls}">${statusTag.text}</span></div><div class="lot-group-chipline">${lots.map(l=>`<span class="pill">${esc(l.lotNo)} · M${l.level}</span>`).join('')||'<span class="hint">Chưa chọn lô</span>'}</div><div class="lot-group-actions"><button class="btn ghost sm" onclick="openConfigGroup('${g.id}')">Sửa nhóm</button><button class="btn ghost sm" onclick="openTargetMatrix('','${g.id}')">Mean/SD</button>${toggleBtn}<button class="btn danger sm" onclick="deleteConfigGroup('${g.id}')">Xóa</button></div></div>`;}).join('');
  ...
  `<div class="lot-config-grid">
    <div class="panel rcfg-list lot-config-left">
      <div class="rcfg-panel-h"><h3>Lô QC</h3><button class="btn teal sm" onclick="openConfigLot()">Thêm lô</button></div>
    ...
    <div class="panel rcfg-list lot-config-right">
      <div class="rcfg-panel-h"><h3>Nhóm lô QC</h3><button class="btn teal sm" onclick="openConfigGroup()">Tạo nhóm lô</button></div>
...
function manageInstruments(){
  const rows=state.instruments.filter(i=>manageMatch([i.name,i.manufacturer,i.model,i.serial,i.section])).map(i=>{const n=state.tests.filter(t=>t.instrumentId===i.id).length;return `<tr><td><b>${esc(i.name)}</b><div class="hint">${esc(i.section||'Chưa phân khoa')}</div></td><td>${esc(i.manufacturer||'—')}</td><td>${esc(i.serial||'—')}</td><td class="num">${n}</td><td><span class="tag ${i.active?'ok':'none'}">${i.active?'Đang hoạt động':'Ngừng hoạt động'}</span></td><td><div class="manage-actions"><button class="btn ghost sm" onclick="openConfigInstrument('${i.id}')">Sửa</button><button class="btn danger sm" onclick="deleteConfigInstrument('${i.id}')">Xóa</button></div></td></tr>`;}).join('');
...
function managePanels(){
  const rows=state.qcPanels.filter(p=>manageMatch([p.name,p.note,instrumentName(p.instrumentId),...(p.testIds||[]).map(id=>(state.tests.find(t=>t.id===id)||{}).name)])).map(p=>{const tests=(p.testIds||[]).map(id=>state.tests.find(t=>t.id===id)).filter(Boolean);return `<tr><td><b>${esc(p.name)}</b></td><td>${esc(instrumentName(p.instrumentId))}</td><td>${tests.map(t=>`<span class="pill">${esc(t.name)}</span>`).join('')||'—'}</td><td class="num">${tests.length}</td><td><span class="tag ${p.active!==false?'ok':'none'}">${p.active!==false?'Đang dùng':'Tạm ngưng'}</span></td><td><div class="manage-actions"><button class="btn ghost sm" onclick="openConfigPanel('${p.id}')">Sửa panel</button><button class="btn teal sm" onclick="openTargetMatrix('${p.id}', '')">Mean/SD</button><button class="btn danger sm" onclick="deleteConfigPanel('${p.id}')">Xóa</button></div></td></tr>`;}).join('');
  return manageToolbar('Panel QC','Dùng Panel QC để gom xét nghiệm và nhập Mean/SD theo nhóm lô nhanh, nhất quán.',"openConfigPanel()",'Tạo Panel QC')+`<div class="panel rcfg-list">${rows?`<table class="panel-qc-table"><thead><tr><th>Tên Panel QC</th><th>Máy xét nghiệm</th><th>Xét nghiệm trong panel</th><th class="num">Số vị trí</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>${rows}</tbody></table>`:emptyState('Chưa có Panel QC','Tạo Panel QC trước, sau đó nhập Mean/SD theo nhóm lô trong thẻ Mean/SD.')}</div>`;
}
...
function manageTargets(){
  ...
  if(!state.qcPanels.length)return manageToolbar('Mean/SD theo nhóm lô QC','Chỉ dùng Panel QC để nhập Mean/SD hàng loạt.',"setManageTab('panels')",'Tạo Panel QC')+`<div class="panel">${emptyState('Chưa có Panel QC','Tạo Panel QC và chọn các xét nghiệm thành viên trước, sau đó quay lại nhập Mean/SD theo nhóm lô.')}</div>`;
  if(!state.qcLots.length)return manageToolbar('Mean/SD theo nhóm lô QC','Chọn Panel QC và nhóm lô để nhập Mean/SD hàng loạt.',"setManageTab('lots')",'Tạo lô QC')+`<div class="panel">${emptyState('Chưa có lô QC','Tạo lô QC trước, gom vào nhóm lô rồi quay lại nhập Mean/SD theo nhóm.')}</div>`;
  if(!state.lotGroups.length)return manageToolbar('Mean/SD theo nhóm lô QC','Chọn Panel QC và nhóm lô để nhập Mean/SD hàng loạt.',"setManageTab('lots')",'Tạo nhóm lô')+`<div class="panel">${emptyState('Chưa có nhóm lô QC','Tạo nhóm lô từ các lô QC trước, sau đó quay lại nhập Mean/SD theo nhóm.')}</div>`;
  ...
  const selectedLevel=Number(manageTargetLevel),levelLots=groupLots.filter(l=>+l.level===selectedLevel),levelDepletedLots=levelLots.filter(l=>l.depleted),q=searchText(manageQ),allTests=targetPanelTests(),tests=allTests.filter(t=>!q||[t.name,t.unit,t.method,t.reagent,t.section,instrumentName(t.instrumentId,t.machine),group&&group.name,...levelLots.map(l=>l.lotNo)].some(v=>searchText(v).includes(q)));
  ...
  const rows=rowItems.map(({t,lot,linked,same,assigned,planned,cfg})=>{const draft=targetRangeDraft(cfg||{}),locked=!!lot.depleted,retiredTo=locked?lotTransitionToNo(lot.id):'',checked=locked?false:(!!linked||!assigned),disabled=locked||!checked;return `<div class="target-row${locked?' target-row-locked':''}" data-test="${t.id}" data-lot="${lot.id}"${locked?' data-locked="1"':''}>
    ...
    <div class="lot-assay-name"><b>${esc(t.name)}</b><small>${esc(t.unit||'Chưa có đơn vị')}</small></div>
    <input class="tm-mean" type="number" step="any" value="${escAttr(targetNumberText(draft.mean))}" placeholder="Trung bình" oninput="syncTargetRange(this,'target')" ${disabled?'disabled':''}>
    <input class="tm-low" type="number" step="any" value="${escAttr(targetNumberText(draft.low))}" placeholder="Giới hạn dưới" oninput="syncTargetRange(this,'limits')" ${disabled?'disabled':''}>
    <input class="tm-high" type="number" step="any" value="${escAttr(targetNumberText(draft.high))}" placeholder="Giới hạn trên" oninput="syncTargetRange(this,'limits')" ${disabled?'disabled':''}>
    <input class="tm-sd" type="number" step="any" value="${escAttr(targetNumberText(draft.sd))}" placeholder="Độ lệch chuẩn" oninput="syncTargetRange(this,'target')" ${disabled?'disabled':''}>
  ...
  `<div class="panel target-matrix-panel">
    ...
    <div class="target-table"><div class="target-head"><span>Dùng</span><span>Xét nghiệm / Lô</span><span>Trung bình mục tiêu</span><span>Giới hạn dưới</span><span>Giới hạn trên</span><span>Độ lệch chuẩn</span><span>Trạng thái</span></div>${rows}</div>
    <div class="modal-f target-actions"><button class="btn ghost" onclick="targetCheckAll(false)">Bỏ chọn tất cả</button><button class="btn ghost" onclick="targetCheckAll(true)">Chọn tất cả</button><button class="btn teal" onclick="saveTargetMatrix()">Lưu Mean/SD mức này</button></div>`:(allTests.length?(levelDepletedLots.length&&levelDepletedLots.length===levelLots.length?emptyState(`Lô mức ${esc(manageTargetLevel)} đã hết QC`,`Lô ${esc(levelDepletedLots.map(l=>l.lotNo).join(', '))} (Mức ${esc(manageTargetLevel)}) trong nhóm này đã hết QC nên không nhập Mean/SD được. Hãy chọn nhóm lô khác ở ô “Nhóm lô QC” phía trên, hoặc tạo lô mới rồi lập hồ sơ chuyển tiếp lô.`):emptyState('Không tìm thấy xét nghiệm','Thử tìm theo tên xét nghiệm, máy, khoa, đơn vị hoặc lô QC.')):emptyState('Panel chưa có xét nghiệm','Sửa Panel QC và chọn các xét nghiệm thành viên trước.'))}</div>`;
}
function manageAssays(){
  const matched=state.tests.filter(t=>manageMatch([t.name,t.unit,t.method,t.reagent,instrumentName(t.instrumentId,t.machine),t.section,t.tea]));
  const rows=matched.map((t,idx)=>`<tr><td class="num">${idx+1}</td><td><b>${esc(t.displayName||t.name)}</b><div class="hint">${esc(t.method||'Chưa nhập phương pháp')} · ${esc(t.unit||'Chưa có đơn vị')}</div></td><td>${esc(instrumentName(t.instrumentId,t.machine))}<div class="hint">${esc(t.section||'Chưa gán khoa/khu vực')}</div></td><td>${esc(t.reagent||'—')}</td><td>${t.tea?esc(t.tea)+'%':'—'}</td><td><span class="tag ${t.closed?'none':'ok'}">${t.closed?'Ngưng dùng':'Đang dùng'}</span></td><td><div class="manage-actions"><button class="btn ghost sm" onclick="openConfigAssay('${t.id}')">Sửa</button><button class="btn ghost sm" onclick="entrySel={testId:'${t.id}',level:${t.levels[0].level}};go('entry')">Xem QC</button><button class="btn danger sm" onclick="delTest('${t.id}')">Xóa</button></div></td></tr>`).join('');
  return manageToolbar('Danh mục xét nghiệm','Chỉ quản lý tên xét nghiệm, máy, đơn vị, phương pháp và TEa. Lô QC và Mean/SD xem ở thẻ Lịch sử dữ liệu.',"openConfigAssay()",'Thêm xét nghiệm')+`<div class="panel rcfg-list">${rows?`<table class="assay-table"><thead><tr><th class="num">STT</th><th>Tên xét nghiệm</th><th>Máy xét nghiệm</th><th>Hóa chất</th><th>TEa</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>${rows}</tbody></table>`:emptyState('Chưa có xét nghiệm','Tạo xét nghiệm trước, sau đó gán lô và Mean/SD ở các thẻ cấu hình tương ứng.')}</div>`;
}
...
function manageHistory(){
  ...
  const opts=matches.map(x=>`<option value="${x.id}" ${x.id===t.id?'selected':''}>${esc(x.name)}</option>`).join('');
  const rows=[];
  (t.levels||[]).forEach(l=>{
  ...
  const html=visibleRows.sort((a,b)=>(+a.l.level)-(+b.l.level)||(a.lotNo||'').localeCompare(b.lotNo||'','vi')||String(a.h.effectiveFrom||'').localeCompare(String(b.h.effectiveFrom||''))).map(r=>{
    ...
    return `<tr><td><span class="pill">M${r.l.level}</span></td><td><b>${esc(r.lotNo||'—')}</b><div class="hint">${esc(r.group)}</div></td><td class="num">${fmt(r.h.mean)}</td><td class="num">${r.h.low!=null?fmt(r.h.low):'—'}</td><td class="num">${r.h.high!=null?fmt(r.h.high):'—'}</td><td class="num">${fmt(r.h.sd,3)}</td><td>${period}</td><td><span class="tag ${r.h.source==='lab'?'warn':'ok'}">${r.h.source==='lab'?'PXN':'NSX'}</span></td><td class="num">${r.pts.length}</td><td><button class="btn ghost sm" onclick="openQcHistoryDetail('${r.t.id}',${r.l.level},'${jsq(r.lotNo||'')}')">Chi tiết</button></td></tr>`;
  }).join('');
...
function teaRefOpenAdd(){
  ...
  openModal(`<div class="modal"><div class="modal-h"><h3>Thêm xét nghiệm tham chiếu</h3><button class="modal-close" onclick="closeModal()">✕</button></div>
    <div class="modal-b">
      <div class="grid2"><div><label>Tên quốc tế *</label><input id="trAddName" placeholder="VD: Creatine kinase-MB"></div><div><label>Viết tắt</label><input id="trAddAbbreviation" placeholder="VD: CK-MB"></div></div>
      ...
      <div class="hint" style="margin-top:8px">Mỗi xét nghiệm dùng một tên quốc tế duy nhất; viết tắt được hiển thị trong ngoặc.</div></div>
    <div class="modal-f"><button class="btn ghost" onclick="closeModal()">Hủy</button><button class="btn teal" onclick="teaRefAddSubmit()">Thêm</button></div></div>`);
...
async function teaRefAddSubmit(){
  ...
  const e=teaRefEnsure(name);e.name=name;
  e.abbreviation=QCCore.cleanText(document.getElementById('trAddAbbreviation').value,40).trim();
  ...
  e.matrix=QCCore.cleanText(document.getElementById('trAddMatrix').value,80).trim();
  e.unit=QCCore.cleanText(document.getElementById('trAddUnit').value,40);
  e.section=QCCore.cleanText(document.getElementById('trAddSection').value,80);
  e.clia=teaRefNumOrNull(document.getElementById('trAddClia').value);
  e.ricos=teaRefNumOrNull(document.getElementById('trAddRicos').value);
...
function manageTeaRefs(){
  ...
  const rows=effectiveTeaRefs()
    .map(([name,unit,clia,ricos,section,,analyteId])=>{const isDef=teaRefIsDefault(analyteId),record=overMap.get(analyteId),naming=teaAnalyteMeta(name,record);return{name,unit,clia,ricos,section,analyteId,...naming,kind:!isDef?'custom':record?'override':'default'};})
  ...
  const kindTag={default:'<span class="tag none">Mặc định</span>',override:'<span class="tag warn">Đã sửa</span>',custom:'<span class="tag ok">Tự thêm</span>'};
  const body=rows.map(r=>{
    const act=!canManage?'':r.kind==='override'?`<button class="btn ghost sm" onclick="teaRefRemove('${escAttr(r.analyteId)}')" title="Khôi phục giá trị mặc định">Khôi phục</button>`
