...
function reportHeader(title,subtitle='Nội kiểm chất lượng xét nghiệm'){const L=state.lab,app=window.QCLAB_APP||{version:'dev',build:''},rules=Object.entries(state.westgardRules||{}).filter(x=>x[1]!==false).map(x=>x[0]).join(', ');return '<div class="rpt-head">'+
  ...
  '<table class="meta-table"><tr><th>Phiên bản app</th><td>'+esc((app.name||'QC Lab')+' '+(app.version||'dev')+(app.build?' · '+app.build:''))+'</td><th>Bộ luật áp dụng</th><td>'+esc(rules||'Chưa cấu hình')+'</td></tr></table>'+
...
async function openPrint(title,bodyHtml,options={}){
  ...
  w.document.write('<!doctype html><html lang="vi"><head><meta charset="utf-8"><title>'+esc(title)+'</title><link href="https://fonts.googleapis.com/css2?family=Manrope:wght@400;500;600;700;800&display=swap" rel="stylesheet"><style>'+
    '@page{size:'+(options.landscape?'A4 landscape':'A4')+';margin:13mm}*{box-sizing:border-box}body{font-family:"Manrope",Arial,Helvetica,sans-serif;color:#14202b;margin:0;background:#eef2f5;font-size:12px;line-height:1.42}'+
    ...
    '</style></head><body><div class="page">'+bodyHtml+'</div><button class="print-btn" onclick="window.print()">In / Lưu PDF</button></body></html>');
