...
async function openPrint(title,bodyHtml,options={}){
  ...
  /* Desktop (Electron): tách 2 nút. "Lưu PDF" → opener.qcPrintPdf.save() → main
     process printToPDF (đường in headless CHUẨN, tôn trọng @media print/@page) —
     dứt điểm lỗi hộp thoại in hệ thống của Electron/Windows raster bằng CSS màn
     hình và phủ nền xám kín trang PDF. "In giấy" → qc-print:paper (main bật class
     .printing trước khi print()). Token ghép đúng cửa sổ in khi mở nhiều popup.
     Trình duyệt thường: 1 nút, window.print() như cũ. */
  const printToken='qp'+Date.now().toString(36)+Math.floor(Math.random()*1e6).toString(36);
  const btns=window.qcPrintPdf
    ?'<button class="print-btn" onclick="qcSavePdf()">Lưu PDF</button><button class="print-btn print-btn-alt" onclick="qcDoPrint()">In giấy</button>'
