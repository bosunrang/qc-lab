...
async function openPrint(title,bodyHtml,options={}){
  ...
  w.document.write('<!doctype html><html lang="vi"><head><meta charset="utf-8"><title>'+esc(title)+'</title><link rel="stylesheet" href="'+printFontCss+'"><style>'+
    ...
    /* Electron/Windows in qua hộp thoại hệ thống có thể raster bằng MEDIA MÀN HÌNH
       (bỏ qua @media print) → nền xám #eef2f5 + khung .page bị in vào PDF. Chế độ
       .printing do JS bật (click nút / beforeprint) đảm bảo trang sạch dù đường in
       nào cũng đúng; trình duyệt thường vẫn ưu tiên @media print như cũ. */
    ...
    '</style></head><body><div class="page">'+bodyHtml+'</div><button class="print-btn" onclick="qcDoPrint()">In / Lưu PDF</button>'+
    '<script>function qcDoPrint(){document.body.classList.add("printing");window.print()}window.onbeforeprint=function(){document.body.classList.add("printing")};window.onafterprint=function(){document.body.classList.remove("printing")};</'+'script></body></html>');
