...
async function printSigmaPeriod(periodId){
  ...
  body+='<table><tr><th class="rpt-key-18">Xét nghiệm</th><td>'+esc(testDisplayName(t))+(t.unit?' · '+esc(t.unit):'')+'</td><th class="rpt-key-15">Kỳ báo cáo</th><td>'+esc(period)+'</td></tr><tr><th>Thiết bị</th><td>'+esc(machine)+'</td><th>Nguồn TEa</th><td>'+esc(trace||'Chưa có thông tin truy xuất')+'</td></tr></table>';
...
async function printSigmaPeriods(){
  ...
  body+='<table><tr><th class="rpt-key-18">Xét nghiệm</th><td>'+esc(testDisplayName(t))+(t.unit?' · '+esc(t.unit):'')+'</td><th class="rpt-key-15">Các kỳ báo cáo</th><td>'+esc(periods||'—')+'</td></tr><tr><th>Thiết bị</th><td>'+esc(machine)+'</td><th>Nguồn TEa</th><td>'+esc(trace||'Chưa có thông tin truy xuất')+'</td></tr></table>';
...
async function printWestgard(){
  ...
  body+='<table><tr><th class="rpt-key-20">Xét nghiệm</th><td>'+esc(testDisplayName(t))+(t.unit?' · '+esc(t.unit):'')+'</td><th class="rpt-key-18">Thiết bị</th><td>'+esc(machine)+'</td></tr><tr><th>Luật theo từng mức</th><td colspan="3">'+esc(withinRules)+'</td></tr><tr><th>Luật liên mức / lần chạy</th><td colspan="3">'+esc(acrossRules)+'</td></tr></table>';
...
async function printReport(){
  ...
  body+='<table><tr><th class="rpt-key-25">Xét nghiệm</th><td>'+esc(testDisplayName(t))+(t.unit?' · '+esc(t.unit):'')+'</td><th class="rpt-key-18">Máy</th><td>'+esc(t.machine||'')+'</td></tr>'+
  ...
  operationalLevels(t).forEach(l=>{
    (typeof previousLotSeries==='function'?previousLotSeries(t,l.level):[]).forEach(s=>{const{inPts,items:allS}=reportPrevLotRows(t,s,inMonth);if(!inPts.length)return;
      ...
      if(violS.length){body+='<p class="rpt-section-label"><b>Điểm vi phạm/cảnh báo (lô cũ):</b></p><table><tr><th>Ngày</th><th>NV</th><th class="num">Giá trị</th><th class="num">Z</th><th>Luật</th><th>Loại sai số</th></tr>'+
    ...
    if(viol.length){body+='<p class="rpt-section-label"><b>Điểm vi phạm/cảnh báo:</b></p><table><tr><th>Ngày</th><th>NV</th><th class="num">Giá trị</th><th class="num">Z</th><th>Luật</th><th>Loại sai số</th></tr>'+
...
async function printRangeForm(tid,level){
  ...
  body+='<table><tr><th class="rpt-key-25">Xét nghiệm</th><td>'+esc(testDisplayName(t))+(t.unit?' · '+esc(t.unit):'')+'</td><th class="rpt-key-18">Mức / Lô</th><td>M'+level+' / '+esc(l.lot||'?')+'</td></tr>'+
