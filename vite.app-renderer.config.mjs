...
const NODE_ONLY_BROWSER_SWAPS = {
  'sha256': './app/main/domain/sha256-browser.ts',
  'password-hash': './app/main/domain/password-hash-browser.ts',
  'db-file-size': './app/main/ipc/db-file-size-browser.ts',
};
...
const swapNodeOnlyModulesForBrowser = {
  ...
  // compatibility'). `app:build` xanh KHÔNG chứng minh plugin chạy đúng —
  ...
  resolveId(source, importer) {
    ...
    // Chỉ đổi khi importer thật sự nằm trong `app/main/`, để một
    // `./sha256` ở nơi khác không bị thay ngầm.
    const importerPath = importer.split(String.fromCharCode(92)).join('/');
    if (!importerPath.includes('/app/main/')) return null;
    return fileURLToPath(new URL(replacement, import.meta.url));
  },
};
// Renderer của app — Vite build thật (ES module, không IIFE thủ công như
...
// qua `npm run app:dev`, KHÔNG áp dụng khi build cho Electron thật qua
// `npm run app:build`) — xem docs/APP-PLAN.md mục "Xem qua localhost".
...
export default defineConfig({
  ...
  server: {
    ...
    // xem trước chỉ chạy khi gọi tay `npm run app:dev`, dữ liệu là bản
  ...
  build: {
    ...
    rollupOptions: {
      input: {
        index: fileURLToPath(new URL('./app/renderer/index.html', import.meta.url)),
        'firebase-guide': fileURLToPath(new URL('./app/renderer/firebase-guide.html', import.meta.url)),
      },
    },
  },
});
