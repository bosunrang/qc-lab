 ...
 * `JSON.parse` để một file khổng lồ không treo tiến trình. app-v2 trước
...
export function createBackupHandlers(db: Db, userDataDir: string) {
  ...
  function exportBackup(actor: Actor): IpcResult<string> {
    ...
    const envelope = buildBackupEnvelope(data, SCHEMA_VERSION, 'app-v2', nowIso());
    ...
    // trong `state`; app-v2 dùng `app_meta` — cùng cơ chế key/value đã dùng
