// Đồng bộ Firebase cho app-v2. Dữ liệu SQLite luôn được đóng thành backup có
...
export function createFirebaseHandlers(db: Db, userDataDir: string, client: FirebaseClient = createFirebaseClient()) {
  ...
  function payload(): FirebasePayload {
    return {
      ...
      backup: buildBackupEnvelope(dumpAllTables(db), SCHEMA_VERSION, 'app-v2', nowIso()),
    };
  }
  function parsePayload(value: unknown): { ok: true; data: FirebasePayload } | { ok: false; message: string } {
    ...
    if (row._format !== 'qclab-v2-firebase' || !row.backup) {
      return { ok: false, message: 'Dữ liệu Firebase này không phải định dạng app-v2. Hãy xuất backup từ app cũ rồi dùng mục Di trú dữ liệu.' };
    }
