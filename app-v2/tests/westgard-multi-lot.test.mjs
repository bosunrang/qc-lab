...
// Đổi riêng scope R4s sang within phải tắt đánh giá chéo. 1-2s vẫn là cảnh
// báo đơn mức, chứng minh hai kênh được phân giải độc lập.
assert.equal(config.saveRuleScope(test.id, 'R4s', 'within', actor).ok, true);
...
console.log('app-v2 current-lot + multi-level Westgard tests passed');
