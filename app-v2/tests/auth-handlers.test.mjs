...
// 15b) Tao tai khoan kem ma viet tat va the duoc phep dung
const withPerms = auth.createUser({
  data: { username: 'kts2', name: 'Nguyen Thi Lan', initials: 'ntl', role: 'technician', password: 'mat-khau-kts2-1', pagePerms: ['dash', 'entry'] },
}, secondAdminActor);
assert.equal(withPerms.ok, true);
assert.equal(withPerms.data.initials, 'NTL', 'ma viet tat phai duoc chuan hoa thanh CHU IN (nhu addUser app cu)');
...
const activityRows = require('../../app-v2-dist/main/ipc/config-handlers.js').createConfigHandlers(db).listActivity(1000);
