...
//
// KHÔNG chốt bất biến "verdict === 'rej' thì accepted === false" cho MỌI điểm:
// `verdict` tính trên CHUỖI THÔ (mọi điểm), còn chuỗi chấp nhận cố ý bỏ điểm
// đã bị loại ra khỏi cả cửa sổ đánh giá các điểm sau — nhờ vậy một lần chạy bị
// loại không "làm bẩn" chuỗi của những lần chạy sau. Hai con số tính trên hai
// cơ sở khác nhau là thiết kế có chủ đích của `acceptedPoints()`, mục 4 dưới
// đây chốt đúng tính chất đó.
...
// 4) Lần chạy bị loại KHÔNG làm bẩn cửa sổ của lần chạy sau — tính chất cốt
//    lõi của `acceptedPoints()`, phải còn nguyên sau bản sửa.
//
//    run0: z=0            → sạch
//    run1: mức 1 +2,5 / mức 2 -2,5 → R4s loại CẢ lần chạy
//    run2: mức 1 +2,2    → nếu điểm run1 còn trong cửa sổ thì [+2,5; +2,2] nổ
//                          2-2s within và điểm run2 cũng bị loại; đúng ra nó
//                          phải được chấp nhận.
{
  ...
  assert.equal(p['L1r2'].accepted, true, 'lần chạy sau vẫn được chấp nhận — cửa sổ đã bỏ điểm bị loại');
  // `verdict` của điểm run2 vẫn là 'rej' vì nó tính trên CHUỖI THÔ (còn điểm
  // run1). Hai con số tính trên hai cơ sở khác nhau là thiết kế có chủ đích,
  // không phải mâu thuẫn mới — chốt lại để lần đọc sau không "sửa" nhầm.
...
console.log('app-v2: chuỗi chấp nhận tính cả luật liên mức — tests passed');
