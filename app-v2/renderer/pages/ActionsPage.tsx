        ...
        .map((lv) => {
          ...
          return {
            ...
            guidance: errorType === 'RE'
              ...
              : 'Rà soát xu hướng, hiệu chuẩn, hóa chất/calibrator và tình trạng thiết bị.',
...
function exportNceLogCsv(records: NceRecord[], testName: (id: string | null) => string) {
  ...
  for (const record of records) {
    ...
    lines.push([
      record.nce_id, testName(record.test_id), record.date, record.level ?? '', record.lot || '', record.rule,
      record.error_type, detail.correction || '', record.approval_status, record.effectiveness_status,
      record.record_status, record.due_date,
    ].map(csvCell).join(','));
  }
                ...
                return (
                  <tr key={record.id} className={cancelled ? 'action-log-row-cancelled' : undefined}>
                    ...
                    <td>
                      ...
                      <div className="action-rule">{record.rule || '—'} · {record.error_type === 'SE' ? 'Sai số hệ thống' : 'Sai số ngẫu nhiên'}</div>
                    </td>
                    <td>
...
function DetailModal({ record, testName, onClose }: { record: NceRecord; testName: string; onClose: () => void }) {
  ...
  return (
    <Modal title={`Hồ sơ ${record.nce_id} — ${testName}`} onClose={onClose} width={640}
      ...
      <p className="action-detail-muted">Hạn hoàn thành: {record.due_date || '—'}</p>
      ...
      {record.action_completed_date && record.effectiveness_status === 'pending' && canReview && (
        <>
          <p className="action-detail-muted">Hoàn thành hành động: {record.action_completed_date}</p>
