...
export function EntryPage() {
  ...
  // nhập, không phải bấm thêm 1 lần. Bản app-v2 trước đây để `testId` rỗng nên
  ...
  const entryColumns: EntryColumn[] = levels.flatMap((level) => {
    ...
    const chartPoints: QcChartPoint[] = candidate.points.map((point) => ({
      ...
      verdict: point.verdict, rules: point.rules, accepted: point.verdict !== 'rej',
    }));
    return [main, {
  ...
  const displayColumns: DisplayColumn[] = entryColumns.map((column) => {
    ...
    return {
      ...
      chartPoints: previous.points.map((point) => ({
        date: point.date, runId: point.run_id, val: point.val,
        z: (point.val - previous.mean) / previous.sd,
        verdict: point.verdict, rules: point.rules, accepted: point.verdict !== 'rej',
      })),
    };
  });
                  ...
                  <table className="qc-sheet">
                    ...
                    <tbody>
                      {days.map((date) => {
                        ...
                        const staff = Array.from(new Set(dayPoints.map((p) => p.operator_name).filter(Boolean)));
              ...
              <div className="panel">
                ...
                <div className="lj-stack">
                  {displayColumns.map((column) => {
                    ...
                    const acceptedPoints = column.previous
                      ? inLjWindow(column.points.filter((point) => !point.voided))
                      : column.parallel
                      ? inLjWindow(column.points.filter((point) => !point.voided && point.verdict !== 'rej'))
