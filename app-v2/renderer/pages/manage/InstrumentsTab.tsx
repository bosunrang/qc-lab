...
export function InstrumentsTab({ createRequest = 0, onCreateRequestHandled }: { createRequest?: number; onCreateRequestHandled?: () => void } = {}) {
  const { instruments, tests, saveInstrument, removeInstrument } = useManageStore();
  ...
  return (
    <>
        <div className="rcfg-toolbar">
          <div><h2>Máy xét nghiệm</h2><p>Quản lý máy xét nghiệm, hãng và số sê-ri.</p></div>
      ...
      <div className="panel rcfg-list">
        {!instruments.length ? <EmptyState title="Chưa có máy xét nghiệm">Thêm máy xét nghiệm đầu tiên để bắt đầu cấu hình QC.</EmptyState>
          ...
          : <table className="instrument-table">
            <thead><tr><th>Máy xét nghiệm</th><th>Nhà sản xuất</th><th>Số sê-ri</th><th>Xét nghiệm</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
            <tbody>
              {visible.map((i) => (
                <tr key={i.id}>
                  <td><b>{i.name}</b><div className="hint">{i.section || 'Chưa phân khoa'}</div></td>
                  <td>{i.manufacturer || '—'}</td><td>{i.serial || '—'}</td>
                  <td className="num">{tests.filter((t) => t.instrument_id === i.id).length}</td>
                  <td><span className={`tag ${i.active ? 'ok' : 'none'}`}>{i.active ? 'Đang hoạt động' : 'Ngừng hoạt động'}</span></td>
