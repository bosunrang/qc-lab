...
function CreatePanel() {
  ...
  async function submit() {
    ...
    const result = await create({ username: username.trim().toLowerCase(), name, initials, role, password, pagePerms });
    if (!result.ok) { await infoDialog(result.error.message); return; }
    setUsername(''); setName(''); setInitials(''); setPassword('');
      ...
      <div className="user-create-layout">
        <div className="user-create-card">
          ...
          <div className="user-create-fields">
            ...
            <div><label htmlFor="uInitials">Mã viết tắt</label><input id="uInitials" maxLength={12} placeholder="NTL" value={initials} onChange={(e) => setInitials(e.target.value)} /></div>
