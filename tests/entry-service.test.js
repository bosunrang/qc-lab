...

{
  ...
  const assayModalSource = fs.readFileSync(path.join(__dirname, '..', 'src', 'react', 'modals', 'AssayModal.tsx'), 'utf8');
  const assayDecimalOptionsSource = assayModalSource;
  ...
  assert.match(assayDecimalOptionsSource, />\{v\}<\/option>/, 'danh sách số thập phân chỉ được hiển thị giá trị số gọn');
