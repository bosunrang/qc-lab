...
let next=0;const records={create:(actions,values)=>{const record={id:'n'+(++next),...values};actions.push(record);return record},update:(record,values)=>Object.assign(record,values)};
...
assert.equal(output.created.ok,true);assert.equal(output.created.mode,'create');assert.equal(output.length,1);
