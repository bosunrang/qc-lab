...
const program = `
  ...
  console.log(createDashboardMissingTargetItemHtml({ escape: value => String(value).replaceAll('<', '&lt;'), testLabel: test => test.name, button: (label, action, variant) => '[' + label + '|' + (typeof action === 'string' ? action : JSON.stringify(action)) + '|' + variant + ']' })({ t: { name: '<A>' }, l: { level: 3 } }));
`;
...
assert.match(result.stdout, /Gán Mean\/SD\|\{"action":"goManageTargets"\}\|ghost sm/);
