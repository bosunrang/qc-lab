...
for(const name of names){
  ...
  const isReportPrintDep=name.includes('Print')||name==='sigmaMuTraceService';
  assert.match(isReportPrintDep?reports:io,new RegExp(`deps\\.${name}`),`${name} must be consumed by the export or print UI`);
}
