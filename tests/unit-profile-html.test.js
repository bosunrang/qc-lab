...
const program = `import { createUnitProfileHtml } from ${JSON.stringify(source)}; console.log(createUnitProfileHtml({escapeAttribute:value => 'ATTR:' + value,button:(label, action) => '<button>' + label + ':' + (typeof action === 'string' ? action : JSON.stringify(action)) + '</button>'})({name:'BV A',dept:'Hóa sinh',address:'1 Đường A'}));`;
...
assert.match(result.stdout, /\{"action":"saveLab"\}/);
