...
const program = `
  ...
  const render = createActionLogPanelHtml({ button: (label, action, variant) => '[' + label + '|' + (typeof action === 'string' ? action : JSON.stringify(action)) + '|' + variant + ']', emptyState: (title, text) => '<empty>' + title + ':' + text + '</empty>' });
...
assert.match(full, /Xuất CSV nhật ký\|\{"action":"exportActionsCSV"\}\|teal sm/);
