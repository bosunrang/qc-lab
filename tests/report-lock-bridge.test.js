...
assert.match(route,/globalThis\.ReportPeriodCommand\.lock\(\{state,/,'Khóa kỳ phải dùng command TypeScript');
assert.match(route,/globalThis\.ReportPeriodCommand\.unlock\(\{state,/,'Mở khóa kỳ phải dùng command TypeScript');
...
assert.match(bridge,/ReportPeriodCommand: ReportPeriodCommand;/,'Command khóa kỳ phải là hợp đồng bridge bắt buộc');
