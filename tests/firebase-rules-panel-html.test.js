...
const program = `import { createFirebaseRulesPanelHtml } from ${JSON.stringify(source)}; console.log(createFirebaseRulesPanelHtml({escape:value => '[' + value + ']',button:(label, action) => '<button>' + label + ':' + (typeof action === 'string' ? action : JSON.stringify(action)) + '</button>'})('<guide>', '{"rules":{}}'));`;
...
assert.match(result.stdout, /\{"action":"copyFirebaseRules"\}/);
