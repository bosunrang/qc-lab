...
const program = `
  ...
  const render = createActionReviewButtonsHtml({ button: (label, action, variant, title = '') => '[' + label + '|' + (typeof action === 'string' ? action : JSON.stringify(action)) + '|' + variant + '|' + title + ']' });
...
assert.match(active, /Chi tiết\|\{"action":"viewActionDetail","args":\[4\]\}\|ghost sm/);
assert.match(active, /Tiếp tục\|\{"action":"editAction","args":\[4\]\}/);
assert.match(active, /Duyệt\|\{"action":"approveAction","args":\[4\]\}/);
assert.match(active, /Mở lại\|\{"action":"reopenAction","args":\[4\]\}\|danger sm/);
assert.doesNotMatch(active, /Hủy hồ sơ/);
assert.match(minimal, /Chi tiết\|\{"action":"viewActionDetail","args":\[2\]\}/);
