...
const program = `
  ...
  const render = createWestgardCusumPageHtml({ empty: (title, message, action = '') => '<empty>' + title + '|' + message + '|' + action + '</empty>', button: (label, action, variant) => '[' + label + '|' + (typeof action === 'string' ? action : JSON.stringify(action)) + '|' + variant + ']', escape: value => String(value).replaceAll('<', '&lt;'), testValue: (_test, value) => 'V:' + value, format: (value, decimals) => Number(value).toFixed(decimals), quote: value => String(value).replaceAll("'", "\\\\'") });
...
assert.match(off, /\{"action":"openConfigAssay","args":\["T'1"\]\}/);
