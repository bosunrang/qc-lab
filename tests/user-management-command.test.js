...
command.updatePermissions(added,{role:'supervisor',pagePerms:inputPerms});inputPerms.push('settings');command.resetPassword(added,'new-hash',false);const afterDisable=command.toggle(added).active;const afterEnable=command.toggle(added).active;const removed=command.remove(users,'u1');const missing=command.remove(users,'missing');
console.log(JSON.stringify({users,added,afterDisable,afterEnable,removed,missing}));`;
