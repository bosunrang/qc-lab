...
run(ctx,`
  ...
  globalThis.qcReportContext={teaInfo:t=>({teaVal:sgTea(t),teaSourceText:sgTeaLabel(sgTeaSource(t))}),multiViews:(t,inRange)=>operationalLevels(t).map(l=>({level:l.level,lot:l.lot,mean:l.mean,sd:l.sd,pts:operationalLotPoints(t,l.level).filter(inRange),label:'M'+l.level+'·'+(l.lot||'?')}))};
  globalThis.qcReportRowsService={previousLot:(t,s,inRange)=>{const inPts=s.pts.filter(inRange);if(!inPts.length)return{inPts,items:[]};const wgP=QCCore.westgardByPoint(s.pts,s.mean,s.sd,rule=>testRuleOnWithin(t,rule)),idxOf=new Map(s.pts.map((p,i)=>[p.id,i]));return{inPts,items:inPts.map(p=>{const i=idxOf.get(p.id),raw=wgP.F[i]||{rules:[]};return{p,f:{...raw,level:ruleResultLevel(t,raw.rules||[])},z:wgP.zs[i]};})};},currentLot:(t,l,wg,inRange)=>{const pts=operationalLotPoints(t,l.level).filter(inRange);return{pts,items:pts.map(p=>{const f=wg.byPoint.get(p.id)||{level:'ok',rules:[],z:(p.val-l.mean)/l.sd};return{p,f,z:f.z};})};},actions:(tid,inRange)=>(state.actions||[]).filter(a=>a.testId===tid&&inRange({date:actionEventDate(a)}))};
  ...
  globalThis.reportExportHelpers={inRange:(start,end)=>p=>(!start||p.date>=start)&&(!end||p.date<=end),nceExcerpt:(value,max=150)=>{const text=String(value||'').replace(/\\s+/g,' ').trim();if(text.length<=max)return text||'—';const cut=text.slice(0,max-1).replace(/\\s+\\S*$/,'').trim();return(cut||text.slice(0,max-1))+'…';}};
