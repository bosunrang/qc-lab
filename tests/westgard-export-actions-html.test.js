...
const program = `
  ...
  const render = createWestgardExportActionsHtml({ button: (label, action, variant, title) => '[' + label + '|' + (typeof action === 'string' ? action : JSON.stringify(action)) + '|' + variant + '|' + title + ']', downloadIcon: () => '<down/>', printIcon: () => '<print/>' });
...
assert.match(lj, /<down\/>Xuất Excel\|\{"action":"exportWestgardXLSX"\}\|teal wg-excel-btn/);
assert.match(lj, /<print\/>In PDF\|\{"action":"printWestgard"\}\|teal wg-print-btn/);
assert.equal(cusum, '');
