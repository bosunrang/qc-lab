...
assert.match(route, /\$\{globalThis\.historySelectorPresentation\(opts,historyTotals\.rowCount,historyTotals\.pointCount\)\}/, 'Bộ chọn lịch sử phải do presentation TypeScript dựng');
assert.match(route, /\$\{globalThis\.historyTablePresentation\(html,q\?emptyState/, 'Bảng lịch sử phải do presentation TypeScript dựng');
