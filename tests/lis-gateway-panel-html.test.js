...
const program = `import { createLisGatewayPanelHtml } from ${JSON.stringify(source)}; console.log(createLisGatewayPanelHtml({escape:value => '[' + value + ']',escapeAttribute:value => 'ATTR:' + value,button:(label, action) => '<button>' + label + ':' + (typeof action === 'string' ? action : JSON.stringify(action)) + '</button>'})({url:'http://127.0.0.1:8787',token:'tok',enabled:true,status:'ok',statusText:'Đã kết nối'}));`;
...
assert.match(result.stdout, /\{"action":"lisGatewaySaveSettings"\}/);
