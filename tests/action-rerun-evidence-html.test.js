...
const program = `
  ...
  const render = createActionRerunEvidenceHtml({ escape: value => String(value).replaceAll('<', '&lt;'), pointValue: point => 'V:' + point.value, date: value => 'D:' + value, button: (label, action, variant, title) => '[' + label + '|' + (typeof action === 'string' ? action : JSON.stringify(action)) + '|' + variant + '|' + title + ']' });
...
assert.match(complete, /\{"action":"openActionQcEvidence","args":\["T'1",2,"P'1","2026-08-12","L1"\]\}/);
