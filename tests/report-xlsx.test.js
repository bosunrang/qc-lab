...
//     as the PDF, and the shared checkbox can suppress only the appendix. ---
{
  run(ctx, `
    ...
    operationalLevels=()=>[];activeWestgard=()=>({});sgTea=()=>5;sgTeaSource=()=>'';sgTeaLabel=()=>'';sgTeaRefText=()=>'';formatDateTimeVN=v=>String(v);userName=()=>'Tester';reportRangeText=()=>'17/07/2026 - 29/07/2026';testDisplayName=t=>t.name;actionEventDate=a=>a.date;actionWorkflowStatus=()=>({label:'Đã khép vòng'});actionRerunStatus=()=>({label:'QC đạt lại',point:{val:140.2,date:'2026-07-17',runId:'2'}});actionLevelShort=()=> 'M1 - Lô 1101';actionApprovalLabel=()=> 'Đã duyệt';actionEffectivenessStatus=()=>({label:'Có hiệu lực'});actionRiskScore=()=>60;actionResidualRiskScore=()=>8;vnDate=v=>String(v);fmt=v=>String(v);globalThis.qcExportValueFormat={value:(t,v)=>fmt(v),stat:(t,v)=>fmt(v),point:(p,t)=>fmt(p&&p.val)};globalThis.qcReportContext={teaInfo:t=>({teaVal:sgTea(t),teaSourceText:sgTeaLabel(sgTeaSource(t))}),multiViews:()=>[]};globalThis.qcReportRowsService={previousLot:()=>({inPts:[],items:[]}),currentLot:()=>({pts:[],items:[]}),actions:(tid,inRange)=>(state.actions||[]).filter(a=>a.testId===tid&&inRange({date:actionEventDate(a)}))};globalThis.reportExportHelpers={inRange:(start,end)=>p=>(!start||p.date>=start)&&(!end||p.date<=end),nceExcerpt:(value,max=150)=>{const text=String(value||'').replace(/\\s+/g,' ').trim();if(text.length<=max)return text||'—';const cut=text.slice(0,max-1).replace(/\\s+\\S*$/,'').trim();return(cut||text.slice(0,max-1))+'…';}};globalThis.actionReportSummary=a=>{const causeLabel=ACTION_LABELS.cause&&ACTION_LABELS.cause[a.causeCategory];return[['Tức thời',reportNceExcerpt(a.correction||a.containmentNote||'Chưa ghi',120)],['Nguyên nhân',reportNceExcerpt([causeLabel,a.cause].filter(Boolean).join(' - ')||'Chưa xác định',140)],['Khắc phục',reportNceExcerpt(a.action||'Chưa ghi',160)]];};
  `);
