...
const route = fs.readFileSync(path.join(__dirname, '..', 'assets', 'modules', 'manage-routes.js'), 'utf8');
...
assert.match(route, /globalThis\.teaReferenceSortPresentation\(rows\);/,
  'bảng TEa phải truyền cả danh sách vào sorter TypeScript');
assert.doesNotMatch(route, /\.sort\(globalThis\.teaReferenceSortPresentation\)/,
