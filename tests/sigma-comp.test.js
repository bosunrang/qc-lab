...

{
  ...
  assert.ok(source.indexOf('<h3>Số liệu theo kỳ</h3>')<source.indexOf('<span>Thiết kế QC theo Sigma (OPSpecs)</span>')&&source.indexOf('<span>Thiết kế QC theo Sigma (OPSpecs)</span>')<source.indexOf('<span>Độ không đảm bảo đo (MU) — ISO 15189:2022</span>'), 'Sigma follows the input-to-results order: period data, QC design, then MU');
  ...
  assert.match(source, /Xuất Excel tổng hợp/, 'combined export is clearly distinguished from row export');
  ...
  assert.match(source, /btn\(icoDownload\(\)\+'Xuất Excel tổng hợp','exportSigmaPeriodsXLSX\(\)','teal sg-combined-export'/, 'combined Excel export is available under the period table');
  assert.match(source, /btn\(printIcon\+'Xuất PDF tổng hợp','printSigmaPeriods\(\)','teal sg-combined-print'/, 'combined PDF export is available under the period table');
