...

{
  ...
  assert.match(sigmaPage, /data-sg-period-id=\{row\.id\}[\s\S]*?onClick=\{e => \{ if/, 'each period row is selected by clicking the row');
  ...
  assert.match(sigmaPage, /onClick=\{\(\) => exportSigmaPeriodXLSX\(row\.id\)\}/, 'each period row exports its own workbook');
  assert.match(sigmaPage, /className="btn danger sm sg-row-delete"[\s\S]*?onClick=\{\(\) => sgDelPeriod\(row\.id\)\}/, 'each writable period row uses the labelled system-danger delete button');
  assert.doesNotMatch(sigmaPage, /class="x" onclick="sgDelPeriod/, 'the obsolete icon-only period delete control is removed');
  assert.match(sigmaPage, /onClick=\{exportSigmaPeriodsXLSX\}/, 'the footer exports the combined multi-period comparison workbook');
  ...
  assert.match(sigmaPage, /sg-combined-export"[\s\S]*?onClick=\{exportSigmaPeriodsXLSX\}[\s\S]*?icoDownloadHtml\(\) \+ 'Xuất Excel'/, 'combined Excel export is available under the period table');
  ...
  assert.match(biasModalSource, /Áp dụng cho kỳ nào\?/, 'EQA Bias modal can apply the reviewed Bias to selected periods');
  ...
  assert.match(sigmaCss, /button\.btn\.sg-row-cv\{[\s\S]*?background:var\(--success-bg\);[\s\S]*?color:var\(--success-ink\);/, 'lot CV action uses the soft success-tag pill style, matching the "Đủ hôm nay" badge');
...
{
  const editCtx=loadSandbox(['core.js','generated/modular-pilot.js']);
...
{
  const pickerCtx=loadSandbox(['core.js','generated/modular-pilot.js']);
