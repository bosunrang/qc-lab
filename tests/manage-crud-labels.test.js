...
const lotTransitionModal = fs.readFileSync(path.join(__dirname, '..', 'src', 'react', 'modals', 'LotTransitionModal.tsx'), 'utf8');
...
records.forEach(([editName,addName])=>{
  ...
  else if(addName==='hồ sơ chuyển lô'){assert.ok(lotTransitionModal.includes(`{id ? 'Sửa ${editName}' : 'Thêm ${addName}'}`), `tiêu đề ${addName} phải phân biệt Thêm/Sửa`);assert.ok(lotTransitionModal.includes(`{id ? 'Lưu thay đổi' : 'Thêm ${addName}'}`), `nút ${addName} phải dùng Thêm khi tạo và Lưu thay đổi khi sửa`);return;}
