...
assert.match(sigma,/const sgCleanCell = \(field: string, val: unknown\) => deps\.SigmaLevelEditService\.clean\(field, val\);/,'Chuẩn hóa mức Sigma phải dùng service TypeScript');
assert.match(sigma,/deps\.SigmaLevelEditService\.update\(L, field, val\)/,'Sửa mức Sigma phải dùng service TypeScript');
