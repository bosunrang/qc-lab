...
const route=fs.readFileSync(path.join(root,'assets','modules','westgard-routes.js'),'utf8');
...
for(const [name,type] of [
  ...
  assert.match(route,new RegExp(`globalThis\\.${name}`),`${name} must be consumed by the Westgard UI`);
}
