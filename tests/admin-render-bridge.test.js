...
const actions=fs.readFileSync(path.join(root,'assets','modules','actions-routes.js'),'utf8');
const manage=fs.readFileSync(path.join(root,'assets','modules','manage-routes.js'),'utf8');
...
for(const [name,type,source] of [
  ['actionGuideContent','ReturnType<typeof createActionGuideContent>',actions],
  ['manageHistoryRowPresentation','ReturnType<typeof createManageHistoryRowHtml>',manage],
  ...
  assert.match(source,new RegExp(`globalThis\\.${name}`),`${name} must be consumed by its UI route`);
}
