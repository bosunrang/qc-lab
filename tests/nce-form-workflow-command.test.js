...
assert.deepEqual(value.invalid,{ok:false,message:'Thiếu'});assert.equal(value.done.ok,true);
// Giai đoạn 7 (state immutable, nhóm actions/NCE, 2026-08-31): submit() giờ tự
// gán lại state.actions=[...actions,record] khi result.mode==='create' (thay vì
// dựa vào create() tự push tại chỗ) — record mới phải xuất hiện trong state.actions.
assert.deepEqual(value.state,{actions:[{id:'n1'}]});
assert.deepEqual(value.logs,[['Tạo','n1','NCE']]);assert.deepEqual([value.resets,value.saves,value.renders],[1,1,1]);
