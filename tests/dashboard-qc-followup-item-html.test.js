...
const program = `
  ...
  const render = createDashboardQcFollowupItemHtml({ escape: value => String(value).replaceAll('<', '&lt;'), testLabel: test => test.name, date: value => 'D:' + value, pointValue: point => 'V:' + point.value, button: (label, action, variant) => '[' + label + '|' + (typeof action === 'string' ? action : JSON.stringify(action)) + '|' + variant + ']', quote: value => String(value).replaceAll("'", "\\\\'") });
...
assert.match(result.stdout, /\{"action":"dashboardGoEntryFollowup","args":\["T'1",2\]\}/);
