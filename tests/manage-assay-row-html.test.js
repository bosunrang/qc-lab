...
const program = `
  ...
  const render = createManageAssayRowHtml({ escape: value => String(value).replaceAll('<', '&lt;'), quote: value => String(value).replaceAll("'", "\\\\'"), button: (label, action, variant) => '[' + label + '|' + (typeof action === 'string' ? action : JSON.stringify(action)) + '|' + variant + ']' });
...
assert.match(active, /Sửa\|\{"action":"openConfigAssay","args":\["A'1"\]\}\|ghost sm/);
