...
/* Cong LIS Gateway (8787) khong the gom ve MOT hang so that su: no hardcode doc lap o
   ...
   (doc truoc khi bat ky JS nao chay), client trinh duyet (lis-client-service.js), va
...
const client=read('assets/modules/lis-client-service.js');
...
assert.ok(clientPorts.length>=3,'lis-client-service.js phai tham chieu cong Gateway o URL mac dinh, danh sach origin cho phep, va thong bao loi');
clientPorts.forEach(p=>assert.equal(p,port,`lis-client-service.js dang tham chieu cong ${p}, lech voi cong mac dinh ${port} cua lis-gateway/server.js`));
