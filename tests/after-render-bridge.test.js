...
const route=fs.readFileSync(path.join(root,'assets','modules','after-render.js'),'utf8');
...
for(const name of ['configNavScrollService','entryJumpScrollService','defaultDateFieldsService','postRenderPageActions']){
  ...
  assert.match(route,new RegExp(`globalThis\\.${name}`),`${name} must be consumed after render`);
}
