...
const program = `
  ...
  const render = createDashboardOverdueActionItemHtml({ escape: value => String(value).replaceAll('<', '&lt;'), testLabel: test => test.name, date: value => 'D:' + value, button: (label, action, variant) => '[' + label + '|' + (typeof action === 'string' ? action : JSON.stringify(action)) + '|' + variant + ']' });
...
assert.match(withTest, /Tiếp tục hồ sơ\|\{"action":"dashboardContinueAction","args":\[3\]\}\|ghost sm/);
