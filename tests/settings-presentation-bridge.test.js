...
assert.match(settings,/deps\.html\.brandPreviewHtml\(\{ logo, markText: deps\.brand\.markText\(\), title: deps\.brand\.title\(\), subtitle: deps\.brand\.subtitle\(\) \}\)/,'Preview thương hiệu phải dùng bridge TypeScript');
assert.match(settings,/deps\.profileCommand\.saveLab\(\{ name: fieldValue\('labName'\)/,'Hồ sơ đơn vị phải dùng command TypeScript');
assert.match(settings,/deps\.html\.firebaseRulesPanelHtml\(deps\.html\.firebaseGuideHtml\(\), deps\.html\.firebaseRulesText\(\)\)/,'Panel quy tắc Firebase phải dùng bridge TypeScript');
