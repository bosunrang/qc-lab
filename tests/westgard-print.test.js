...
const ctx = loadSandbox(['core.js', 'generated/modular-pilot.js'], { window: { QCLAB_APP: { name: 'QC Lab', version: '2.4.0' } }, document: { getElementById: () => null, addEventListener: () => {} } });
run(ctx, `
  reportSignBlock=()=>'';
  reportHeaderPresentation=model=>'<div>'+model.app.name+' '+model.app.version+'</div>';
  reportQcFormat={value:(test,value)=>String(value),stat:(test,value)=>String(value),point:point=>String(point&&point.val)};
  reportPointsTableService=items=>'<table>'+items.map(item=>'<tr><td>'+item.p.date+'</td><td>'+(item.p.runId||'—')+'</td><td>'+(item.p.staff||'—')+'</td><td>'+item.p.val+'</td><td>'+item.z+'</td><td>'+item.f.level+'</td><td>'+(item.f.rules||[]).join(', ')+'</td></tr>').join('')+'</table>';
