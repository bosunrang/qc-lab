...
const actionSuggestionsSource = read('src/presentation/nce/action-suggestions.ts');
const actionFormShellSource = read('src/presentation/nce/action-form-shell.ts');
const entryWorksheetSource = read('src/presentation/entry/entry-worksheet-html.ts');
const entryPageSource = read('src/presentation/entry/entry-page-html.ts');
const entryRangeSummarySource = read('src/presentation/entry/entry-range-summary-html.ts');
const entryChartPanelSource = read('src/presentation/entry/entry-chart-panel-html.ts');
const entryPointsPanelSource = read('src/presentation/entry/entry-points-panel-html.ts');
const entryPointTableCardSource = read('src/presentation/entry/entry-point-table-card-html.ts');
const entryVoidedPointsSource = read('src/presentation/entry/entry-voided-points-html.ts');
const entryChartCardSource = read('src/presentation/entry/entry-chart-card-html.ts');
const entryLevelHeaderSource = read('src/presentation/entry/entry-level-header-html.ts');
const entryTableWindowNoteSource = read('src/presentation/entry/entry-table-window-note-html.ts');
const entryChartStatsSource = read('src/presentation/entry/entry-chart-stats-html.ts');
const entrySheetMonthOptionsSource = read('src/presentation/entry/entry-sheet-month-options-html.ts');
const entryRangeButtonsSource = read('src/presentation/entry/entry-range-buttons-html.ts');
const entryPointRowSource = read('src/presentation/entry/entry-point-row-html.ts');
const entryVoidedPointRowSource = read('src/presentation/entry/entry-voided-point-row-html.ts');
const entrySheetRowSource = read('src/presentation/entry/entry-sheet-row-html.ts');
const entryRunCellSource = read('src/presentation/entry/entry-run-cell-html.ts');
const entryDayStatusSource = read('src/presentation/entry/entry-day-status-html.ts');
const entryDayNoteSource = read('src/presentation/entry/entry-day-note-html.ts');
const entryStaffCellSource = read('src/presentation/entry/entry-staff-cell-html.ts');
const entrySavedRunSource = read('src/presentation/entry/entry-saved-run-html.ts');
const entryEmptyRunSource = read('src/presentation/entry/entry-empty-run-html.ts');
const entryAddRunButtonSource = read('src/presentation/entry/entry-add-run-button-html.ts');
const entryRangePresetButtonSource = read('src/presentation/entry/entry-range-preset-button-html.ts');
const entryCumulativeStatsSource = read('src/presentation/entry/entry-cumulative-stats-html.ts');
const entryTreeEmptySource = read('src/presentation/entry/entry-tree-empty-html.ts');
const entryTreeMachineSource = read('src/presentation/entry/entry-tree-machine-html.ts');
const entryTreeGroupSource = read('src/presentation/entry/entry-tree-group-html.ts');
const entryTreeAssaySource = read('src/presentation/entry/entry-tree-assay-html.ts');
const entryMachineOptionsSource = read('src/presentation/entry/entry-machine-options-html.ts');
const entryTreeHeaderSource = read('src/presentation/entry/entry-tree-header-html.ts');
const entryChartLotActionSource = read('src/presentation/entry/entry-chart-lot-action-html.ts');
const entryTreeToggleButtonSource = read('src/presentation/entry/entry-tree-toggle-button-html.ts');
const entryEmptyPageSource = read('src/presentation/entry/entry-empty-page-html.ts');
const entrySheetEmptyRowSource = read('src/presentation/entry/entry-sheet-empty-row-html.ts');
const entryTreePanelIconSource = read('src/presentation/entry/entry-tree-panel-icon-html.ts');
const entryChartRangeSource = read('src/presentation/entry/entry-chart-range-source-html.ts');
const entrySheetNavigationButtonsSource = read('src/presentation/entry/entry-sheet-navigation-buttons-html.ts');
...
assert.match(actionSuggestionsSource, /export function createActionSuggestions\(/, 'NCE suggestion logic must be a typed presentation factory');
assert.doesNotMatch(actionSuggestionsSource, /\bglobalThis\b|\bdocument\b/, 'NCE suggestion logic must receive rendering dependencies explicitly');
assert.match(generated, /root\.ActionSuggestions\s*=\s*createActionSuggestions/, 'artifact must publish TypeScript NCE suggestions for the legacy bridge');
assert.match(actionFormShellSource, /export function createActionFormShell\(/, 'NCE form shell must be a typed presentation factory');
assert.doesNotMatch(actionFormShellSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'NCE form shell must receive rendering data and dependencies explicitly');
assert.match(generated, /root\.ActionFormShell\s*=\s*createActionFormShell/, 'artifact must publish TypeScript NCE form shell for the legacy bridge');
assert.match(entryWorksheetSource, /export function createEntryWorksheetHtml\(/, 'QC entry worksheet must be a typed presentation factory');
assert.doesNotMatch(entryWorksheetSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry worksheet must receive rendering data and dependencies explicitly');
assert.match(generated, /root\.entryWorksheetHtml\s*=\s*createEntryWorksheetHtml/, 'artifact must publish TypeScript QC entry worksheet for the legacy bridge');
assert.match(entryPageSource, /export function createEntryPageHtml\(/, 'QC entry page shell must be a typed presentation factory');
assert.doesNotMatch(entryPageSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry page shell must receive rendering data and dependencies explicitly');
assert.match(generated, /root\.entryPageHtml\s*=\s*createEntryPageHtml/, 'artifact must publish TypeScript QC entry page shell for the legacy bridge');
assert.match(entryRangeSummarySource, /export function createEntryRangeSummaryHtml\(/, 'QC range summary must be a typed presentation factory');
assert.doesNotMatch(entryRangeSummarySource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC range summary must receive rendering data and dependencies explicitly');
assert.match(generated, /root\.entryRangeSummaryHtml\s*=\s*createEntryRangeSummaryHtml/, 'artifact must publish TypeScript QC range summary for the legacy bridge');
assert.match(entryChartPanelSource, /export function createEntryChartPanelHtml\(/, 'QC Levey-Jennings panel must be a typed presentation factory');
assert.doesNotMatch(entryChartPanelSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC Levey-Jennings panel must receive rendering data and dependencies explicitly');
assert.match(generated, /root\.entryChartPanelHtml\s*=\s*createEntryChartPanelHtml/, 'artifact must publish TypeScript QC Levey-Jennings panel for the legacy bridge');
assert.match(entryPointsPanelSource, /export function createEntryPointsPanelHtml\(/, 'QC point-history panel must be a typed presentation factory');
assert.doesNotMatch(entryPointsPanelSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC point-history panel must receive rendering data and dependencies explicitly');
assert.match(generated, /root\.entryPointsPanelHtml\s*=\s*createEntryPointsPanelHtml/, 'artifact must publish TypeScript QC point-history panel for the legacy bridge');
assert.match(entryPointTableCardSource, /export function createEntryPointTableCardHtml\(/, 'QC point table card must be a typed presentation factory');
assert.doesNotMatch(entryPointTableCardSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC point table card must receive rendering data and dependencies explicitly');
assert.match(generated, /root\.entryPointTableCardHtml\s*=\s*createEntryPointTableCardHtml/, 'artifact must publish TypeScript QC point table card for the legacy bridge');
assert.match(entryVoidedPointsSource, /export function createEntryVoidedPointsHtml\(/, 'QC voided-points table must be a typed presentation factory');
assert.doesNotMatch(entryVoidedPointsSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC voided-points table must receive rendering data explicitly');
assert.match(generated, /root\.entryVoidedPointsHtml\s*=\s*createEntryVoidedPointsHtml/, 'artifact must publish TypeScript QC voided-points table for the legacy bridge');
assert.match(entryChartCardSource, /export function createEntryChartCardHtml\(/, 'QC Levey-Jennings card must be a typed presentation factory');
assert.doesNotMatch(entryChartCardSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC Levey-Jennings card must receive rendering data and dependencies explicitly');
assert.match(generated, /root\.entryChartCardHtml\s*=\s*createEntryChartCardHtml/, 'artifact must publish TypeScript QC Levey-Jennings card for the legacy bridge');
assert.match(entryLevelHeaderSource, /export function createEntryLevelHeaderHtml\(/, 'QC entry level header must be a typed presentation factory');
assert.doesNotMatch(entryLevelHeaderSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry level header must receive rendering data and dependencies explicitly');
assert.match(generated, /root\.entryLevelHeaderHtml\s*=\s*createEntryLevelHeaderHtml/, 'artifact must publish TypeScript QC entry level header for the legacy bridge');
assert.match(entryTableWindowNoteSource, /export function createEntryTableWindowNoteHtml\(/, 'QC entry table-window note must be a typed presentation factory');
assert.doesNotMatch(entryTableWindowNoteSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry table-window note must receive rendering data and dependencies explicitly');
assert.match(generated, /root\.entryTableWindowNoteHtml\s*=\s*createEntryTableWindowNoteHtml/, 'artifact must publish TypeScript QC entry table-window note for the legacy bridge');
assert.match(entryChartStatsSource, /export function createEntryChartStatsHtml\(/, 'QC Levey-Jennings stats strip must be a typed presentation factory');
assert.doesNotMatch(entryChartStatsSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC Levey-Jennings stats strip must receive rendering data and dependencies explicitly');
assert.match(generated, /root\.entryChartStatsHtml\s*=\s*createEntryChartStatsHtml/, 'artifact must publish TypeScript QC Levey-Jennings stats strip for the legacy bridge');
assert.match(entrySheetMonthOptionsSource, /export function createEntrySheetMonthOptionsHtml\(/, 'QC entry month options must be a typed presentation factory');
assert.doesNotMatch(entrySheetMonthOptionsSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry month options must receive rendering data explicitly');
assert.match(generated, /root\.entrySheetMonthOptionsHtml\s*=\s*createEntrySheetMonthOptionsHtml/, 'artifact must publish TypeScript QC entry month options for the legacy bridge');
assert.match(entryRangeButtonsSource, /export function createEntryRangeButtonsHtml\(/, 'QC entry range buttons must be a typed presentation factory');
assert.doesNotMatch(entryRangeButtonsSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry range buttons must receive rendering data explicitly');
assert.match(generated, /root\.entryRangeButtonsHtml\s*=\s*createEntryRangeButtonsHtml/, 'artifact must publish TypeScript QC entry range buttons for the legacy bridge');
assert.match(entryPointRowSource, /export function createEntryPointRowHtml\(/, 'QC point-history row must be a typed presentation factory');
assert.doesNotMatch(entryPointRowSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC point-history row must receive rendering data explicitly');
assert.match(generated, /root\.entryPointRowHtml\s*=\s*createEntryPointRowHtml/, 'artifact must publish TypeScript QC point-history row for the legacy bridge');
assert.match(entryVoidedPointRowSource, /export function createEntryVoidedPointRowHtml\(/, 'QC voided-point audit row must be a typed presentation factory');
assert.doesNotMatch(entryVoidedPointRowSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC voided-point audit row must receive rendering data explicitly');
assert.match(generated, /root\.entryVoidedPointRowHtml\s*=\s*createEntryVoidedPointRowHtml/, 'artifact must publish TypeScript QC voided-point audit row for the legacy bridge');
assert.match(entrySheetRowSource, /export function createEntrySheetRowHtml\(/, 'QC entry sheet row must be a typed presentation factory');
assert.doesNotMatch(entrySheetRowSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry sheet row must receive rendering data explicitly');
assert.match(generated, /root\.entrySheetRowHtml\s*=\s*createEntrySheetRowHtml/, 'artifact must publish TypeScript QC entry sheet row for the legacy bridge');
assert.match(entryRunCellSource, /export function createEntryRunCellHtml\(/, 'QC entry run cell must be a typed presentation factory');
assert.doesNotMatch(entryRunCellSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry run cell must receive rendering data explicitly');
assert.match(generated, /root\.entryRunCellHtml\s*=\s*createEntryRunCellHtml/, 'artifact must publish TypeScript QC entry run cell for the legacy bridge');
assert.match(entryDayStatusSource, /export function entryDayStatusHtml\(/, 'QC entry day status must be a typed presentation helper');
assert.doesNotMatch(entryDayStatusSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry day status must receive rendering data explicitly');
assert.match(generated, /root\.entryDayStatusHtml\s*=\s*entryDayStatusHtml/, 'artifact must publish TypeScript QC entry day status for the legacy bridge');
assert.match(entryDayNoteSource, /export function createEntryDayNoteHtml\(/, 'QC entry day note must be a typed presentation factory');
assert.doesNotMatch(entryDayNoteSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry day note must receive rendering data explicitly');
assert.match(generated, /root\.entryDayNoteHtml\s*=\s*createEntryDayNoteHtml/, 'artifact must publish TypeScript QC entry day note for the legacy bridge');
assert.match(entryStaffCellSource, /export function createEntryStaffCellHtml\(/, 'QC entry staff cell must be a typed presentation factory');
assert.doesNotMatch(entryStaffCellSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry staff cell must receive rendering data explicitly');
assert.match(generated, /root\.entryStaffCellHtml\s*=\s*createEntryStaffCellHtml/, 'artifact must publish TypeScript QC entry staff cell for the legacy bridge');
assert.match(entrySavedRunSource, /export function createEntrySavedRunHtml\(/, 'QC saved run slot must be a typed presentation factory');
assert.doesNotMatch(entrySavedRunSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC saved run slot must receive rendering data explicitly');
assert.match(generated, /root\.entrySavedRunHtml\s*=\s*createEntrySavedRunHtml/, 'artifact must publish TypeScript QC saved run slot for the legacy bridge');
assert.match(entryEmptyRunSource, /export function createEntryEmptyRunHtml\(/, 'QC empty run slot must be a typed presentation factory');
assert.doesNotMatch(entryEmptyRunSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC empty run slot must receive rendering data explicitly');
assert.match(generated, /root\.entryEmptyRunHtml\s*=\s*createEntryEmptyRunHtml/, 'artifact must publish TypeScript QC empty run slot for the legacy bridge');
assert.match(entryAddRunButtonSource, /export function createEntryAddRunButtonHtml\(/, 'QC add-run button must be a typed presentation factory');
assert.doesNotMatch(entryAddRunButtonSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC add-run button must receive rendering data explicitly');
  assert.match(generated, /root\.entryAddRunButtonHtml\s*=\s*createEntryAddRunButtonHtml/, 'artifact must publish TypeScript QC add-run button for the legacy bridge');
  assert.match(entryRangePresetButtonSource, /export function createEntryRangePresetButtonHtml\(/, 'QC range preset button must be a typed presentation factory');
  assert.doesNotMatch(entryRangePresetButtonSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC range preset button must receive rendering data explicitly');
  assert.match(generated, /root\.entryRangePresetButtonHtml\s*=\s*createEntryRangePresetButtonHtml/, 'artifact must publish TypeScript QC range preset button for the legacy bridge');
  assert.match(entryCumulativeStatsSource, /export function createEntryCumulativeStatsHtml\(/, 'QC cumulative stats must be a typed presentation factory');
  assert.doesNotMatch(entryCumulativeStatsSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC cumulative stats must receive rendering data explicitly');
  assert.match(generated, /root\.entryCumulativeStatsHtml\s*=\s*createEntryCumulativeStatsHtml/, 'artifact must publish TypeScript QC cumulative stats for the legacy bridge');
  assert.match(entryTreeEmptySource, /export function entryTreeEmptyHtml\(/, 'QC entry empty tree must be a typed presentation helper');
  assert.doesNotMatch(entryTreeEmptySource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry empty tree must not read global rendering state');
  assert.match(generated, /root\.entryTreeEmptyHtml\s*=\s*entryTreeEmptyHtml/, 'artifact must publish TypeScript QC entry empty tree for the legacy bridge');
  assert.match(entryTreeMachineSource, /export function createEntryTreeMachineHtml\(/, 'QC entry machine node must be a typed presentation factory');
  assert.doesNotMatch(entryTreeMachineSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry machine node must receive rendering data explicitly');
  assert.match(generated, /root\.entryTreeMachineHtml\s*=\s*createEntryTreeMachineHtml/, 'artifact must publish TypeScript QC entry machine node for the legacy bridge');
  assert.match(entryTreeGroupSource, /export function createEntryTreeGroupHtml\(/, 'QC entry lot-group node must be a typed presentation factory');
  assert.doesNotMatch(entryTreeGroupSource, /\bglobalThis\b|\bdocument\b/, 'QC entry lot-group node must receive rendering data explicitly');
  assert.match(generated, /root\.entryTreeGroupHtml\s*=\s*createEntryTreeGroupHtml/, 'artifact must publish TypeScript QC entry lot-group node for the legacy bridge');
  assert.match(entryTreeAssaySource, /export function createEntryTreeAssayHtml\(/, 'QC entry assay node must be a typed presentation factory');
  assert.doesNotMatch(entryTreeAssaySource, /\bglobalThis\b|\bdocument\b/, 'QC entry assay node must receive rendering data explicitly');
  assert.match(generated, /root\.entryTreeAssayHtml\s*=\s*createEntryTreeAssayHtml/, 'artifact must publish TypeScript QC entry assay node for the legacy bridge');
  assert.match(entryMachineOptionsSource, /export function createEntryMachineOptionsHtml\(/, 'QC entry machine options must be a typed presentation factory');
  assert.doesNotMatch(entryMachineOptionsSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry machine options must receive rendering data explicitly');
  assert.match(generated, /root\.entryMachineOptionsHtml\s*=\s*createEntryMachineOptionsHtml/, 'artifact must publish TypeScript QC entry machine options for the legacy bridge');
  assert.match(entryTreeHeaderSource, /export function createEntryTreeHeaderHtml\(/, 'QC entry tree header must be a typed presentation factory');
  assert.doesNotMatch(entryTreeHeaderSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry tree header must receive rendering data explicitly');
  assert.match(generated, /root\.entryTreeHeaderHtml\s*=\s*createEntryTreeHeaderHtml/, 'artifact must publish TypeScript QC entry tree header for the legacy bridge');
  assert.match(entryChartLotActionSource, /export function createEntryChartLotActionHtml\(/, 'QC entry chart lot action must be a typed presentation factory');
  assert.doesNotMatch(entryChartLotActionSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry chart lot action must receive rendering data explicitly');
  assert.match(generated, /root\.entryChartLotActionHtml\s*=\s*createEntryChartLotActionHtml/, 'artifact must publish TypeScript QC entry chart lot action for the legacy bridge');
  assert.match(entryTreeToggleButtonSource, /export function createEntryTreeToggleButtonHtml\(/, 'QC entry tree toggle button must be a typed presentation factory');
  assert.doesNotMatch(entryTreeToggleButtonSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry tree toggle button must receive rendering data explicitly');
  assert.match(generated, /root\.entryTreeToggleButtonHtml\s*=\s*createEntryTreeToggleButtonHtml/, 'artifact must publish TypeScript QC entry tree toggle button for the legacy bridge');
  assert.match(entryEmptyPageSource, /export function createEntryEmptyPageHtml\(/, 'QC entry empty page must be a typed presentation factory');
  assert.doesNotMatch(entryEmptyPageSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry empty page must receive rendering data explicitly');
  assert.match(generated, /root\.entryEmptyPageHtml\s*=\s*createEntryEmptyPageHtml/, 'artifact must publish TypeScript QC entry empty page for the legacy bridge');
  assert.match(entrySheetEmptyRowSource, /export function entrySheetEmptyRowHtml\(/, 'QC entry empty sheet row must be a typed presentation helper');
  assert.doesNotMatch(entrySheetEmptyRowSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry empty sheet row must receive rendering data explicitly');
  assert.match(generated, /root\.entrySheetEmptyRowHtml\s*=\s*entrySheetEmptyRowHtml/, 'artifact must publish TypeScript QC entry empty sheet row for the legacy bridge');
  assert.match(entryTreePanelIconSource, /export function entryTreePanelIconHtml\(/, 'QC entry tree icon must be a typed presentation helper');
  assert.doesNotMatch(entryTreePanelIconSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry tree icon must not read global rendering state');
  assert.match(generated, /root\.entryTreePanelIconHtml\s*=\s*entryTreePanelIconHtml/, 'artifact must publish TypeScript QC entry tree icon for the legacy bridge');
  assert.match(entryChartRangeSource, /export function entryChartRangeSourceHtml\(/, 'QC entry chart range source must be a typed presentation helper');
  assert.doesNotMatch(entryChartRangeSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry chart range source must not read global rendering state');
  assert.match(generated, /root\.entryChartRangeSourceHtml\s*=\s*entryChartRangeSourceHtml/, 'artifact must publish TypeScript QC entry chart range source for the legacy bridge');
  assert.match(entrySheetNavigationButtonsSource, /export function createEntrySheetNavigationButtonsHtml\(/, 'QC entry sheet navigation buttons must be a typed presentation factory');
  assert.doesNotMatch(entrySheetNavigationButtonsSource, /\bglobalThis\b|\bdocument\b|\bstate\b/, 'QC entry sheet navigation buttons must receive rendering data explicitly');
  assert.match(generated, /root\.entrySheetNavigationButtonsHtml\s*=\s*createEntrySheetNavigationButtonsHtml/, 'artifact must publish TypeScript QC entry sheet navigation buttons for the legacy bridge');
