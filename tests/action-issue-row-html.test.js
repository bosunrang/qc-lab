...
const program = `
  ...
  const render = createActionIssueRowHtml({ escape: value => String(value).replaceAll('<', '&lt;'), quote: value => String(value).replaceAll("'", "\\\\'"), button: (label, action, variant) => '[' + label + '|' + (typeof action === 'string' ? action : JSON.stringify(action)) + '|' + variant + ']' });
...
assert.match(create, /Lập hồ sơ\|\{"action":"beginActionFromIssue","args":\["T'1",2,"1'3s","SE","Kiểm tra","P'1","2026-08-12"\]\}\|ghost sm/);
assert.match(resume, /Tiếp tục hồ sơ\|\{"action":"editAction","args":\[3\]\}\|ghost sm/);
