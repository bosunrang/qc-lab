...
assert.match(form,/deps\.ActionBiasService\.info\(t, l, biasBeforeRaw, biasAfterRaw\)/,'Đánh giá Bias NCE phải dùng service TypeScript');
assert.match(form,/deps\.ActionBiasService\.latestSigmaBias\(t, level, state\(\)\.sigmaData\)/,'Bias Sigma NCE phải dùng service TypeScript');
assert.match(form,/deps\.ActionBiasPresentation\.thresholdHtml\(info\)/,'Ngưỡng Bias NCE phải dùng presentation TypeScript');
