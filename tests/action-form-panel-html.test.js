...
const legacy=fs.readFileSync(path.join(root,'assets','modules','action-form.js'),'utf8');
...
assert.match(legacy,/return globalThis\.actionFormPanelPresentation\(\{editing,formOpen,guideButtonHtml:btn\('Quy trình 8 bước','openActionGuide\(\)','ghost sm'\),closedHtml:actionFormClosedHtml\(issueCount\),formBodyHtml\}\)/,'route NCE phải gọi renderer TypeScript');
