...
const ctx = loadSandbox(['core.js', 'generated/modular-pilot.js', 'modules/state.js', 'modules/qc-domain.js']);
...
assert.match(rangeWorkflowSource, /class="range-workflow-comparison"/, 'bảng so sánh dải QC phải có bố cục cột riêng');
...
const appliedTarget = JSON.parse(JSON.stringify(run(ctx, `(function(){const l={mean:135,sd:2.5,low:130,high:140,rangeK:2,applied:'mfg'};qcRangeCandidateService.assignTarget(l,140.053846154,0.54379086239,'lab');return l;})()`)));
...
const restoredTarget = JSON.parse(JSON.stringify(run(ctx, `(function(){const l={mean:140.05,sd:0.54,low:138.97,high:141.13,rangeK:2,applied:'lab'};qcRangeCandidateService.assignTarget(l,140,2.5,'mfg');return l;})()`)));
