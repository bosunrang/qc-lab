...
assert.match(components, /\.modal-b\{padding:var\(--panel-content-gap\) var\(--modal-inline-padding\);overflow:auto\}/,'modal và panel phải dùng cùng token khoảng cách header–nội dung');
...
assert.match(components, /\.sg-chart-box > h3\{[^}]*min-height:var\(--subpanel-header-min-height\)[^}]*padding:var\(--space-sm\) var\(--space-lg\)/,'tiêu đề biểu đồ Sigma phải dùng header có padding chuẩn');
