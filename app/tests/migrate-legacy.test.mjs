...
// sang hình dạng cột SQLite app — không cần DB, dùng 1 state tổng hợp đủ
...
const { parseLegacyBackupEnvelope, mapLegacyStateToTables, summarizeMappedTables } = require('../../app-dist/main/domain/migrate-legacy.js');
const { auditEntryHash, verifyAuditChain } = require('../../app-dist/main/domain/audit-chain.js');
...
// 2) format sai (vd backup app 'qclab-v2-backup') phải bị chặn rõ ràng
...
// 4) Ánh xạ đúng hình dạng cột app cho từng bảng
...
console.log('app migrate-legacy oracle tests passed');
