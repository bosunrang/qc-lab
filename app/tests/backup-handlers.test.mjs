// Giai đoạn C3: xuất/phục hồi toàn bộ dữ liệu app. Kiểm chứng END-TO-END
...
const wrongFormat = wrongFormatTarget.importBackup({ data: { json: JSON.stringify({ format: 'unsupported-backup', schemaVersion: 1, data: {}, checksum: 'x' }) } }, actor);
...
futureSchema.checksum = require('../../app-dist/main/domain/backup.js').computeChecksum(JSON.stringify(futureSchema.data));
...
// Ho so don vi tro ve mac dinh cua schema (khop blankAppState() hệ thống)
