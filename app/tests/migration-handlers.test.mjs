...
// ghi DB, import ghi thật + xoá sạch dữ liệu app cũ (nếu có) + tạo bản
...
const { createMigrationHandlers } = require('../../app-dist/main/ipc/migration-handlers.js');
...
const { verifyAuditChain, auditEntryHash } = require('../../app-dist/main/domain/audit-chain.js');
...
// 6) format sai (backup app that su, khong phai app cu) phai bi tu choi ro rang
...
console.log('app migration-handlers end-to-end tests passed');
