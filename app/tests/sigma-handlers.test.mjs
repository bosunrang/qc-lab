...
// của `savePeriod()` chặn mọi mức chưa có trong `test_levels`.
...
// Mỗi vòng EQA bắt buộc có cặp kết quả PXN và Target; không nhận Bias% rời.
const biasOnlyEqa = sigmaHandlers.savePeriod({ testId: test.id, period: '2026-12', tea: 15, levels: [{ level: 1, cv: 3, eqaRounds: [-2, 2] }] }, actor);
assert.equal(biasOnlyEqa.ok, false);
assert.equal(biasOnlyEqa.error.code, 'invalid-eqa-round');
