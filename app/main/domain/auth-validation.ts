...
import { initialsFromName } from './name-initials';
...
export function validateUserCreate(input: UserCreateInput, existingUsernames: readonly string[]): ValidationResult<PreparedUserCreate> {
  ...
  return { ok: true, data: { username, name, initials: initialsFromName(name), role, password, pagePerms: perms.data } };
}

export interface UserUpdateInput { name?: unknown; role?: unknown; active?: unknown; initials?: unknown; pagePerms?: unknown }
/** `pagePerms` là `undefined` khi request KHÔNG gửi field đó — nghĩa là GIỮ
 * NGUYÊN giá trị đang có. Mã viết tắt luôn được tính lại từ họ tên, không tin
 * giá trị do renderer gửi lên. */
...
export function validateUserUpdate(input: UserUpdateInput): ValidationResult<PreparedUserUpdate> {
  ...
  if (!name) return { ok: false, code: 'missing-name', message: 'Nhập họ tên.' };
  const role = cleanRole(input.role);
  const data: PreparedUserUpdate = { name, role, active: input.active !== false, initials: initialsFromName(name) };
