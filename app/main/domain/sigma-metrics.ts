...
export function uncertaintyBudget(input: UncertaintyBudgetInput): UncertaintyBudgetResult | null {
  ...
  return {
    // `uCref` chỉ đi vào ngân sách QUA nhánh `u(bias)`; chưa có bias thì nó
    // không đóng góp gì vào `u_c`. Trả nguyên giá trị như bản trước làm bảng
    // MU in ra một con số u(Cref) trông như đang được tính. Số ĐÃ NHẬP vẫn
    // nằm ở `SigmaLevelResult.uCref` cho phần hồ sơ/xuất file.
    k, uRw, uBias, uCal, bias: includeBias ? bias : null, uCref: uBias != null ? uCref : null, includeBias,
