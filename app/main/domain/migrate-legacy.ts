...
// [points]}`, `qcLots[]`, ...) sang schema SQLite quan hệ của app. KHÁC
...
//
// Hồ sơ NCE (`actions[]`) là phần LỆCH HÌNH DẠNG nhiều nhất: app's NCE
...
// vào cột thật + `detail_json`'s 4 trường app UI đọc được
...
// UI hiện tại chưa hiển thị hết, để lần sau app's NCE UI được làm đầy đủ
...
import { normalizeErrorClass } from './westgard-rules';
...
export function parseLegacyBackupEnvelope(raw: unknown): ValidationResult<{ data: LegacyState; meta: Record<string, unknown> }> {
  ...
  if (env.format !== LEGACY_BACKUP_FORMAT) {
    return { ok: false, code: 'wrong-format', message: `File này không phải backup app CŨ (format='${LEGACY_BACKUP_FORMAT}'). Nếu đây là backup app ('qclab-v2-backup'), dùng mục "Sao lưu & phục hồi" thay vì "Di trú dữ liệu từ app cũ".` };
  }
...
/** Ánh xạ THUẦN — trả về đúng hình dạng cột SQLite của app cho từng bảng,
...
export function mapLegacyStateToTables(legacy: LegacyState): MappedTables {
  ...
  const tests = arr(legacy.tests).map((t) => ({
    ...
    // App cũ không có "section" theo TỪNG xét nghiệm (chỉ có ở máy) — app
    // thêm trường này để tự điền Khoa/Khu vực; suy từ section của máy đang
    // gắn, 1 lần khi di trú (khớp đúng hành vi auto-fill hiện có của app
  ...
  // phải lịch sử: đó là số nhập sẵn cho một lô chưa dùng. app có bảng riêng
  ...
  arr(legacy.tests).forEach((t) => {
    ...
    arr(t.levels).forEach((l) => {
      ...
      // app, chỉ chốt giá trị CŨ) — soi đúng mốc đó lấy effectiveFrom
  ...
  // Hồ sơ NCE — xem chú thích đầu file: chỉ những trường app hiện dùng
  ...
  const actions = arr(legacy.actions).map((a) => {
    ...
    return {
      ...
      error_type: normalizeErrorClass(a.errorType), qc_verdict: ['warn', 'rej', 'invalid'].includes(a.qcVerdict) ? a.qcVerdict : '',
  ...
  // prevHash='' — phải mang đúng anchor đó sang `app_meta` (app đã có sẵn
