...
`app-v2/` là bản viết lại QC Lab từ đầu bằng kiến trúc khác hẳn phần còn lại
của repo — KHÔNG phải bước tiếp theo của "Giai đoạn 9" (xem "Kernel / gỡ
global bridge" bên dưới). Giai đoạn 9 tiến hoá app cũ (`index.html`/`assets/`/
`src/`) tại chỗ; `app-v2/` là một sản phẩm khác, không đọc/ghi chung dữ liệu,
không chia sẻ code với `src/`/`assets/`. Bắt đầu 2026-08-31, chưa nằm trong
`build.files` — chạy/test bằng các script `app-v2:*` riêng.

- **`docs/APP-V2-PLAN.md` là kế hoạch + trạng thái hiện hành**, tổ chức theo
  TỪNG THẺ (11 thẻ). Đọc file đó trước khi làm bất cứ việc gì trong `app-v2/`.
- `docs/archive/APP-V2-LOG-2026-08-to-09.md` là nhật ký chi tiết 08–09/2026 đã
  tách khỏi file này (2026-09-11). Đó là LỊCH SỬ, không phải chỉ dẫn: nó còn
  chứa quyết định "giao diện lấy app cũ làm golden master" đã bị huỷ.

### Hai nguyên tắc thay thế mọi quyết định cũ (chốt 2026-09-11)

**1. Giao diện: app cũ CHỈ là tham khảo, app-v2 viết code mới.**
Người dùng sở hữu giao diện app-v2 và đã tự cải tiến nhiều chỗ (chiều cao
header, màu thẻ header, tab Lịch sử dữ liệu, nút thao tác dạng icon, một xét
nghiệm gán nhiều máy…). Từ đây:

- **KHÔNG được sửa giao diện app-v2 cho giống app cũ.** Lệch với app cũ không
  phải lỗi và không phải việc cần "rà soát rồi chỉnh lại".
- Khi cần dựng một màn hình chưa có, được XEM app cũ để hiểu luồng thao tác và
  các thông tin cần hiển thị, rồi **viết code mới** theo hệ thống hiện có của
...
| Gate | Trạng thái |
|---|---|
| `app-v2:style-parity` | **ĐÃ BỎ** — nó so computed style với app cũ, tức khẳng định "khác app cũ = sai". |
| `app-v2:ui-parity` | **CHỈ BÁO CÁO**, không chặn. Dùng để phát hiện một màn hình thiếu hẳn thông tin, không phải để bắt giống app cũ. Đừng sửa app-v2 chỉ vì con số của nó khác 0. |
...
3. App cũ — dùng để PHÁT HIỆN lệch
   ...
   Đây là bộ dò TRÔI KHI PORT, không phải oracle; nó sẽ bị xoá cùng lúc với
   app cũ.

Khi đã quyết một mục nghiệp vụ (kể cả quyết "để nguyên"), **đừng mở lại** ở
lượt sau chỉ vì app cũ làm khác.

### Kiến trúc (không tranh luận lại trừ khi có lý do kỹ thuật mới)

Electron 2 tiến trình thật: `app-v2/main/` (main process — `node:sqlite` là
nguồn dữ liệu thật, `ipcMain.handle` theo named channel `<module>:<action>`,
`preload.ts` lộ đúng các hàm đó qua `contextBridge` dưới `window.qcApi`) và
`app-v2/renderer/` (React 19 + `react-router-dom` `HashRouter` — bắt buộc hash
vì app đóng gói `file://` — + Zustand store thật theo `set()`/`get()` chuẩn,
mỗi trang 1 store).

- **`app-v2/shared/qc-api.d.ts` là NGUỒN DUY NHẤT của hợp đồng IPC.** Main
...
- **Schema SQLite** (`main/db/schema.ts`, `applySchema()` idempotent) thiết kế
  ...
  `main/domain/migrate-legacy.ts` — `table-io.ts`'s `restoreAllTables()` bind
  `null` cho field vắng mặt, không để SQLite dùng `DEFAULT`.
- **Mỗi IPC handler cùng khuôn**: input thô → validate thuần trong
  `main/domain/*-validation.ts` (không đụng DB) → transaction → `writeAudit()`
  (chuỗi hash tamper-evident, `main/domain/audit-chain.ts`) → `IpcResult<T>`.
  Tiện ích dùng chung ở `main/ipc/shared.ts` (`Actor`/`IpcResult`/`writeAudit`/
  `requireWrite`/`requireAdmin`/`notifyChanged`).
- **Cổng quyền là câu lệnh ĐẦU TIÊN của handler**, trước cả validate lẫn
  `writeAudit()` — sai thứ tự thì mỗi lần một người không có quyền bấm nút sẽ
  đẻ một dòng audit rác. Không ném exception; trả `{ok:false,error}` để
  renderer hiện được thông báo tiếng Việt.
- **Handler không import handler khác** — mỗi file tự SQL, chỉ dùng chung hàm
  thuần ở `main/domain/` hoặc module dữ liệu dùng chung ở `main/db/`. Ngoại lệ
  DUY NHẤT có chủ đích: `lis-handlers.ts` import `createEntryHandlers` để điểm
  QC từ LIS đi CÙNG đường `addPoint()` với nhập tay (kỳ khoá + audit).
- **Module dữ liệu dùng chung ở `main/db/`** cho những tập hợp mà nhiều
  handler PHẢI hiểu giống hệt nhau — nhân bản SQL ở đây là nguồn của lớp lỗi
  "hai màn hình nói hai chuyện": `operational-levels.ts` (mức QC đang vận
  hành), `lot-lineage.ts` (chuỗi lô cũ), `table-io.ts` (backup/di trú).
- **Không xoá thật QC** — chỉ `voided` (soft-delete).
- `notifyChanged()` phát `store:changed` tới renderer; `writeAudit()` tự gọi
  `notifyChanged(['activity'])`, mỗi handler chỉ thêm bảng của riêng nó.
  Renderer dùng `renderer/lib/useStoreInvalidation.ts`.
- **Trang gọi `window.qcApi` thẳng là ngoại lệ, không phải mặc định** — mọi
  thao tác GHI và mọi dữ liệu cần tự cập nhật phải đi qua store. 8 chỗ còn gọi
  thẳng đều có ghi chú lý do ngay tại dòng đó; đừng "dọn" tiếp cho đủ 0.

### Chạy và kiểm thử

- `npm run app-v2:test` (`app-v2/scripts/run-tests.cjs`) tự build main process
  (CommonJS) + bản mock sang `app-v2-dist/` rồi chạy
  `node --test app-v2/tests/*.test.mjs`. **BẮT BUỘC build trước** vì phần lớn
  module `main/` import chéo nhau và Node ESM type-stripping không resolve
  được import không đuôi file. Chỉ module KHÔNG import chéo (vd
  `westgard-rules.ts`, `page-roles.ts`, `sigma-tea-core.ts`) mới test thẳng
  trên `.ts` qua ESM.
- `npm run app-v2:typecheck` chạy 2 lượt `tsc` riêng (main CommonJS, renderer
  JSX) vì 2 tiến trình khác target/module.
- `npm run app-v2:dev` (Vite, cổng 5174) mở renderer qua trình duyệt: từ
  2026-09-09 bản xem trước chạy **CHÍNH handler thật** trên SQLite thật
  (sql.js/WASM trong IndexedDB) qua `renderer/browser-mock/real-api.ts` +
  `sqlite-shim.ts`, không còn bản giả lập viết tay. 3 primitive có bản song
  song (`sha256`, `password-hash` 20k vòng thay 600k, `db-file-size`) được
  hoán đổi bằng plugin `swap-node-only-modules-for-browser`; plugin đó phải
  giữ `enforce:'pre'` và khớp theo TÊN MODULE, không theo chuỗi import.
- Verify thật quan trọng hơn gate: chạy trong cửa sổ Electron thật (Playwright
  `_electron`) hoặc trong tab `app-v2:dev`, và ĐỌC LẠI dữ liệu từ nguồn
  (`window.qcApi`/SQLite) thay vì tin thông báo thành công.

### Bẫy đã trả giá — đừng lặp lại

- **Gate phục vụ `app-v2-dist/renderer` (bản ĐÃ BUILD).** Sửa nguồn rồi đo
  ngay thì số không đổi; phải `npm run app-v2:build` trước mỗi lượt đo.
- **Trang có state chọn phải TỰ CHỌN mục đầu tiên** khi lựa chọn hiện tại
  không hợp lệ. Quên 1 dòng `useEffect` làm cả trang chỉ render vỏ — đã xảy ra
  5 lần (Entry, Sigma, Westgard, Reagent, modal Panel QC).
- **Seed của một màn hình chưa từng đo phải có dữ liệu cho màn hình đó** —
  "0 lệch" của một trang trống là con số vô nghĩa (đã xảy ra 5 lần). Và mọi
  field của bản v2 phải SUY RA TỪ dữ liệu app cũ, không tự bịa giá trị "cho
  đẹp", nếu không gate báo lỗi oan cho code.
- **Ô uncontrolled (`defaultValue`/`defaultChecked`) cần `key` đổi theo dữ
  liệu.** React tái dùng DOM node cũ nên đổi mức/lô/bản ghi mà `key` không đổi
  sẽ giữ nguyên giá trị cũ trên màn hình. Đây là lớp lỗi tái diễn nhiều nhất
  ở renderer.
- **Ma trận Mean/SD đọc/ghi THẲNG DOM của cả hàng** (`closest('tr')`), không
  tính từ React state: 4 ô có thể lần lượt mất focus rất nhanh khi Tab qua,
  tính từ state đã render sẽ ghi đè bằng giá trị cũ.
- **Thiếu wiring là lớp lỗi nặng nhất của app này**: cột có trong schema mà
  không handler nào ghi (`void_kind`, `low`/`high`, `qc_mean`/`qc_sd`), nút có
  trong JSX mà không có `onClick`, hàm có trong domain mà không IPC nào gọi.
  Khi thêm một trường, đi hết chuỗi schema → validate → SQL → `qc-api.d.ts` →
  `preload.ts` → store → UI và kiểm lại từng mắt xích.
- **Không dựng control chết.** Nút/ô nhìn thấy phải có hành vi thật; tính năng
  backend chưa có thì làm trước hoặc không dựng nút.
- **Sửa hàng loạt bằng script thì GREP LẠI để xác nhận đã ghi ra đĩa** — đừng
  tin số đếm script tự in; và kiểm cả ranh giới chuỗi khi `replace` trong CSS
  (một chuỗi có thể là PHẦN của selector dài hơn).
- **Khi đối chiếu hai bản dùng hai quy ước biểu diễn khác nhau cho "không có
  giá trị", phải chuẩn hoá ĐỐI XỨNG cả hai phía** rồi chốt riêng khác biệt
  hợp đồng — chuẩn hoá một phía là đang so hai thứ khác nhau.
- **Đừng hard-code ngày trong test đối chiếu** với code đọc đồng hồ thật.
