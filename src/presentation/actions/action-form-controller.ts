...
export function createActionFormController(deps: {
  ...
  const actionSuggestRow = (targetId: string, phrases: string[]): string => {
    ...
    return deps.pres.actionSuggestRowPresentation(deps.escapeAttr(targetId), deps.jsq(targetId), phrases.map(p => ({ phraseHtml: deps.esc(p), phraseJs: deps.jsq(p) })));
  };
  ...
  const actionFormClosedHtml = (issueCount: number) => {
    const manual = deps.canWrite() ? deps.btn('Lập hồ sơ từ nguồn khác', 'beginActionManual()', 'ghost') : '';
    return issueCount
  ...
  const actionSection = (key: string, badge: string, title: string, hint: string, bodyHtml: string, chipInfo: AnyRec, openSet: Set<string>) => {
    ...
    return deps.pres.actionFormSectionPresentation({ keyHtml: deps.escapeAttr(key), keyJs: deps.jsq(key), open, badgeHtml: deps.esc(badge), titleHtml: deps.esc(title), hintHtml: deps.esc(hint), chipHtml: chip, bodyHtml });
  };
