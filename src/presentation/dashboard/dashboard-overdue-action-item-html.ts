...
export function createDashboardOverdueActionItemHtml(deps: {
  ...
  return (input: { action: Action; index: number; info: { label: string }; test?: Record<string, any> }) => {
    ...
    return `<div class="shift-item rej"><div><b>${deps.escape(input.action.nceId || 'Hồ sơ khắc phục')} · ${title}</b><div class="meta">${deps.escape(input.info.label)} · hạn ${deps.date(input.action.dueDate)} · phụ trách ${deps.escape(input.action.by || '—')}</div></div>${deps.button('Tiếp tục hồ sơ', { action: 'dashboardContinueAction', args: [input.index] }, 'ghost sm')}</div>`;
  };
}
