export function createReportLockPanelHtml(deps: {
  button: (label: string, action: string | { action: string; args?: unknown[] } | null, variant: string, title?: string, options?: unknown) => string;
}) {
  return (input: { isAdmin: boolean; year: number; month: number; months: number[]; years: number[]; already: boolean; lockListHtml: string }) => {
    ...
    const action = input.isAdmin
      ? (input.already ? deps.button('Kỳ này đã khóa', '', 'ghost', '', { disabled: true }) : deps.button('Khóa kỳ này', { action: 'reportLockPeriod' }, 'teal'))
