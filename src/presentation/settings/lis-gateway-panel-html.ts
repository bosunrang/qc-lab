export function createLisGatewayPanelHtml(deps: { escape: (value: unknown) => string; escapeAttribute: (value: unknown) => string; button: (label: string, action: string | { action: string; args?: unknown[] } | null, variant: string) => string }) {
