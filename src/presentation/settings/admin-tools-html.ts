export function createAdminToolsHtml(button: (label: string, action: string | { action: string; args?: unknown[] } | null, variant: string) => string) {
  return (backupStatus: unknown, backupCapacity: unknown): string => `<div class="panel"><h2 class="panel-title">Quản trị dữ liệu</h2>
     <div class="admin-tools">
        ...
        <div class="admin-tool"><b>Nhập backup</b><span>Khôi phục dữ liệu từ file backup đã xuất. Chỉ quản trị viên được nhập.</span>${button('Chọn file backup', { action: 'clickElementById', args: ['imp'] }, 'ghost')}<input id="imp" type="file" accept="application/json" style="display:none" data-action="importData" data-action-on="change"></div>
        <div class="admin-tool"><b>Kiểm tra backup</b><span>Kiểm tra checksum, cấu trúc và số điểm — không ảnh hưởng dữ liệu đang dùng.</span>${button('Chọn file để kiểm tra', { action: 'clickElementById', args: ['verifyBackup'] }, 'ghost')}<input id="verifyBackup" type="file" accept="application/json" style="display:none" data-action="verifyBackupFile" data-action-on="change"></div>
