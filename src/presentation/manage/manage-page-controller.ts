...
export function createManagePageController(deps: {
  ...
  const manageToolbar = (title: string, sub: string, action?: string | { action: string; args?: unknown[] } | null, label?: string) => {
  ...
  const manageLots = () => {
    ...
    const groupRows = state().lotGroups.filter((g: AnyRec) => manageMatch([g.name, g.note, ...(g.lotIds || []).map((id: unknown) => (state().qcLots.find((l: AnyRec) => l.id === id) || {}).lotNo)])).map((g: AnyRec) => {
      ...
      const toggle = deps.pres.lotGroupToggleActionPresentation(archived, g.status, inUse), toggleBtn = toggle ? deps.btn(toggle.label, { action: toggle.command === 'activate' ? 'activateLotGroup' : 'toggleLotGroupStatus', args: [g.id] }, toggle.variant) : '';
      const lotsHtml = deps.pres.lotGroupLotPillsHtml(lots.map((l: AnyRec) => ({ lotNo: deps.esc(l.lotNo), level: l.level }))), actionsHtml = deps.btn('Sửa nhóm', { action: 'openConfigGroup', args: [g.id] }, 'ghost sm') + deps.btn('Mean/SD', { action: 'openTargetMatrix', args: ['', g.id] }, 'ghost sm') + toggleBtn + deps.btn('Xóa', { action: 'deleteConfigGroup', args: [g.id] }, 'danger sm'), model = { archived, name: g.name, note: g.note, status: statusTag, lotsHtml, actionsHtml };
      return deps.pres.manageLotGroupCardPresentation(model);
    }).join('');
    return manageToolbar('Lô & Nhóm QC', 'Quản lý từng lô và nhóm lô QC.') + deps.pres.manageLotConfigLayoutPresentation({ lotAddButtonHtml: deps.btn('Thêm lô QC', { action: 'openConfigLot' }, 'teal sm'), lotRowsHtml: rows, lotEmptyHtml: deps.emptyState('Chưa có lô QC', 'Tạo từng lô QC độc lập, sau đó nhập Mean/SD cho Panel QC.'), groupAddButtonHtml: deps.btn('Thêm nhóm lô', { action: 'openConfigGroup' }, 'teal sm'), groupRowsHtml: groupRows, groupEmptyHtml: deps.emptyState('Chưa có nhóm lô', 'Chọn các lô QC đã tạo để ghép thành một nhóm, ví dụ 1101/1102.') });
  };
  ...
  const manageInstruments = () => {
    ...
    return manageToolbar('Máy xét nghiệm', 'Quản lý máy xét nghiệm, hãng và số sê-ri.', { action: 'openConfigInstrument' }, 'Thêm máy xét nghiệm') + deps.pres.manageInstrumentTablePresentation({ rowsHtml: rows, emptyHtml: deps.emptyState('Chưa có máy xét nghiệm', 'Thêm máy trước khi cấu hình xét nghiệm.') });
  };
  ...
  const managePanels = () => {
    ...
    return manageToolbar('Panel QC', 'Nhóm các xét nghiệm theo từng máy để thiết lập và quản lý QC.', { action: 'openConfigPanel' }, 'Thêm Panel QC') + deps.pres.managePanelTablePresentation({ rowsHtml: rows, emptyHtml: deps.emptyState('Chưa có Panel QC', 'Tạo Panel QC trước, sau đó nhập Mean/SD theo nhóm lô trong thẻ Mean/SD.') });
  };
  ...
  const manageTransitionsV2 = () => {
    ...
    return manageToolbar('Chuyển tiếp lô QC', 'Theo dõi lô cũ, lô mới và trạng thái khi thay lô.', { action: 'openLotTransitionV2' }, 'Thêm hồ sơ chuyển lô') + deps.pres.manageTransitionTablePresentation({ rowsHtml: rows, emptyHtml: deps.emptyState('Chưa có hồ sơ chuyển lô', 'Tạo hồ sơ để theo dõi chuyển từ lô cũ sang lô mới.') });
  };
  ...
  const manageTargets = () => {
    ...
    if (prerequisite === 'tests') return manageToolbar('Mean/SD theo nhóm lô QC', 'Chọn Panel QC để nhập Mean/SD hàng loạt.', { action: 'setManageTab', args: ['assays'] }, 'Thêm xét nghiệm') + deps.pres.manageEmptyPanelPresentation(deps.emptyState('Chưa có xét nghiệm', 'Tạo xét nghiệm trước, sau đó quay lại nhập Mean/SD theo nhóm lô.'));
    if (prerequisite === 'panels') return manageToolbar('Mean/SD theo nhóm lô QC', 'Chỉ dùng Panel QC để nhập Mean/SD hàng loạt.', { action: 'setManageTab', args: ['panels'] }, 'Thêm Panel QC') + deps.pres.manageEmptyPanelPresentation(deps.emptyState('Chưa có Panel QC', 'Tạo Panel QC và chọn các xét nghiệm thành viên trước, sau đó quay lại nhập Mean/SD theo nhóm lô.'));
    if (prerequisite === 'lots') return manageToolbar('Mean/SD theo nhóm lô QC', 'Chọn Panel QC và nhóm lô để nhập Mean/SD hàng loạt.', { action: 'setManageTab', args: ['lots'] }, 'Thêm lô QC') + deps.pres.manageEmptyPanelPresentation(deps.emptyState('Chưa có lô QC', 'Tạo lô QC trước, gom vào nhóm lô rồi quay lại nhập Mean/SD theo nhóm.'));
    if (prerequisite === 'groups') return manageToolbar('Mean/SD theo nhóm lô QC', 'Chọn Panel QC và nhóm lô để nhập Mean/SD hàng loạt.', { action: 'setManageTab', args: ['lots'] }, 'Thêm nhóm lô') + deps.pres.manageEmptyPanelPresentation(deps.emptyState('Chưa có nhóm lô QC', 'Tạo nhóm lô từ các lô QC trước, sau đó quay lại nhập Mean/SD theo nhóm.'));
    ...
    const targetLevelTabs = deps.pres.targetLevelTabsPresentation(targetLevels, ui().manageTargetLevel), targetLevelToolbar = deps.pres.targetLevelToolbarPresentation(ui().manageTargetLevel, levelLots.map((l: AnyRec) => l.lotNo), targetLevelTabs, deps.esc), targetContent = rowItems.length ? targetLevelToolbar + deps.pres.targetMatrixTablePresentation(rows) + deps.pres.targetMatrixActionsPresentation(deps.btn('Bỏ chọn tất cả', { action: 'targetCheckAll', args: [false] }, 'ghost'), deps.btn('Chọn tất cả', { action: 'targetCheckAll', args: [true] }, 'ghost'), deps.btn('Lưu Mean/SD mức này', { action: 'saveTargetMatrix' }, 'teal')) : (() => { const empty = deps.pres.targetEmptyStatePresentation(allTests.length, levelLots.map((l: AnyRec) => l.lotNo), levelDepletedLots.map((l: AnyRec) => l.lotNo), ui().manageTargetLevel); return deps.emptyState(empty.title, empty.description); })();
  ...
  const manageAssays = () => {
    ...
    return manageToolbar('Danh mục xét nghiệm', 'Quản lý xét nghiệm, máy, đơn vị, phương pháp và TEa.', { action: 'openConfigAssay' }, 'Thêm xét nghiệm') + deps.pres.manageAssayTablePresentation({ rowsHtml: rows, emptyHtml: deps.emptyState('Chưa có xét nghiệm', 'Tạo xét nghiệm trước, sau đó gán lô và Mean/SD ở các thẻ cấu hình tương ứng.') });
  };
  ...
  const manageTeaRefs = () => {
    ...
    const body = rows.map((r: AnyRec) => {
      const rowActions = deps.pres.teaReferenceRowActionsPresentation(r.kind, canManage, r.lab != null), act = rowActions.action === 'restore' ? deps.btn('Khôi phục', { action: 'teaRefRemove', args: [r.analyteId] }, 'ghost sm', 'Khôi phục giá trị mặc định') : rowActions.action === 'remove' ? `<button class="x" data-action="teaRefRemove" data-args="${deps.escapeAttr(JSON.stringify([r.analyteId]))}" title="Xóa xét nghiệm tự thêm">✕</button>` : '';
      const namingTitle = deps.pres.teaReferenceNamingTitlePresentation(r);
      const labButton = rowActions.labProfile === 'none' ? '' : deps.btn(rowActions.labProfile === 'add' ? 'Thêm hồ sơ' : 'Xem hồ sơ', { action: 'teaLabProfileOpen', args: [r.analyteId] }, 'ghost sm', rowActions.labProfile === 'add' ? 'Lập hồ sơ TEa chuẩn hóa' : 'Xem hoặc cập nhật nguồn và lý do lựa chọn');
      return deps.pres.teaReferenceRowPresentation({ namingTitle: deps.escapeAttr(namingTitle), displayName: deps.esc(r.displayName || r.name), unit: deps.esc(r.unit || '—'), section: deps.esc(r.section || '—'), disabled: ro, cliaValue: deps.pres.teaReferenceInputValuePresentation(r.clia), ricosValue: deps.pres.teaReferenceInputValuePresentation(r.ricos), cliaChangeActionAttrs: `data-action="teaRefEdit" data-args='${JSON.stringify([r.analyteId, 'clia'])}' data-action-on="change"`, ricosChangeActionAttrs: `data-action="teaRefEdit" data-args='${JSON.stringify([r.analyteId, 'ricos'])}' data-action-on="change"`, labCellHtml: deps.pres.teaReferenceLabValuePresentation(r.lab, deps.fmt) + labButton, statusHtml: teaStatus(r.kind), actionHtml: act });
    }).join('');
    const empty = deps.pres.teaReferenceEmptyStatePresentation(!!deps.searchText(ui().manageQ));
    return manageToolbar('Bảng TEa tham chiếu', 'Tổng hợp TEa từ các nguồn tham chiếu, dùng thống nhất khi tính Sigma.', canManage ? { action: 'teaRefOpenAdd' } : '', 'Thêm xét nghiệm') + teaSourceRegistryHtml() + deps.pres.teaReferenceTablePresentation({ rowsHtml: body, emptyHtml: deps.emptyState(empty.title, empty.description) });
  };
