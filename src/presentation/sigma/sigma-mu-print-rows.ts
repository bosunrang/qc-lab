...
export function createSigmaMuPrintRows(deps:{mu:(test:Value,entry:Value,level:any)=>Value|undefined;format:(value:any,decimals?:number)=>string;escape:(value:any)=>string;period:(value:any)=>string}) {
  ...
  const periodRows = (test:Value,row:Value,levels:any[]) => (levels || []).map((level,index) => '<tr><td><b>Mức ' + level + '</b></td>' + cells(test,row,level,index) + '</tr>').join('');
  const periodsRows = (test:Value,rows:Value[],levels:any[]) => (rows || []).flatMap(row => { const period = deps.period(row.e.period) || row.e.period || '?'; return (levels || []).map((level,index) => '<tr><td><b>' + deps.escape(period) + '</b></td><td><b>Mức ' + level + '</b></td>' + cells(test,row,level,index) + '</tr>'); }).join('');
