...
type BackupExportDeps={current:()=>Record<string,any>;log:()=>void;save:()=>void;create:(state:Record<string,any>)=>Promise<BackupPackage>;confirmOversized:(bytes:number,detail:any)=>Promise<boolean>;confirm:(dialog:any)=>Promise<boolean>;download:(name:string,text:string)=>boolean;mark:(bytes:number)=>void;update:()=>void;};
type BackupExportResult={status:'done'|'cancelled'|'create-error'|'download-error';error?:unknown};
export type BackupExportCommand=Readonly<{exportFull:(name:string,oversizeDetail:any,warning:(bytes:number)=>any)=>Promise<BackupExportResult>;snapshot:(prefix:string)=>Promise<boolean>}>;
