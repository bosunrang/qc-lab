...
import { entryDetailState } from '../presentation/entry/entry-detail-state';
import { entryTreeOpenState } from '../presentation/entry/entry-tree-open-state';
import { entryMachineSelection } from '../presentation/entry/entry-machine-selection';
import { entryTreeCollapsedState, entryTreeCollapsedToggle } from '../presentation/entry/entry-tree-collapsed-state';
import { entryTreeAutoOpenState } from '../presentation/entry/entry-tree-auto-open-state';
import { entryPreviousLotState } from '../presentation/entry/entry-previous-lot-state';
import { createEntrySheetPendingFocus } from '../presentation/entry/entry-sheet-pending-focus';
import { entryExtraRunState } from '../presentation/entry/entry-extra-run-state';
import { createEntrySelectionRecovery } from '../presentation/entry/entry-selection-recovery';
import { createEntryUnusualDataIssuesHtml } from '../presentation/entry/entry-unusual-data-issues-html';
import { createEntryUnusualDataModalHtml } from '../presentation/entry/entry-unusual-data-modal-html';
import { createEntryDateNoteMessageHtml } from '../presentation/entry/entry-date-note-message-html';
import { createEntrySaveFeedbackHtml } from '../presentation/entry/entry-save-feedback-html';
import { createEntryRecordErrorHtml } from '../presentation/entry/entry-record-error-html';
import { entryInlineSavePlan } from '../presentation/entry/entry-inline-save-plan';
import { createEntrySaveMessageHtml } from '../presentation/entry/entry-save-message-html';
import { entryVoidConfirmDetail } from '../presentation/entry/entry-void-confirm-detail';
import { createEntryVoidFeedbackHtml } from '../presentation/entry/entry-void-feedback-html';
import { createEntryVoidModalHtml } from '../presentation/entry/entry-void-modal-html';
import { entryVoidConfirmDialog } from '../presentation/entry/entry-void-confirm-dialog';
import { entryVoidPlan } from '../presentation/entry/entry-void-plan';
import { entrySheetMonthState, entrySheetTodayState } from '../presentation/entry/entry-sheet-month-state';
import { entryQcWarningPlan } from '../presentation/entry/entry-qc-warning-plan';
import { entryQcUnavailableHtml } from '../presentation/entry/entry-qc-unavailable-html';
import { createEntrySdZeroWarningHtml } from '../presentation/entry/entry-sd-zero-warning-html';
import { entryVoidPeriodLockedHtml } from '../presentation/entry/entry-void-period-locked-html';
import { entryMachineFilterState } from '../presentation/entry/entry-machine-filter-state';
import { createEntryDateNoteErrorHtml } from '../presentation/entry/entry-date-note-error-html';
import { entryInvalidInputHtml } from '../presentation/entry/entry-invalid-input-html';
import { createEntryVoidPointContext } from '../presentation/entry/entry-void-point-context';
import { entrySaveAuditDetail } from '../presentation/entry/entry-save-audit-detail';
import { createEntrySaveWorkflow } from '../presentation/entry/entry-save-workflow';
import { entryVoidAuditDetail } from '../presentation/entry/entry-void-audit-detail';
import { createEntryVoidWorkflow } from '../presentation/entry/entry-void-workflow';
import { createEntryScreenSelectionWorkflow } from '../presentation/entry/entry-screen-selection-workflow';
import { createEntrySheetWorkflow } from '../presentation/entry/entry-sheet-workflow';
import { createEntryTreeInteractionWorkflow } from '../presentation/entry/entry-tree-interaction-workflow';
import { createEntryTableInteractionWorkflow } from '../presentation/entry/entry-table-interaction-workflow';
import { entryVoidReasonErrorState } from '../presentation/entry/entry-void-reason-error-state';
import { entryRangeSourceLabel } from '../presentation/entry/entry-range-source-label';
import { entryRangeState } from '../presentation/entry/entry-range-state';
import { entrySheetKeyPlan } from '../presentation/entry/entry-sheet-key-plan';
import { createEntryWorksheetHtml } from '../presentation/entry/entry-worksheet-html';
import { createEntryPageHtml } from '../presentation/entry/entry-page-html';
import { createEntryRangeSummaryHtml } from '../presentation/entry/entry-range-summary-html';
import { createEntryChartPanelHtml } from '../presentation/entry/entry-chart-panel-html';
import { createEntryPointsPanelHtml } from '../presentation/entry/entry-points-panel-html';
import { createEntryPointTableCardHtml } from '../presentation/entry/entry-point-table-card-html';
import { createEntryVoidedPointsHtml } from '../presentation/entry/entry-voided-points-html';
import { createEntryChartCardHtml } from '../presentation/entry/entry-chart-card-html';
import { createEntryLevelHeaderHtml } from '../presentation/entry/entry-level-header-html';
import { createEntryTableWindowNoteHtml } from '../presentation/entry/entry-table-window-note-html';
import { createEntryChartStatsHtml } from '../presentation/entry/entry-chart-stats-html';
import { createEntrySheetMonthOptionsHtml } from '../presentation/entry/entry-sheet-month-options-html';
import { createEntryRangeButtonsHtml } from '../presentation/entry/entry-range-buttons-html';
import { createEntryPointRowHtml } from '../presentation/entry/entry-point-row-html';
import { createEntryVoidedPointRowHtml } from '../presentation/entry/entry-voided-point-row-html';
import { createEntrySheetRowHtml } from '../presentation/entry/entry-sheet-row-html';
import { createEntryRunCellHtml } from '../presentation/entry/entry-run-cell-html';
import { entryDayStatusHtml } from '../presentation/entry/entry-day-status-html';
import { createEntryDayNoteHtml } from '../presentation/entry/entry-day-note-html';
import { createEntryStaffCellHtml } from '../presentation/entry/entry-staff-cell-html';
import { createEntrySavedRunHtml } from '../presentation/entry/entry-saved-run-html';
import { createEntryEmptyRunHtml } from '../presentation/entry/entry-empty-run-html';
import { createEntryAddRunButtonHtml } from '../presentation/entry/entry-add-run-button-html';
import { createEntryRangePresetButtonHtml } from '../presentation/entry/entry-range-preset-button-html';
import { createEntryCumulativeStatsHtml } from '../presentation/entry/entry-cumulative-stats-html';
import { entryTreeEmptyHtml } from '../presentation/entry/entry-tree-empty-html';
import { createEntryTreeMachineHtml } from '../presentation/entry/entry-tree-machine-html';
import { createEntryTreeGroupHtml } from '../presentation/entry/entry-tree-group-html';
import { createEntryTreeAssayHtml } from '../presentation/entry/entry-tree-assay-html';
import { createEntryMachineOptionsHtml } from '../presentation/entry/entry-machine-options-html';
import { createEntryTreeHeaderHtml } from '../presentation/entry/entry-tree-header-html';
import { createEntryChartLotActionHtml } from '../presentation/entry/entry-chart-lot-action-html';
import { createEntryTreeToggleButtonHtml } from '../presentation/entry/entry-tree-toggle-button-html';
import { createEntryEmptyPageHtml } from '../presentation/entry/entry-empty-page-html';
import { entrySheetEmptyRowHtml } from '../presentation/entry/entry-sheet-empty-row-html';
import { entryTreePanelIconHtml } from '../presentation/entry/entry-tree-panel-icon-html';
import { entryChartRangeSourceHtml } from '../presentation/entry/entry-chart-range-source-html';
import { createEntrySheetNavigationButtonsHtml } from '../presentation/entry/entry-sheet-navigation-buttons-html';
...
import { createManageNavigationWorkflow } from '../presentation/manage/manage-navigation-workflow';
import { createManagePanelWorkflow, managePanelRemovalDialog } from '../presentation/manage/manage-panel-workflow';
import { createManageLotGroupWorkflow, manageLotGroupRemovalDialog } from '../presentation/manage/manage-lot-group-workflow';
import { createManageLotWorkflow, manageLotRemovalDialog } from '../presentation/manage/manage-lot-workflow';
import { createManageInstrumentWorkflow, manageInstrumentRemovalDialog } from '../presentation/manage/manage-instrument-workflow';
import { createManageTransitionWorkflow, manageTransitionRemovalDialog } from '../presentation/manage/manage-transition-workflow';
import { createManageAssayRemovalWorkflow, manageAssayLockedMessage, manageAssayRemovalConfirm, manageAssayRemovalReauth } from '../presentation/manage/manage-assay-removal-workflow';
import { createManageTargetMatrixWorkflow } from '../presentation/manage/manage-target-matrix-workflow';
import { createManageTargetPickWorkflow } from '../presentation/manage/manage-target-pick-workflow';
import { createManageTargetOverwriteWorkflow } from '../presentation/manage/manage-target-overwrite-workflow';
import { createManageTargetBackfillWorkflow } from '../presentation/manage/manage-target-backfill-workflow';
import { manageTargetCommitAudit } from '../presentation/manage/manage-target-commit-workflow';
import { managePanelFormState } from '../presentation/manage/manage-panel-form-workflow';
...
import { createActionSuggestions, type ActionSuggestionsApi } from '../presentation/nce/action-suggestions';
import { createActionFormShell, type ActionFormShellApi } from '../presentation/nce/action-form-shell';
...
type QCLabGlobal = typeof globalThis & {
  ...
  ActionSuggestions?: ActionSuggestionsApi;
  ActionFormShell?: ActionFormShellApi;
  ...
  entryDetailState?: typeof entryDetailState;
  entryTreeOpenState?: typeof entryTreeOpenState;
  entryMachineSelection?: typeof entryMachineSelection;
  entryTreeCollapsedState?: typeof entryTreeCollapsedState;
  entryTreeCollapsedToggle?: typeof entryTreeCollapsedToggle;
  entryTreeAutoOpenState?: typeof entryTreeAutoOpenState;
  entryPreviousLotState?: typeof entryPreviousLotState;
  entrySheetPendingFocus?: ReturnType<typeof createEntrySheetPendingFocus<any>>;
  entryExtraRunState?: typeof entryExtraRunState;
  entrySelectionRecovery?: ReturnType<typeof createEntrySelectionRecovery<any>>;
  entryUnusualDataIssuesHtml?: ReturnType<typeof createEntryUnusualDataIssuesHtml>;
  entryUnusualDataModalHtml?: ReturnType<typeof createEntryUnusualDataModalHtml>;
  entryDateNoteMessageHtml?: ReturnType<typeof createEntryDateNoteMessageHtml>;
  entrySaveFeedbackHtml?: ReturnType<typeof createEntrySaveFeedbackHtml>;
  entryRecordErrorHtml?: ReturnType<typeof createEntryRecordErrorHtml>;
  entryInlineSavePlan?: typeof entryInlineSavePlan;
  entrySaveMessageHtml?: ReturnType<typeof createEntrySaveMessageHtml>;
  entryVoidConfirmDetail?: typeof entryVoidConfirmDetail;
  entryVoidFeedbackHtml?: ReturnType<typeof createEntryVoidFeedbackHtml>;
  entryVoidModalHtml?: ReturnType<typeof createEntryVoidModalHtml>;
  entryVoidConfirmDialog?: typeof entryVoidConfirmDialog;
  entryVoidPlan?: typeof entryVoidPlan;
  entrySheetMonthState?: typeof entrySheetMonthState;
  entrySheetTodayState?: typeof entrySheetTodayState;
  entryQcWarningPlan?: typeof entryQcWarningPlan;
  entryQcUnavailableHtml?: typeof entryQcUnavailableHtml;
  entrySdZeroWarningHtml?: ReturnType<typeof createEntrySdZeroWarningHtml>;
  entryVoidPeriodLockedHtml?: typeof entryVoidPeriodLockedHtml;
  entryMachineFilterState?: typeof entryMachineFilterState;
  entryDateNoteErrorHtml?: ReturnType<typeof createEntryDateNoteErrorHtml>;
  entryInvalidInputHtml?: typeof entryInvalidInputHtml;
  entryVoidPointContext?: ReturnType<typeof createEntryVoidPointContext>;
  entrySaveAuditDetail?: typeof entrySaveAuditDetail;
  entrySaveWorkflow?: ReturnType<typeof createEntrySaveWorkflow>;
  entryVoidAuditDetail?: typeof entryVoidAuditDetail;
  entryVoidWorkflow?: ReturnType<typeof createEntryVoidWorkflow>;
  entryScreenSelectionWorkflow?: ReturnType<typeof createEntryScreenSelectionWorkflow<any>>;
  entrySheetWorkflow?: ReturnType<typeof createEntrySheetWorkflow<any>>;
  entryTreeInteractionWorkflow?: ReturnType<typeof createEntryTreeInteractionWorkflow>;
  entryTableInteractionWorkflow?: ReturnType<typeof createEntryTableInteractionWorkflow>;
  manageNavigationWorkflow?: ReturnType<typeof createManageNavigationWorkflow>;
  managePanelWorkflow?: ReturnType<typeof createManagePanelWorkflow>;
  manageLotGroupWorkflow?: ReturnType<typeof createManageLotGroupWorkflow>;
  manageLotWorkflow?: ReturnType<typeof createManageLotWorkflow>;
  manageInstrumentWorkflow?: ReturnType<typeof createManageInstrumentWorkflow>;
  manageTransitionWorkflow?: ReturnType<typeof createManageTransitionWorkflow>;
  manageAssayRemovalWorkflow?: ReturnType<typeof createManageAssayRemovalWorkflow>;
  manageTargetMatrixWorkflow?: ReturnType<typeof createManageTargetMatrixWorkflow>;
  manageTargetPickWorkflow?: ReturnType<typeof createManageTargetPickWorkflow>;
  manageTargetOverwriteWorkflow?: ReturnType<typeof createManageTargetOverwriteWorkflow>;
  manageTargetBackfillWorkflow?: ReturnType<typeof createManageTargetBackfillWorkflow>;
  manageTargetCommitAudit?: typeof manageTargetCommitAudit;
  managePanelFormState?: typeof managePanelFormState;
  entryVoidReasonErrorState?: typeof entryVoidReasonErrorState;
  entryRangeSourceLabel?: typeof entryRangeSourceLabel;
  entryRangeState?: typeof entryRangeState;
  entrySheetKeyPlan?: typeof entrySheetKeyPlan;
  entryWorksheetHtml?: ReturnType<typeof createEntryWorksheetHtml>;
  entryPageHtml?: ReturnType<typeof createEntryPageHtml>;
  entryRangeSummaryHtml?: ReturnType<typeof createEntryRangeSummaryHtml>;
  entryChartPanelHtml?: ReturnType<typeof createEntryChartPanelHtml>;
  entryPointsPanelHtml?: ReturnType<typeof createEntryPointsPanelHtml>;
  entryPointTableCardHtml?: ReturnType<typeof createEntryPointTableCardHtml>;
  entryVoidedPointsHtml?: ReturnType<typeof createEntryVoidedPointsHtml>;
  entryChartCardHtml?: ReturnType<typeof createEntryChartCardHtml>;
  entryLevelHeaderHtml?: ReturnType<typeof createEntryLevelHeaderHtml>;
  entryTableWindowNoteHtml?: ReturnType<typeof createEntryTableWindowNoteHtml>;
  entryChartStatsHtml?: ReturnType<typeof createEntryChartStatsHtml>;
  entrySheetMonthOptionsHtml?: ReturnType<typeof createEntrySheetMonthOptionsHtml>;
  entryRangeButtonsHtml?: ReturnType<typeof createEntryRangeButtonsHtml>;
  entryPointRowHtml?: ReturnType<typeof createEntryPointRowHtml>;
  entryVoidedPointRowHtml?: ReturnType<typeof createEntryVoidedPointRowHtml>;
  entrySheetRowHtml?: ReturnType<typeof createEntrySheetRowHtml>;
  entryRunCellHtml?: ReturnType<typeof createEntryRunCellHtml>;
  entryDayStatusHtml?: typeof entryDayStatusHtml;
  entryDayNoteHtml?: ReturnType<typeof createEntryDayNoteHtml>;
  entryStaffCellHtml?: ReturnType<typeof createEntryStaffCellHtml>;
  entrySavedRunHtml?: ReturnType<typeof createEntrySavedRunHtml>;
  entryEmptyRunHtml?: ReturnType<typeof createEntryEmptyRunHtml>;
  entryAddRunButtonHtml?: ReturnType<typeof createEntryAddRunButtonHtml>;
  entryRangePresetButtonHtml?: ReturnType<typeof createEntryRangePresetButtonHtml>;
  entryCumulativeStatsHtml?: ReturnType<typeof createEntryCumulativeStatsHtml>;
  entryTreeEmptyHtml?: typeof entryTreeEmptyHtml;
  entryTreeMachineHtml?: ReturnType<typeof createEntryTreeMachineHtml>;
  entryTreeGroupHtml?: ReturnType<typeof createEntryTreeGroupHtml>;
  entryTreeAssayHtml?: ReturnType<typeof createEntryTreeAssayHtml>;
  entryMachineOptionsHtml?: ReturnType<typeof createEntryMachineOptionsHtml>;
  entryTreeHeaderHtml?: ReturnType<typeof createEntryTreeHeaderHtml>;
  entryChartLotActionHtml?: ReturnType<typeof createEntryChartLotActionHtml>;
  entryTreeToggleButtonHtml?: ReturnType<typeof createEntryTreeToggleButtonHtml>;
  entryEmptyPageHtml?: ReturnType<typeof createEntryEmptyPageHtml>;
  entrySheetEmptyRowHtml?: typeof entrySheetEmptyRowHtml;
  entryTreePanelIconHtml?: typeof entryTreePanelIconHtml;
  entryChartRangeSourceHtml?: typeof entryChartRangeSourceHtml;
  entrySheetNavigationButtonsHtml?: ReturnType<typeof createEntrySheetNavigationButtonsHtml>;
...
root.entrySheetPendingFocus=createEntrySheetPendingFocus<any>({date:item=>String(item.dataset.focusDate||''),level:item=>String(item.dataset.focusLevel||''),choose:items=>(root as any).entrySheetFocus(items)});
root.entryExtraRunState=entryExtraRunState;
root.entrySelectionRecovery=createEntrySelectionRecovery<any>({id:test=>test.id,levels:test=>(root as any).operationalLevels(test)});
root.entryUnusualDataIssuesHtml=createEntryUnusualDataIssuesHtml({escape:value=>(root as any).esc(value)});
root.entryUnusualDataModalHtml=createEntryUnusualDataModalHtml({button:(label,action,variant)=>(root as any).btn(label,action,variant)});
root.entryDateNoteMessageHtml=createEntryDateNoteMessageHtml({escape:value=>(root as any).esc(value)});
root.entrySaveFeedbackHtml=createEntrySaveFeedbackHtml({escape:value=>(root as any).esc(value)});
root.entryRecordErrorHtml=createEntryRecordErrorHtml({escape:value=>(root as any).esc(value)});
root.entryInlineSavePlan=entryInlineSavePlan;
root.entrySaveMessageHtml=createEntrySaveMessageHtml({escape:value=>(root as any).esc(value),feedbackHtml:feedback=>(root as any).entrySaveFeedbackHtml(feedback)});
root.entryVoidConfirmDetail=entryVoidConfirmDetail;
root.entryVoidFeedbackHtml=createEntryVoidFeedbackHtml({escape:value=>(root as any).esc(value)});
root.entryVoidModalHtml=createEntryVoidModalHtml({button:(label,action,variant)=>(root as any).btn(label,action,variant),escape:value=>(root as any).esc(value)});
...
root.entrySdZeroWarningHtml=createEntrySdZeroWarningHtml({escape:value=>(root as any).esc(value)});
root.entryVoidPeriodLockedHtml=entryVoidPeriodLockedHtml;
root.entryMachineFilterState=entryMachineFilterState;
root.entryDateNoteErrorHtml=createEntryDateNoteErrorHtml({escape:value=>(root as any).esc(value)});
root.entryInvalidInputHtml=entryInvalidInputHtml;
root.entryVoidPointContext=createEntryVoidPointContext({errorType:rules=>(root as any).errorType(rules)});
root.entrySaveAuditDetail=entrySaveAuditDetail;
root.entrySaveWorkflow=createEntrySaveWorkflow({inputPlan:entryInlineSavePlan,warningPlan:entryQcWarningPlan,unavailableHtml:entryQcUnavailableHtml,invalidInputHtml:entryInvalidInputHtml,sdZeroWarningHtml:createEntrySdZeroWarningHtml({escape:value=>(root as any).esc(value)}),unusualIssuesHtml:createEntryUnusualDataIssuesHtml({escape:value=>(root as any).esc(value)}),unusualModalHtml:createEntryUnusualDataModalHtml({button:(label,action,variant)=>(root as any).btn(label,action,variant)}),auditDetail:entrySaveAuditDetail,messageHtml:createEntrySaveMessageHtml({escape:value=>(root as any).esc(value),feedbackHtml:feedback=>(root as any).entrySaveFeedbackHtml(feedback)})});
root.entryVoidAuditDetail=entryVoidAuditDetail;
root.entryVoidWorkflow=createEntryVoidWorkflow({plan:entryVoidPlan,modalHtml:createEntryVoidModalHtml({button:(label,action,variant)=>(root as any).btn(label,action,variant),escape:value=>(root as any).esc(value)}),reasonValid:entryVoidReasonValid,reasonError:entryVoidReasonErrorState,pointContext:createEntryVoidPointContext({errorType:rules=>(root as any).errorType(rules)}),confirmDetail:entryVoidConfirmDetail,confirmDialog:entryVoidConfirmDialog,periodLockedHtml:entryVoidPeriodLockedHtml,auditDetail:entryVoidAuditDetail,feedbackHtml:createEntryVoidFeedbackHtml({escape:value=>(root as any).esc(value)})});
root.entryScreenSelectionWorkflow=createEntryScreenSelectionWorkflow({recoverSelection:createEntrySelectionRecovery<any>({id:test=>test.id,levels:test=>(root as any).operationalLevels(test)}),machineSelection:entryMachineSelection,autoOpenState:entryTreeAutoOpenState});
root.entrySheetWorkflow=createEntrySheetWorkflow({monthValue:entrySheetMonthValue,monthState:entrySheetMonthState,todayState:entrySheetTodayState,monthPart:entrySheetMonthPart,keyPlan:entrySheetKeyPlan,orderInputs:createEntrySheetInputOrder<any>({date:element=>String(element.dataset.focusDate||''),run:element=>Number(element.dataset.focusRun||0),level:element=>Number(element.dataset.focusLevel||0)}),navigationTarget:createEntrySheetNavigation<any>({date:element=>String(element.dataset.focusDate||''),run:element=>String(element.dataset.focusRun||''),level:element=>String(element.dataset.focusLevel||'')}).target,pendingFocus:createEntrySheetPendingFocus<any>({date:item=>String(item.dataset.focusDate||''),level:item=>String(item.dataset.focusLevel||''),choose:items=>(root as any).entrySheetFocus(items)}),focus:createEntrySheetFocus<any>(element=>!!element.classList.contains('empty'))});
root.entryTreeInteractionWorkflow=createEntryTreeInteractionWorkflow({detailState:entryDetailState,openState:entryTreeOpenState,collapsedToggle:entryTreeCollapsedToggle,keyCommand:entryTreeKeyCommand,navigationTarget:createEntryTreeNavigation<any>().target,visibility:entryTreeVisibility,machineFilter:entryMachineFilterState,selection:entrySelectionState,previousLot:entryPreviousLotState});
root.entryTableInteractionWorkflow=createEntryTableInteractionWorkflow({extraRunRequest:entryExtraRunRequest,extraRunState:entryExtraRunState,rangePreset:entryRangePreset,rangeState:entryRangeState,dateRangeInput:createEntryDateRangeInput(value=>(root as any).parseVN(value)),dateNoteFeedback:entryDateNoteFeedback,dateNoteErrorMessage:entryDateNoteErrorMessage});
root.manageNavigationWorkflow=createManageNavigationWorkflow();
root.managePanelWorkflow=createManagePanelWorkflow({removal:(state,input)=>(root as any).ManageConfigService.panelRemoval(state,input),removalDialog:managePanelRemovalDialog});
root.manageLotGroupWorkflow=createManageLotGroupWorkflow({removal:(state,input)=>(root as any).ManageConfigService.lotGroupRemoval(state,input),removalDialog:manageLotGroupRemovalDialog});
root.manageLotWorkflow=createManageLotWorkflow({removal:(state,input)=>(root as any).ManageConfigService.lotRemoval(state,input),removalDialog:manageLotRemovalDialog});
root.manageInstrumentWorkflow=createManageInstrumentWorkflow({removal:(state,input)=>(root as any).ManageConfigService.instrumentRemoval(state,input),removalDialog:manageInstrumentRemovalDialog});
root.manageTransitionWorkflow=createManageTransitionWorkflow({removal:(state,input)=>(root as any).ManageConfigService.lotTransitionRemoval(state,input),removalDialog:manageTransitionRemovalDialog});
root.manageAssayRemovalWorkflow=createManageAssayRemovalWorkflow({lockedMessage:manageAssayLockedMessage,confirmDialog:manageAssayRemovalConfirm,reauthDialog:manageAssayRemovalReauth});
root.manageTargetMatrixWorkflow=createManageTargetMatrixWorkflow();
root.manageTargetPickWorkflow=createManageTargetPickWorkflow({normalize:input=>(root as any).ManageConfigService.normalizeTargetPick(input)});
root.manageTargetOverwriteWorkflow=createManageTargetOverwriteWorkflow();
root.manageTargetBackfillWorkflow=createManageTargetBackfillWorkflow({pointsForPick:pick=>{const test=(state.tests||[]).find((item:any)=>item.id===pick.testId);return test?(root as any).ManageConfigService.targetPickBackfillPoints((state.data&&state.data[test.id]||[]),test,pick.lot,pick):[];}});
...
root.entryWorksheetHtml=createEntryWorksheetHtml({escape:value=>(root as any).esc(value),button:(label,action,variant,title)=> (root as any).btn(label,action,variant,title)});
root.entryPageHtml=createEntryPageHtml({head:(title,subtitle)=>(root as any).headOnly(title,subtitle)});
root.entryRangeSummaryHtml=createEntryRangeSummaryHtml({escape:value=>(root as any).esc(value)});
root.entryChartPanelHtml=createEntryChartPanelHtml({escape:value=>(root as any).esc(value)});
root.entryPointsPanelHtml=createEntryPointsPanelHtml({escape:value=>(root as any).esc(value)});
root.entryPointTableCardHtml=createEntryPointTableCardHtml({escape:value=>(root as any).esc(value)});
root.entryVoidedPointsHtml=createEntryVoidedPointsHtml();
root.entryChartCardHtml=createEntryChartCardHtml({escape:value=>(root as any).escAttr(value)});
root.entryLevelHeaderHtml=createEntryLevelHeaderHtml({escape:value=>(root as any).escAttr(value)});
root.entryTableWindowNoteHtml=createEntryTableWindowNoteHtml({button:(label,action,variant)=>(root as any).btn(label,action,variant)});
root.entryChartStatsHtml=createEntryChartStatsHtml({escape:value=>(root as any).esc(value)});
root.entrySheetMonthOptionsHtml=createEntrySheetMonthOptionsHtml();
root.entryRangeButtonsHtml=createEntryRangeButtonsHtml({presetButton:input=>createEntryRangePresetButtonHtml({escape:value=>(root as any).escAttr(value)})(input)});
root.entryPointRowHtml=createEntryPointRowHtml({escape:value=>(root as any).esc(value)});
root.entryVoidedPointRowHtml=createEntryVoidedPointRowHtml({escape:value=>(root as any).esc(value)});
root.entrySheetRowHtml=createEntrySheetRowHtml({escape:value=>(root as any).esc(value)});
root.entryRunCellHtml=createEntryRunCellHtml();
root.entryDayStatusHtml=entryDayStatusHtml;
root.entryDayNoteHtml=createEntryDayNoteHtml({escape:value=>(root as any).esc(value)});
root.entryStaffCellHtml=createEntryStaffCellHtml({escape:value=>(root as any).escAttr(value)});
root.entrySavedRunHtml=createEntrySavedRunHtml({escape:value=>(root as any).esc(value)});
root.entryEmptyRunHtml=createEntryEmptyRunHtml({escape:value=>(root as any).escAttr(value)});
root.entryAddRunButtonHtml=createEntryAddRunButtonHtml({escape:value=>(root as any).escAttr(value)});
root.entryRangePresetButtonHtml=createEntryRangePresetButtonHtml({escape:value=>(root as any).escAttr(value)});
root.entryCumulativeStatsHtml=createEntryCumulativeStatsHtml({escape:value=>(root as any).esc(value)});
root.entryTreeEmptyHtml=entryTreeEmptyHtml;
root.entryTreeMachineHtml=createEntryTreeMachineHtml({escape:value=>(root as any).escAttr(value)});
root.entryTreeGroupHtml=createEntryTreeGroupHtml({escape:value=>(root as any).escAttr(value)});
root.entryTreeAssayHtml=createEntryTreeAssayHtml({escape:value=>(root as any).escAttr(value)});
root.entryMachineOptionsHtml=createEntryMachineOptionsHtml({escape:value=>(root as any).escAttr(value)});
root.entryTreeHeaderHtml=createEntryTreeHeaderHtml({escape:value=>(root as any).escAttr(value)});
root.entryChartLotActionHtml=createEntryChartLotActionHtml({button:(label,action,variant)=>(root as any).btn(label,action,variant),rangeSource:input=>entryChartRangeSourceHtml(input)});
root.entryTreeToggleButtonHtml=createEntryTreeToggleButtonHtml({button:(label,action,variant,title,options)=>(root as any).btn(label,action,variant,title,options)});
root.entryEmptyPageHtml=createEntryEmptyPageHtml({head:(title,subtitle)=>(root as any).headOnly(title,subtitle)});
...
root.entrySheetNavigationButtonsHtml=createEntrySheetNavigationButtonsHtml({button:(label,action,variant)=>(root as any).btn(label,action,variant)});
...
root.ActionSuggestions = createActionSuggestions({ escape: value => (root as any).esc(value), quote: value => (root as any).jsq(value) });
root.ActionFormShell = createActionFormShell({ emptyState: (title,description,action) => (root as any).emptyState(title,description,action), button: (label,action,variant) => (root as any).btn(label,action,variant), escape: value => (root as any).esc(value) });
