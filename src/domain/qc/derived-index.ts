...
export function createQcDerivedIndex(deps:{operationalGroup:(group:S)=>boolean;switchesLot:(transition:S)=>boolean}){
  ...
  const index=(state:S)=>{if(cached&&stamp(state,cached.stamp)===true)return cached;const currentStamp=stamp(state),panels=(state.qcPanels||[]).filter((panel:S)=>panel.active!==false),testPanel=new Map(),testOrder=new Map(),lotGroupByLotId=new Map();panels.forEach((panel:S,panelIndex:number)=>(panel.testIds||[]).forEach((id:string,testIndex:number)=>{if(!testPanel.has(id))testPanel.set(id,panel);const order=panelIndex*10000+testIndex;if(!testOrder.has(id)||order<testOrder.get(id))testOrder.set(id,order);}));(state.lotGroups||[]).filter(deps.operationalGroup).forEach((group:S)=>(group.lotIds||[]).forEach((id:string)=>{if(!lotGroupByLotId.has(id))lotGroupByLotId.set(id,group)}));const acceptedTransitionToLot=new Map();(state.lotTransitions||[]).filter(deps.switchesLot).forEach((transition:S)=>{if(!acceptedTransitionToLot.has(transition.toLotId))acceptedTransitionToLot.set(transition.toLotId,transition)});return cached={stamp:currentStamp,panels,testPanel,testOrder,lotById:new Map((state.qcLots||[]).map((lot:S)=>[lot.id,lot])),lotGroupByLotId,acceptedTransitionToLot,operationalTests:null,levels:new Map(),groups:new Map()};};
  return Object.assign(index,{clear:()=>{cached=null;}});
}
