...
import {
  ...
  voidQcPoint, entryToggleRows, entryDetailToggled, openRangeWorkflow, revertRange, entrySheetKey,
          ...
          {model.canAdd ? (
            <div className="empty-actions">
              {model.addTarget === 'manage'
                ? <button className="btn teal" onClick={() => go('manage')}>Thêm xét nghiệm</button>
                : <button className="btn teal" onClick={goManageTargets}>Cấu hình Mean/SD</button>}
            </div>
          ) : null}
        </div>
      </div>
...
function TreeNodeView({ node }: { node: EntryTreeNode }) {
  ...
  if (node.kind === 'machine') {
    return (
      <div className="tnode tn-machine" data-tree-role="machine" data-key={node.key} role="treeitem" tabIndex={0} aria-expanded={node.open} onClick={() => treeToggle(node.key)} onKeyDown={e => entryTreeKey(e.currentTarget, e)}>
  ...
  if (node.kind === 'group') {
    return (
      <div className={`tnode tn-test ${node.open ? 'open' : ''}`} data-tree-role="group" data-key={node.key} data-search={node.search} role="treeitem" tabIndex={0} aria-expanded={node.open} style={node.parentOpen ? undefined : { display: 'none' }} onClick={() => treeToggle(node.key)} onKeyDown={e => entryTreeKey(e.currentTarget, e)}>
        <span className="caret" aria-hidden="true">{node.open ? '−' : '+'}</span>{node.name}<span className={`state ${node.stateClass}`}>{node.stateText}</span>
      </div>
    );
  }
  return (
    <div className={`tnode tn-config ${node.selected ? 'on' : ''}`} data-tree-role="assay" data-test-id={node.testId} data-search={node.search} role="treeitem" tabIndex={0} aria-current={node.selected ? 'true' : 'false'} style={node.visible ? undefined : { display: 'none' }} onClick={() => entryPick(node.testId, node.level)} onKeyDown={e => entryTreeKey(e.currentTarget, e)}>
  ...
  return (
    <div className="tree" id="entryTreePanel">
      <div className="entry-tree-head">
        ...
        <button type="button" className="btn ghost icon entry-tree-toggle" title="Ẩn danh mục nội kiểm" aria-label="Ẩn danh mục nội kiểm" aria-controls="entryTreePanel" aria-expanded="true" onClick={toggleEntryTree}><TreeIcon /></button>
      </div>
      <div className="tree-tools">
        <input id="entrySearch" aria-label="Tìm xét nghiệm, máy hoặc lô" placeholder="Tìm test, máy hoặc lô..." defaultValue={model.query} onChange={e => entryFilter(e.target.value)} />
        <select aria-label="Lọc theo máy xét nghiệm" defaultValue={model.selectedMachine} onChange={e => entrySetMachine(e.target.value)}>
    ...
    return (
      <div className="qc-run-slot">
        <input className="qc-inline-input empty" type="text" inputMode="decimal" autoComplete="off" placeholder="--" title={slot.title} aria-label={slot.ariaLabel} aria-keyshortcuts="ArrowUp ArrowDown ArrowLeft ArrowRight Enter"
          data-focus-date={slot.date} data-focus-run={slot.runNo} data-focus-level={slot.levelIndex}
          onKeyDown={e => entrySheetKey(e.currentTarget, e)}
          onBlur={e => entrySheetRunChanged(slot.actionArgs[0], slot.actionArgs[1], slot.actionArgs[2], slot.actionArgs[3], slot.actionArgs[4], e.target.value)} />
      </div>
    );
  }
  return (
...
function SheetCellView({ cell }: { cell: EntrySheetCellModel }) {
  return (
    <td className={`num qc-run-cell${cell.parallel ? ' qc-parallel-cell' : ''}`}>
      ...
      {cell.addRun ? <button type="button" className="qc-add-run-btn" title="Thêm lần chạy bổ sung" onClick={() => entryUnlockExtraRun(cell.addRun!.actionArgs[0], cell.addRun!.actionArgs[1], cell.addRun!.actionArgs[2], cell.addRun!.actionArgs[3], cell.addRun!.actionArgs[4])}><span className="qc-add-run-icon">+</span><span className="qc-add-run-label">Thêm</span></button> : null}
    </td>
  );
}
...
function SheetDayNote({ testId, date, note }: { testId: string; date: string; note: EntrySheetRow['note'] }) {
  ...
  return <textarea className="qc-note-input" rows={1} placeholder={note.placeholder} defaultValue={note.value} onBlur={e => entryDateNoteSave(testId, date, e.target.value)} />;
}
    ...
    <div className="panel qc-sheet-panel">
      <div className="qc-sheet-heading">
        ...
        <div className="qc-month-area"><div className="qc-month-picker">
          <select aria-label="Chọn tháng" key={`m-${ws.sheetYear}-${ws.sheetMonthNo}`} defaultValue={ws.sheetMonthNo} onChange={e => entrySetSheetPart('month', e.target.value)}>
            {monthOptions.map(m => <option value={m} key={m}>Tháng {m}</option>)}
          </select>
          <select aria-label="Chọn năm" key={`y-${ws.sheetYear}-${ws.sheetMonthNo}`} defaultValue={ws.sheetYear} onChange={e => entrySetSheetPart('year', e.target.value)}>
            {yearOptions.map(y => <option value={y} key={y}>{y}</option>)}
          </select>
          <button type="button" className="btn ghost sm qc-current-month" onClick={() => entrySetSheetMonth(ws.currentIsoMonth)}>Tháng hiện tại</button>
          <button type="button" className="btn teal sm qc-today-jump" onClick={entryGoToday}>Tới hôm nay</button>
        </div></div>
      </div>
...
function LjAction({ action }: { action: EntryLjPanelItem['action'] }) {
  ...
  if (action.kind === 'showCurrent') return <button type="button" className="btn teal sm" onClick={() => entryShowCurrentLot(action.level)}>Xem lô mới</button>;
  return <button type="button" className="btn ghost sm" onClick={() => entryShowPrevLot(action.level, action.lot)}>Xem lô cũ</button>;
}

function LjMini({ item }: { item: EntryLjPanelItem }) {
  const lot = item.lot || '?';
  return (
    <div className={`lj-mini ${item.on ? 'on' : ''}${item.parallel ? ' lj-mini-parallel' : ''}`}
      onClick={e => { if ((e.target as HTMLElement).closest('button, input, select')) return; entryFocusLevel(item.level); }}
      onKeyDown={e => { if ((e.target as HTMLElement).closest('button, input, select')) return; if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); entryFocusLevel(item.level); } }}
      role="button" tabIndex={0} aria-label={`Chọn mức ${item.level}, lô ${lot}, ${item.pointCount} điểm${item.parallel ? ', lô chạy song song' : ''}`}>
...
function TableRowView({ row, testId }: { row: EntryTableCard['rows'][number]; testId: string }) {
  ...
  return (
    <tr className={cls} data-qc-point-id={row.pointId} tabIndex={-1}>
      ...
      <td className="qc-row-actions">{row.canVoid ? <button type="button" className="btn danger sm" title="Hủy điểm QC có ghi lý do" onClick={() => voidQcPoint(testId, row.pointId)}>Hủy</button> : null}</td>
    </tr>
  );
}
        ...
        <>
          ...
          {card.rowControl ? (
            <div className="table-window-note">
              ...
              <button type="button" className="btn ghost sm" onClick={() => entryToggleRows(card.rowControl!.tableKey)}>{card.rowControl.limited ? 'Hiện toàn bộ' : 'Thu gọn'}</button>
            </div>
          ) : null}
        </>
...
function PointsInViewPanel({ model }: { model: Extract<EntryModel, { empty: false }> }) {
  ...
  return (
    <details className="panel entry-secondary-panel qc-points-panel" open={p.open} onToggle={e => entryDetailToggled('points', (e.target as HTMLDetailsElement).open)}>
...
function RangeSummaryPanel({ r }: { r: EntryRangeSummary }) {
  ...
  return (
    <details className="panel entry-secondary-panel range-summary-panel" open={r.open} onToggle={e => entryDetailToggled('range', (e.target as HTMLDetailsElement).open)}>
      ...
      <div className="entry-secondary-body">
        ...
        {(r.eligible || r.canRevert) ? (
          <div style={{ margin: '8px 14px 0', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {r.eligible ? <button type="button" className="btn teal sm" title="Xem điều kiện, dải đề xuất và phê duyệt" onClick={() => openRangeWorkflow(r.testId, r.level)}>Workflow dải QC</button> : null}
            {r.canRevert ? <button type="button" className="btn ghost icon" title="Về dải nhà sản xuất" onClick={() => revertRange(r.testId, r.level)}>↶</button> : null}
          </div>
        ) : null}
      </div>
    </details>
  ...
  return (
    <>
      ...
      <div className={`entrygrid${model.treeCollapsed ? ' tree-collapsed' : ''}`}>
        <button type="button" className="btn teal icon entry-tree-expand" title="Hiện danh mục nội kiểm" aria-label="Hiện danh mục nội kiểm" aria-controls="entryTreePanel" aria-expanded="false" onClick={toggleEntryTree}><TreeIcon /></button>
